...
if (!state.deprecated.includes(expect)) {
  ...
  render(state);
  console.error(
    `::warning::${pkg}@${expect} is not visible as deprecated yet. The write may still be propagating through the registry CDN, or it may not have landed; if ${pkg} was recently added to the allowlist, check that NPM_AGENT_DX_TOKEN on npmjs.com can write it and has not expired. Re-run this check before re-running the deprecate step.`,
  );
  process.exit(3);
}
