...
- **The shell check's program-name resolution is a single leading token, nothing more.** `/usr/bin/env bash {0}`, `env bash {0}` and `sudo bash {0}` all refuse, even though each really does run the gate script under bash: the resolved program name is `env`/`sudo`, not `bash`, and this rule does not walk past it to find the real interpreter (a documented false positive in the safe direction). A non-literal `runs-on:` (an expression or a matrix reference) is also not resolved to a concrete OS, so an absent shell there stays certified even when the matrix could produce a Windows runner.
