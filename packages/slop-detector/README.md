...
**The gate step's shell.** `audit-gate-shape` also reads which shell actually runs the gate step, and refuses to certify anything it does not resolve to bash: the step's own `shell:` key, else its enclosing job's `defaults.run.shell`, else the workflow's own `defaults.run.shell`. Certified: an absent `shell:` at all three levels (GitHub's own default on Linux/macOS runners), the literal `bash`, and a custom shell command template whose program is bash (`bash -e {0}`, `bash --noprofile --norc -eo pipefail {0}` -- GitHub's own default expansion -- or any other flag combination). Refused: `pwsh`, `powershell`, `python`, `cmd`, `sh` (GitHub's `sh` keyword is its own documented shell, not bash, even where `/bin/sh` happens to be a bash symlink), any other custom command whose program is not bash, a non-literal `${{ ... }}` value at any of the three levels, a `shell:` key present but empty, and an absent shell on a job whose `runs-on:` is a literal naming a Windows runner (a bare `windows` label or a `windows-*` GitHub-hosted name -- Windows runners default to `pwsh`, not bash). An unresolved `runs-on:` (an expression, or a matrix reference) is a documented residual: left certified exactly as an absent shell always was, never resolved to a concrete runner OS. Program names are matched case-sensitively against GitHub's own lower-case keywords, so `Bash`/`BASH` refuse too, on the same reasoning: nothing in GitHub's documented behaviour says a differently-cased name is treated as the same interpreter. A custom bash template's flags are not otherwise inspected, deliberately: `R-bare` permits no statement after the gate command at all, and `R-classify` manages `errexit` itself with an explicit `set +e`/`set -e` and an explicit `exit`, so neither recognised shape's exit-code guarantee depends on the invoking shell's own `-e`/`pipefail` defaults -- a custom bash template missing `-e` is certified exactly like the literal `bash` keyword. This check runs, and refuses, before a registered template is even considered: a template match is an attestation about the script as bash text, which does not hold under a different interpreter.
...
- **The shell check reads the resolved program name, not the full command line.** A custom bash template whose command line does not actually execute the file it is handed (for example a hard-coded `bash -c true {0}`, which never runs `{0}` at all) still certifies: only the template's first token is checked against `bash`, not whether the rest of the line makes any use of `{0}`. A non-literal `runs-on:` (an expression or a matrix reference) is also not resolved to a concrete OS, so an absent shell there stays certified even when the matrix could produce a Windows runner.
