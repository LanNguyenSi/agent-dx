...
anchor](#path-pattern-anchor); a whole file matching any pattern is
...
One rule, for every path pattern family and every entry point: when a `--config`/`configPath` is given AND the checked target lies inside that config file's own directory, every path pattern in the config is matched against the target's path made relative to the config file's directory. Otherwise (no config given, or the target lies outside the config file's directory) the pre-existing per-target resolution applies, unchanged: a directory target anchors to itself, a single-file target anchors to its own parent directory, and stdin content (or the MCP `slop_check` tool's `text` input) anchors to the nearest directory containing a `package.json` walking up from the assumed path, and failing that, `process.cwd()`. "Inside the config file's directory" is decided on resolved absolute paths (`path.resolve`, never `fs.realpathSync`), so a symlinked target is judged by the path it was spelled with, not by where it ultimately points.

This is what makes `check <file> [<file>...] --config slop.config.yml` judge each named file exactly as `check . --config slop.config.yml` judges it, from the same working directory: a root-anchored pattern (e.g. `packages/foo/README.md`) still matches when `packages/foo/README.md` is scanned as an explicit file argument instead of being reached by walking `.`. It also means an out-of-tree or central `--config` (one whose directory does not contain the scanned target, for example `check . --config ../shared/slop.config.yml`, or an absolute path to a config outside the scan) leaves every pattern family at its pre-existing per-target resolution: pointing at a shared config from inside a subdirectory does not retroactively move the anchor to that config's own directory, so a directory scan's own verdict is never affected by an out-of-tree `--config`.

Every path pattern family in the config is covered by the same rule: `review.allowPaths`, `placement.instructionGlobs`, `entrypointGlobs`, `ignorePaths`, `treatAsProse`, and `treatAsCode`. All four entry points, the CLI's file/directory branch, the CLI's `--stdin-path` branch, and the MCP `slop_check` tool's `path` and `text` branches, derive the anchor through the same helper (`resolvePatternAnchor` in `src/util/pattern-anchor.ts`), so a root-anchored pattern reads the same way from any of them.
