...
**Stdin:** the `--stdin-path` value (and the MCP `slop_check` tool's `filename`) is the target path for both the containment decision and every pattern above, so without containment the first three families fall back to the nearest directory holding a `package.json` walking up from that value (and failing that, `process.cwd()`), `treatAsProse`/`treatAsCode` match that value exactly as spelled, and `ignorePaths` never applies at all, since stdin content is scanned directly rather than reached by walking a directory.
