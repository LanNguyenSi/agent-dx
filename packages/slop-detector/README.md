...
`review.allowPaths`, `placement.instructionGlobs`, `entrypointGlobs`, and every other path pattern family in the config are matched against a scanned file's path made relative to one shared anchor: the directory holding the `--config` file. Both resolve every pattern relative to the config file's own directory, never to the individual scan target, so `check <file> [<file>...] --config slop.config.yml` judges each named file exactly as `check . --config slop.config.yml` judges it from the same working directory: a root-anchored pattern (e.g. `packages/foo/README.md`) still matches when `packages/foo/README.md` is scanned as an explicit file argument instead of being reached by walking `.`. Without `--config` there is no config file directory to anchor to, so resolution falls back to per-target defaults instead: a directory target anchors to itself, a single-file target anchors to its own parent directory, and stdin content (`checkText`) anchors to the nearest directory containing a `package.json` walking up from the assumed path, and failing that, `process.cwd()`.
