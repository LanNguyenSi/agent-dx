...
describe("agent-tics", () => {
  it("flags </result> in prose", () => {
    ...
    expect(
      v.find((x) => x.ruleId === "agent-tics/stray-result-tag"),
    ).toBeDefined();
    expect(
      v.find((x) => x.ruleId === "agent-tics/stray-result-tag")?.severity,
    ).toBe("block");
  });
  ...
  it("flags </invoke> in prose", () => {
    ...
    expect(
      v.find((x) => x.ruleId === "agent-tics/stray-invoke-tag"),
    ).toBeDefined();
  });
  ...
  it("flags Claude Code footer in prose", () => {
    ...
    expect(
      v.find((x) => x.ruleId === "agent-tics/claude-code-footer"),
    ).toBeDefined();
  });
  ...
  it("flags duplicate Summary headings", () => {
    ...
    const matches = v.filter(
      (x) => x.ruleId === "agent-tics/doubled-summary-heading",
    );
  ...
  it("does not flag a single Summary heading", () => {
    ...
    expect(
      v.find((x) => x.ruleId === "agent-tics/doubled-summary-heading"),
    ).toBeUndefined();
  });

  it("flags TODO placeholder text", () => {
    const v = checkText(
      "TODO: [insert real description here]",
      "x.md",
      baseOpts(),
    );
    expect(
      v.find((x) => x.ruleId === "agent-tics/placeholder-todo"),
    ).toBeDefined();
  });

  it("does not flag a real TODO with content", () => {
    const v = checkText(
      "TODO: investigate the cache invalidation race",
      "x.md",
      baseOpts(),
    );
    expect(
  ...
  it("respects disable-line scoped to a specific rule", () => {
    const text =
      "</result> <!-- slop-detector:disable-line=agent-tics/stray-result-tag -->";
    const v = checkText(text, "x.md", baseOpts());
    expect(
      v.find((x) => x.ruleId === "agent-tics/stray-result-tag"),
    ).toBeUndefined();
  ...
  it("does not skip unrelated rules with scoped disable", () => {
    const text =
      "</result> seamless <!-- slop-detector:disable-line=agent-tics/stray-result-tag -->";
    const v = checkText(text, "x.md", baseOpts());
    expect(
      v.find((x) => x.ruleId === "agent-tics/stray-result-tag"),
    ).toBeUndefined();
  ...
  it("ellipsis-promise rule is off by default", () => {
    ...
    expect(
      v.find((x) => x.ruleId === "agent-tics/ellipsis-promise"),
    ).toBeUndefined();
  });
  ...
  it("does not flag </result> inside inline code in prose files", () => {
    const v = checkText(
      "The `</result>` artefact is what we catch.",
      "x.md",
      baseOpts(),
    );
    expect(
  ...
  it("scoped disable on a specific rule-id leaves sibling rules active", () => {
    const text =
      "</result> seamless cutting-edge <!-- slop-detector:disable-line=prose-slop/marketing-adjectives -->";
    const v = checkText(text, "x.md", baseOpts());
    expect(
      v.find((x) => x.ruleId === "prose-slop/marketing-adjectives"),
    ).toBeUndefined();
