...
/**
 * Shared by every describe block that compares the `class_closure` output
 * contract block between SKILL.md and implementer.md, so the extraction
 * regex itself cannot drift between call sites (mirrors
 * extractMutationProbesBlock above).
 */
function extractClassClosureBlock(raw: string): string {
...
describe("mutation probe naming and not-applicable signal ship in step 6 and both contract copies", () => {
  ...
  it("each copy pins the mutation_probes field block by its exact sub-field names and result enum, not just cross-copy equality", () => {
    const field =
      'mutation_probes: - mutant: "" file: "" anchor: "" before: "" after: "" verified_applied_via: "" result: killed | survived | not_applicable expectation: met | violated | not_applicable reason: "" restored_verified: "" replayed: false | true';
    expect(skillMd).toContain(field);
    expect(implementerMd).toContain(field);

    // Exercise each pin separately. Cross-copy equality would still pass if
    // both assets lost one enum value; each exact-name pin must reject that
    // coordinated regression on its own.
    const withoutNotApplicableResult = field.replace(
      "result: killed | survived | not_applicable",
      "result: killed | survived",
    );
    expect(skillMd).not.toContain(withoutNotApplicableResult);
...
describe("class_closure fix-round obligation ships in the implementer prompt and contract", () => {
  ...
  it("the installed implementer prompt states the closed field semantics and the unclosed-site risks requirement", () => {
    expect(implementerMd).toContain(
      "and `closed: true` when every site the round found (by search or by source-level closure) is fixed this round, `false` when a found site is not; name an unclosed site in `risks` with the reason.",
    );
  });
  ...
  it("the subagent misfire rule names the omission of class_closure, on any round after the task's first, as a misfire", () => {
    expect(skillMd).toContain(
      "or that omits the `class_closure` field on any round after the task's first",
    );
  });
  ...
  it("both implementer output contracts carry an identical class_closure field (raw, not line-unwrapped)", () => {
    ...
    expect(skillBlock).toContain("closed: true | false");
  });

  it("each copy pins the class_closure field block by its exact sub-field names, not just cross-copy equality", () => {
    const classClosureField =
      'class_closure: kind: enumerated | source | not_applicable command: "" sites: - "" closed: true | false';
    expect(skillMd).toContain(classClosureField);
    expect(implementerMd).toContain(classClosureField);

    // Exercise the enum pin separately from cross-copy equality: a
    // coordinated regression that drops not_applicable from both copies
    // would still pass a byte-for-byte comparison but must fail here.
    const withoutNotApplicable = classClosureField.replace(
      "kind: enumerated | source | not_applicable",
      "kind: enumerated | source",
    );
    expect(skillMd).not.toContain(withoutNotApplicable);
    expect(implementerMd).not.toContain(withoutNotApplicable);
  });

  it("both copies' class_closure block has exactly the four sub-fields in a fixed order", () => {
    const skillBlock = extractClassClosureBlock(readAsset("skill/SKILL.md"));
    const implementerBlock = extractClassClosureBlock(
      readAsset("agents/implementer.md"),
    );
  ...
  it("step 8's halt sentence and the Round-2 halt rule's cross-reference to it state the identical introduced_by_delta scope, built from one shared constant", () => {
    expect(skillMd).toContain(
      `Halt at the first \`recurrence: repeated\` finding ${HALT_SCOPE_PHRASE} (see Round-2 halt rule below): before any further implementer spawn on that task, name split or redesign in \`03-decisions.md\`.`,
    );
    expect(skillMd).toContain(
      `Step 8 of the detailed workflow states the operational halt: stop at the first \`recurrence: repeated\` finding ${HALT_SCOPE_PHRASE}, before any further implementer spawn on the task, and record the split-or-redesign decision in \`03-decisions.md\`.`,
    );
  });
...
describe("fix-round mutation probe replay ships in step 6, step 7, and both implementer contracts", () => {
   ...
   * actual heading in the file, including a versioned parenthetical title,
   * so a future pointer added without its
...
const SIBLING_GUARD_BUNDLE_ALLOWLIST: SiblingGuardAllowlistEntry[] = [
  ...
  },
  {
    ...
    uncitedLines: [988],
    claim:
      "the sentence describes the per-template asset read driven by `listTemplateNames()`; line 968 is the read of a templates-directory asset, while uncited 988 reads the skill asset for a different install step the sentence never mentions.",
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "wrong-sibling-anchor",
    ...
    uncitedLines: [199],
    claim:
      "the sentence claims an inline marker mention survives a re-run; line 207 is the assertion that it did survive, while uncited 199 is the test's own input string, which writes the mention before the run and asserts nothing.",
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "wrong-sibling-anchor",
    ...
    uncitedLines: [962],
    claim:
      "the sentence is explicitly about the path-exists-and-unedited branch; line 954 records the hash inside that branch's own `if`, while uncited 962 is the path-does-not-exist branch's record, which the sentence's own wording excludes.",
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "wrong-sibling-anchor",
    ...
    uncitedLines: [122],
    claim:
      "the sentence claims a plain SECOND run is a byte-for-byte no-op; line 146 is the idempotence block's second-run assertion, while uncited 122 is the same assertion inside an earlier first-install test, which says nothing about a second run.",
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "wrong-sibling-anchor",
  ...
  },
  {
    ...
    uncitedLines: [1133],
    claim:
      "the :156 sentence names the effort-line computation call and the model argument it is computed from; line 1129 is that argument, while uncited 1133 passes the computed local into the agent-composition call that the separately cited `init.ts:1131-1133` sentence describes.",
  },
  {
    doc: "model-preselection.md",
    kind: "wrong-sibling-anchor",
    ...
    uncitedLines: [1497],
    claim:
      "the sentence names the content assertion pinning the five-line default frontmatter; line 1517 is the asserted array element, while uncited 1497 is a comment above the test restating the same literal in prose.",
  },
  {
    doc: "model-preselection.md",
    kind: "wrong-sibling-anchor",
  ...
  },
  {
    ...
    uncitedLines: [991],
    claim:
      'line 968 cites `readAsset(join("templates", name)),`; uncited 991 is instead line 991 in `content: readAsset(join("skill", "references", name)),`, a different site the citing sentence never names.',
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "distant-duplicate-anchor",
    ...
    uncitedLines: [590],
    claim:
      "line 1064 cites `composeClaudeAgentVariant(`; uncited 590 is instead line 590 in `function composeClaudeAgentVariant(`, a different site the citing sentence never names.",
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "distant-duplicate-anchor",
    ...
    uncitedLines: [559],
    claim:
      "the sentence describes `effortLine` being passed into `composeOpencodeAgentVariant` as a parameter; line 679 is that function's own effortLine push, while uncited 559 is the byte-identical push inside `composeOpencodeAgent`, the default (non-variant) composer the sentence does not name.",
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "distant-duplicate-anchor",
  ...
  },
  {
    ...
    anchorKey: "08402497",
    paragraphLine: 198,
    uncitedLines: [1152, 2156],
    claim:
      'line 2585 cites the `harnesses-stickiness gate is immune to an all-unknown-names…` test (`expect(result.stdout).toContain("installed for: claude");`); uncited 1152 and 2156 is instead line 1152 in the `init --yes runs non-interactively and installs` test (`expect(result.stdout).toContain("installed for: claude");`); and line 2156 in the `prints the tiers status in both the \'Found existing install…` test (`expect(second.stdout).toMatch(/installed for: claude.*tiers: true/);`), a different site the citing sentence never names.',
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "distant-duplicate-anchor",
  ...
  },
  {
    ...
    anchorKey: "be999e8e",
    paragraphLine: 327,
    uncitedLines: [558, 1616],
    claim:
      'line 1180 cites the `a plain re-run keeps the previously chosen models` test (`).toContain("model: haiku");`); uncited 558 and 1616 is instead line 558 in the `installs all four adapters; opencode agents omit model: whe…` test (`expect(claudeSlicer).toContain("model: haiku");`); and line 1616 in the `tiers=true, claude, full profile: exactly 15 agent files wi…` test (`expect(explorerLow).toContain("model: haiku");`), a different site the citing sentence never names.',
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "distant-duplicate-anchor",
    ...
    uncitedLines: [1020],
    claim:
      "line 957 cites `installedFiles[relativePath] = recorded;`; uncited 1020 is instead line 1020 in `if (recorded !== undefined) installedFiles[relativePath] = recorded;`, a different site the citing sentence never names.",
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "distant-duplicate-anchor",
    ...
    uncitedLines: [2315],
    claim:
      'line 341 cites the `keeps a user-edited kit file as a conflict and preserves th…` test (`createHash("sha256").update("user edit\\n", "utf8").digest("hex"),`); uncited 2315 is instead line 2315 in the `repo kit-version pin (operator apply support)` test (`createHash("sha256").update(content, "utf8").digest("hex");`), a different site the citing sentence never names.',
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "distant-duplicate-anchor",
    ...
    uncitedLines: [2042],
    claim:
      "the sentence's first clause describes the plain (non-tiers) second run's no-op, correctly anchored at the `a second run changes no file` test's own `report.updated` assertion (line 146); uncited 2042 is the byte-identical assertion inside the `a second run with tiers=true changes no file (idempotent)` test, which the same bullet's second clause already cites separately, by its own anchor, at test/init.test.ts:2026-2037.",
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "distant-duplicate-anchor",
    ...
    uncitedLines: [2358],
    claim:
      "the sentence describes the `tiers: true` second-run no-op; line 2037 is that assertion inside the `a second run with tiers=true changes no file (idempotent)` test, while uncited 2358 is the byte-identical assertion inside the separate `sets a pin, is a byte-for-byte no-op on repeat...` test, which covers the `pin` option, not tiers, and the sentence does not name.",
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "distant-duplicate-anchor",
    ...
    anchorKey: "6b912d8f",
    paragraphLine: 352,
    uncitedLines: [122, 146],
    claim:
      "line 2042 cites the `a second run with tiers=true changes no file (idempotent)` test (`expect(report.updated).toEqual([]);`); uncited 122 and 146 is instead line 122 in the `creates run state, AGENTS.md section, and claude adapter fi…` test (`expect(report.updated).toEqual([]);`); and line 146 in the `a second run changes no file` test (`expect(report.updated).toEqual([]);`), a different site the citing sentence never names.",
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "distant-duplicate-anchor",
    ...
    anchorKey: "13baa7b3",
    paragraphLine: 352,
    uncitedLines: [113, 359],
    claim:
      'line 1516 cites the `a legacy manifest with no tiers field defaults to false and…` test (`"model: sonnet",`); uncited 113 and 359 is instead line 113 in the `creates run state, AGENTS.md section, and claude adapter fi…` test (`expect(slicer).toContain("model: sonnet");`); and line 359 in the `installs the explorer with a read-only posture on both harn…` test (`expect(claudeExplorer).toContain("model: sonnet");`), a different site the citing sentence never names.',
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "distant-duplicate-anchor",
    ...
    uncitedLines: [604],
    claim:
      'line 1949 cites the `opencode: a claude-family model behind a non-anthropic prov…` test (`"model: github-copilot/claude-sonnet-4.6",`); uncited 604 is instead line 604 in the `emits model: line when opencodeModels provides a FQ id` test (`expect(explorer).toContain("model: github-copilot/claude-sonnet-4.6");`), a different site the citing sentence never names.',
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "distant-duplicate-anchor",
  ...
  },
  {
    ...
    uncitedLines: [1022],
    claim:
      'line 1078 cites the `opencode + tiers on + unresolved tier-class models (0 varia…` test (`(note) => !note.includes("-low.md") && !note.includes("-high.md"),`); uncited 1022 is instead line 1022 in the `a claude install with tiers on, then a re-run switching to …` test (`expect(report.notes.every((note) => !note.includes(".opencode"))).toB…`), a different site the citing sentence never names.',
  },
  {
    doc: "install-fence-mechanics.md",
    kind: "distant-duplicate-anchor",
  ...
  },
  {
    ...
    uncitedLines: [1152],
    claim:
      "the sentence describes the harnesses-stickiness F1 regression test asserting a live claude install is not stuck to templates-only; line 2585 is that test's own stdout assertion, while uncited 1152 is the byte-identical assertion inside the unrelated `init --yes runs non-interactively and installs` smoke test in the `cli smoke` describe block, which the sentence does not name.",
  },
  {
    doc: "model-preselection.md",
    kind: "distant-duplicate-anchor",
  ...
  },
  {
    ...
    uncitedLines: [605],
    claim:
      "the sentence describes `composeClaudeAgent`'s read-only-roles push; line 528 is that function's own push, while uncited 605 is the byte-identical push inside `composeClaudeAgentVariant`, the tier-variant sibling covered separately under \"Composition\" below, which this sentence does not name.",
  },
  {
    doc: "model-preselection.md",
    kind: "distant-duplicate-anchor",
    ...
    uncitedLines: [605],
    claim:
      "the sentence names the read-only roles getting `disallowedTools:` right after `effort:` inside `composeClaudeAgent`; line 528 is that function's own push, while uncited 605 is the byte-identical push inside `composeClaudeAgentVariant`, the tier-variant sibling this sentence does not name.",
  },
  {
    doc: "model-preselection.md",
    kind: "distant-duplicate-anchor",
    ...
    anchorKey: "13baa7b3",
    paragraphLine: 133,
    uncitedLines: [359, 1516],
    claim:
      'line 113 cites the `creates run state, AGENTS.md section, and claude adapter fi…` test (`expect(slicer).toContain("model: sonnet");`); uncited 359 and 1516 is instead line 359 in the `installs the explorer with a read-only posture on both harn…` test (`expect(claudeExplorer).toContain("model: sonnet");`); and line 1516 in the `a legacy manifest with no tiers field defaults to false and…` test (`"model: sonnet",`), a different site the citing sentence never names.',
  },
  {
    doc: "model-preselection.md",
    kind: "distant-duplicate-anchor",
    ...
    uncitedLines: [682],
    claim:
      "the sentence describes `composeOpencodeAgent`'s read-only-roles push (`permission:`/`edit: deny`); line 562 is that function's own push, while uncited 682 is the byte-identical push inside `composeOpencodeAgentVariant`, the tier-variant sibling covered separately under \"Composition\" below, which this sentence does not name.",
  },
  {
    doc: "model-preselection.md",
    kind: "distant-duplicate-anchor",
    ...
    anchorKey: "10aeb3a5",
    paragraphLine: 168,
    uncitedLines: [1398, 2262],
    claim:
      'line 2232 cites the `writes the --opencode-provider hint to STDERR (not stdout) …` test (`expect(result.stdout).not.toContain("--opencode-provider");`); uncited 1398 and 2262 is instead line 1398 in the `a subsequent run with harnesses: [] over a claude install l…` test (`expect(result.stdout).not.toContain("installed for: ");`); and line 2262 in the `--tiers with an empty catalog warns once per unresolved mod…` test (`expect(result.stdout).not.toContain(`Tier model class`);`), a different site the citing sentence never names.',
  },
  {
    doc: "model-preselection.md",
    kind: "distant-duplicate-anchor",
  ...
  },
  {
    ...
    uncitedLines: [562],
    claim:
      "the sentence describes `composeOpencodeAgentVariant`'s own read-only-roles push; line 682 is that function's own push, while uncited 562 is the byte-identical push inside `composeOpencodeAgent`, the default (non-variant) composer this sentence does not name.",
  },
  {
    doc: "model-preselection.md",
    kind: "distant-duplicate-anchor",
    ...
    anchorKey: "bd75126a",
    paragraphLine: 351,
    uncitedLines: [1996, 2299],
    claim:
      "line 2266 cites the `--tiers with an empty catalog warns once per unresolved mod…` test (`expect(agents.sort()).toEqual([`); uncited 1996 and 2299 is instead line 1996 in the `opencode: an unresolved class model (undefined) renders no …` test (`expect(agents.sort()).toEqual([`); and line 2299 in the `--tiers with fully-qualified --models but still no live cat…` test (`expect(agents.sort()).toEqual([`), a different site the citing sentence never names.",
  },
  {
    doc: "model-preselection.md",
    kind: "distant-duplicate-anchor",
  ...
  },
  {
    ...
    anchorKey: "44b3936a",
    paragraphLine: 378,
    uncitedLines: [974, 994],
    claim:
      "line 1556 cites the `default agent files are byte-identical whether tiers is on …` test (`tiers: false,`); uncited 974 and 994 is instead line 974 in the `opencode + unresolved tier-class models (0 variant files ev…` test (`tiers: false,`); and line 994 in the `a claude install with tiers on, then a re-run switching to …` test (`tiers: false,`), a different site the citing sentence never names.",
  },
  {
    doc: "model-preselection.md",
    kind: "distant-duplicate-anchor",
  ...
  },
  {
    ...
    uncitedLines: [1773],
    claim:
      'line 1926 cites the `opencode: a non-anthropic, non-ollama resolved class id get…` test (`expect(implementerHigh).toContain("reasoningEffort: high");`); uncited 1773 is instead line 1773 in the `opencode default files: a non-Claude-family provider-qualif…` test (`expect(rendered).toContain("reasoningEffort: high");`), a different site the citing sentence never names.',
  },
  {
    doc: "model-preselection.md",
    kind: "distant-duplicate-anchor",
  ...
  },
  {
    ...
    uncitedLines: [605],
    claim:
      "the sentence names `composeClaudeAgent` as the function 0.22.0 gave a pinned `effort:` line; line 528 is that function's own read-only-roles push, included in the cited range, while uncited 605 is the byte-identical push inside `composeClaudeAgentVariant`, the tier-variant sibling this sentence does not name.",
  },
  {
    doc: "model-preselection.md",
    kind: "distant-duplicate-anchor",
    ...
    uncitedLines: [682],
    claim:
      "the sentence names `composeOpencodeAgent` as the function 0.22.0 gave a pinned effort line; line 562 is that function's own read-only-roles push, included in the cited range, while uncited 682 is the byte-identical push inside `composeOpencodeAgentVariant`, the tier-variant sibling this sentence does not name.",
  },
  {
    doc: "model-preselection.md",
    kind: "distant-duplicate-anchor",
  ...
  },
  {
    ...
    uncitedLines: [602],
    claim:
      "line 73 cites the test that 00-goal.md carries the keyed run-base placeholder line byte-exactly; uncited 602 repeats the identical marker line at a wholly separate site the citing sentence never names.",
  },
  {
    doc: "run-state-lifecycle-and-markers.md",
    kind: "distant-duplicate-anchor",
    ...
    uncitedLines: [602],
    claim:
      "line 95 cites the test that the keyed run-base placeholder line sits directly below the unkeyed one; uncited 602 repeats the identical marker line at a wholly separate site the citing sentence never names.",
  },
  {
    doc: "run-state-lifecycle-and-markers.md",
    kind: "distant-duplicate-anchor",
