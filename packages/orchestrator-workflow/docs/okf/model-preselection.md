...
2. **Base resolution + interactive fallback.** The CLI builds the base map
   ...
   already-resolved value (`src/cli-inputs.ts:267-273#"...DEFAULT_MODELS,"`, prompt UI at
   `src/cli-inputs.ts:89-131#"return models;"`). Since agent-dx task T-003, this whole
   ...
   the roles `rolesForProfile(profile)` selects (`src/cli-inputs.ts:89-94#"for (const role of roles) {"`
   ...
   with no interactive prompt at all (`src/cli-inputs.ts:280-290#"const tiers = opts.tiers ?? previous?.tiers ?? false;"`; see "Effort
   tiers" below).
3. **Manifest.** `runInit` writes the resolved map to
   `.ai/workflow/manifest.json` under `models` (`src/init.ts:710-747#"force: true,"`,
...
4. **Per-harness frontmatter.** For each selected harness, `runInit` calls a
   ...
   invocations at `src/init.ts:639-641#"composeClaudeAgent(role, options.models[role]),"` for Claude Code and `:580-582#"composeOpencodeAgent(role, modelValue, defaultEffortLine),"` for
...
- **opencode.** opencode needs a fully-qualified `provider/model-id`.
  ...
  `opencodeEffortLine(DEFAULT_TIER[role], modelValue)` (`src/init.ts:669#"modelValue,"`)
...
- **Codex.** Codex gets no per-role subagent definition files at all: `init`
  ...
  `codex` harness (`src/init.ts:656-657#".agents"`); there is no `model:` surface for
...
(`src/cli-inputs.ts:280-290#"const tiers = opts.tiers ?? previous?.tiers ?? false;"`; the code comment there states explicitly why: tiers
...
(`init.ts:643-648#"composeClaudeAgentVariant(role, tier),"` Claude Code,
...
(`cli-inputs.ts:311-337#"// and need no live catalog lookup, so they are unaffected)."`) now returns one warning
...
opencode tier loop (`init.ts:675-702#"effortLine,"`) now checks whether the variant's
class model resolved at all (`variantModelValue !== undefined`,
`init.ts:680-692#"// rules in opencodeEffortLine above."`) and skips the write, and the manifest ledger entry,
...
(`cli-inputs.ts:311-337#"// and need no live catalog lookup, so they are unaffected)."`) is `Warning: Tier model class "<class>" (alias
...
**Rendering (`init.ts:636-702#"effortLine,"`).** For each harness and each role
...
`cli-inputs.ts:311-337#"// and need no live catalog lookup, so they are unaffected)."`, mirroring the existing per-role opencode resolution just above
it at `cli-inputs.ts:298-309#"warnings.push("`). The Claude-family-`variant:` and Ollama-no-effort-field
...
at the call site, `init.ts:669#"modelValue,"`, and passed in as a parameter the same
...
interactive branch: `opts.tiers ?? previous?.tiers ?? false` (`cli-inputs.ts:290#"const tiers = opts.tiers ?? previous?.tiers ?? false;"`).
...
`installKitFile` hash ledger as every other kit-owned file (`init.ts:587-607#"installFile(report, path, content, { force });"`,
...
...(previous?.models ?? {}) }` in `src/cli-inputs.ts:269-271#"if (interactive) profile = await promptProfile(profile);"`. The same
override-vs-persist rule now also covers `--profile` (`src/cli-inputs.ts:255-265#"if (opts.profile) {"`)
and, since 0.19.0, `--tiers`/`--no-tiers` (`src/cli-inputs.ts:280-290#"const tiers = opts.tiers ?? previous?.tiers ?? false;"`, see
