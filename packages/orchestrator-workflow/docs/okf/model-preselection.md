...
2. **Base resolution + interactive fallback.** The CLI builds the base map
   ...
   already-resolved value (`src/cli-inputs.ts:332-332#"...DEFAULT_MODELS,"`, prompt UI at
   ...
   with no interactive prompt at all (`src/cli-inputs.ts:349-349#"const tiers = opts.tiers ?? previous?.tiers ?? false;"`; see "Effort
   tiers" below).
...
(`src/cli-inputs.ts:349-349#"const tiers = opts.tiers ?? previous?.tiers ?? false;"`; the code comment there states explicitly why: tiers
...
(`cli-inputs.ts:396-396#"// and need no live catalog lookup, so they are unaffected)."`) now returns one warning
...
(`cli-inputs.ts:396-396#"// and need no live catalog lookup, so they are unaffected)."`) is `Warning: Tier model class "<class>" (alias
...
`cli-inputs.ts:396-396#"// and need no live catalog lookup, so they are unaffected)."`, mirroring the existing per-role opencode resolution just above
it at `cli-inputs.ts:397#"warnings.push("`). The Claude-family-`variant:` and Ollama-no-effort-field
...
interactive branch: `opts.tiers ?? previous?.tiers ?? false` (`cli-inputs.ts:349#"const tiers = opts.tiers ?? previous?.tiers ?? false;"`).
...
...(previous?.models ?? {}) }` in `src/cli-inputs.ts:328-328#"if (interactive) profile = await promptProfile(profile);"`. The same
override-vs-persist rule now also covers `--profile` (`src/cli-inputs.ts:324-324#"if (opts.profile) {"`)
and, since 0.19.0, `--tiers`/`--no-tiers` (`src/cli-inputs.ts:349-349#"const tiers = opts.tiers ?? previous?.tiers ?? false;"`, see
