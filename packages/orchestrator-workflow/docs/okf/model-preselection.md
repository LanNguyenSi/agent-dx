...
Since 0.19.0, a standalone `describe` (`test/docs-consistency.test.ts:1687#"defaultTier: defaultTierCell.trim(),"`) guards a tier-specific
...
(review finding L4), a second, sibling `describe` (`test/docs-consistency.test.ts:1766#"const def = TIER_DEFS[tier];"`) guards
...
third, site-specific `describe` (`test/docs-consistency.test.ts:1827#"return readmeMd.slice(startIdx, endIdx);"`)
...
(`test/docs-consistency.test.ts:2432#"must not sit inside the tiers-gated clause"`) guards the pinned-default-effort
