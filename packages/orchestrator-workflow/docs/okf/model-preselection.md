...
2. **Base resolution + interactive fallback.** The CLI builds the base map
   ...
   already-resolved value (`src/cli-inputs.ts:245-251#"...DEFAULT_MODELS,"`, prompt UI at
   `src/cli-inputs.ts:78-120#"return models;"`). Since agent-dx task T-003, this whole
   ...
   the roles `rolesForProfile(profile)` selects (`src/cli-inputs.ts:78-83#"for (const role of roles) {"`
   ...
   with no interactive prompt at all (`src/cli-inputs.ts:258-268#"const tiers = opts.tiers ?? previous?.tiers ?? false;"`; see "Effort
   tiers" below).
3. **Manifest.** `runInit` writes the resolved map to
   `.ai/workflow/manifest.json` under `models` (`src/init.ts:690-727#"force: true,"`,
   ...
   (`src/init.ts:143-226#"? { pin: candidate.pin.trim() }"`, specifically `:148-157`).
4. **Per-harness frontmatter.** For each selected harness, `runInit` calls a
   ...
   into that harness's frontmatter shape (`src/init.ts:235-296#"permission:"`,
   invocations at `src/init.ts:619-621#"composeClaudeAgent(role, options.models[role]),"` for Claude Code and `:580-582#"composeOpencodeAgent(role, modelValue, defaultEffortLine),"` for
   ...
   (`src/init.ts:315-326#"disallowedTools: Edit, Write, NotebookEdit"` Claude Code, `:362-385` opencode) composes the
...
- **Claude Code.** `composeClaudeAgent` always emits a `model:` line;
  ...
  through unchanged (`src/models.ts:88-89#"return (MODEL_ALIASES as string[]).includes(value);"`, `src/init.ts:251-262#"disallowedTools: Edit, Write, NotebookEdit"`). Since
  0.22.0 it also always emits a pinned `effort:` line right after `model:`
  (`TIER_DEFS[DEFAULT_TIER[role]].effort`, `src/init.ts:258#"effort: ${TIER_DEFS[DEFAULT_TIER[role]].effort}"`; medium for
  ...
  (`src/init.ts:261-262#"disallowedTools: Edit, Write, NotebookEdit"`). Test coverage:
...
- **opencode.** opencode needs a fully-qualified `provider/model-id`.
  ...
  (`src/init.ts:276-296#"permission:"`, comment at `:268-269#"// Omitting it lets the subagent inherit the session/default model."`). Since 0.22.0 it also takes
  an `effortLine` parameter, computed once by the caller via
  `opencodeEffortLine(DEFAULT_TIER[role], modelValue)` (`src/init.ts:649#"modelValue,"`)
...
- **Codex.** Codex gets no per-role subagent definition files at all: `init`
  ...
  `codex` harness (`src/init.ts:636-637#".agents"`); there is no `model:` surface for
...
(`src/cli-inputs.ts:258-268#"const tiers = opts.tiers ?? previous?.tiers ?? false;"`; the code comment there states explicitly why: tiers
...
(`init.ts:623-628#"composeClaudeAgentVariant(role, tier),"` Claude Code,
...
**Composition.** `composeClaudeAgentVariant` (`init.ts:315-326#"disallowedTools: Edit, Write, NotebookEdit"`) is the
...
`READ_ONLY_ROLES`. `composeOpencodeAgentVariant` (`init.ts:384-403#"permission:"`) is the
opencode sibling; its effort line is decided by `opencodeEffortLine`
(`init.ts:361-374#"reasoningEffort: ${TIER_DEFS[tier].effort}"`, renamed from `opencodeVariantEffortLine` in 0.22.0 since
...
`isClaudeFamilyModel` (`init.ts:341-345#"return remainder.includes("`) now treats a resolved model id as
...
composers share `tierDescriptionSuffix` (`init.ts:302-306#"${asset.description} (Effort tier: ${tier}.)"`), which appends
...
(`cli-inputs.ts:289-315#"// and need no live catalog lookup, so they are unaffected)."`) now returns one warning
...
opencode tier loop (`init.ts:655-682#"effortLine,"`) now checks whether the variant's
class model resolved at all (`variantModelValue !== undefined`,
`init.ts:660-672#"// rules in opencodeEffortLine above."`) and skips the write, and the manifest ledger entry,
...
(`cli-inputs.ts:289-315#"// and need no live catalog lookup, so they are unaffected)."`) is `Warning: Tier model class "<class>" (alias
...
**Rendering (`init.ts:616-682#"effortLine,"`).** For each harness and each role
...
`cli-inputs.ts:289-315#"// and need no live catalog lookup, so they are unaffected)."`, mirroring the existing per-role opencode resolution just above
it at `cli-inputs.ts:276-287#"warnings.push("`). The Claude-family-`variant:` and Ollama-no-effort-field
...
`composeClaudeAgent` (`init.ts:251-262#"disallowedTools: Edit, Write, NotebookEdit"`) and `composeOpencodeAgent`
(`init.ts:276-296#"permission:"`) add the role's own
`` `effort: ${TIER_DEFS[DEFAULT_TIER[role]].effort}` `` (Claude Code,
`init.ts:258#"effort: ${TIER_DEFS[DEFAULT_TIER[role]].effort}"`) or the equivalent opencode effort line (via
`opencodeEffortLine(DEFAULT_TIER[role], modelValue)`, computed once per role
at the call site, `init.ts:649#"modelValue,"`, and passed in as a parameter the same
...
`false` (`init.ts:199-203#"const tiers = typeof candidate.tiers ==="`), the same per-field-degradation style already
...
interactive branch: `opts.tiers ?? previous?.tiers ?? false` (`cli-inputs.ts:268#"const tiers = opts.tiers ?? previous?.tiers ?? false;"`).
...
`full -> minimal` profile downgrade does (`init.ts:502-512#"now untracked after tiers were turned off"`, review finding
...
(`init.ts:477-488#"now untracked after the full -> ${profile} profile downgrade; run"`, review finding M3;
...
`installKitFile` hash ledger as every other kit-owned file (`init.ts:567-587#"installFile(report, path, content, { force });"`,
...
...(previous?.models ?? {}) }` in `src/cli-inputs.ts:245-247#"if (interactive) profile = await promptProfile(profile);"`. The same
override-vs-persist rule now also covers `--profile` (`src/cli-inputs.ts:233-243#"if (opts.profile) {"`)
and, since 0.19.0, `--tiers`/`--no-tiers` (`src/cli-inputs.ts:258-268#"const tiers = opts.tiers ?? previous?.tiers ?? false;"`, see
...
(`src/init.ts:143-226#"? { pin: candidate.pin.trim() }"`; end-to-end proof at `test/init.test.ts:278-319#"expect(manifest.models.implementer).toBe("`,
