...
`README.md:295#"escalations happen precisely when the situation is hard, so it shares the reviewer's strongest-model default"`):
...
- **opencode.** opencode needs a fully-qualified `provider/model-id`.
  ...
  must be passed as fully-qualified `--models` entries (`README.md:348#"reviewer=openrouter/anthropic/claude-opus-4.8"`,
...
paragraph stating the same rule (`README.md:374#"effort deterministic and independent of the caller's session."`), and the CHANGELOG
...
Since 0.19.0, a standalone `describe` (`test/docs-consistency.test.ts:1774#"defaultTier: defaultTierCell.trim(),"`) guards a tier-specific
...
(review finding L4), a second, sibling `describe` (`test/docs-consistency.test.ts:1853#"const def = TIER_DEFS[tier];"`) guards
...
third, site-specific `describe` (`test/docs-consistency.test.ts:1914#"return readmeMd.slice(startIdx, endIdx);"`)
...
(`test/docs-consistency.test.ts:2519#"must not sit inside the tiers-gated clause"`) guards the pinned-default-effort
