...
- 2026-09-26T11:29:00Z (deferred sources fix): `subagent-contracts-superset.md`
  ...
  offset. A new bullet inserted at the top of `CHANGELOG.md`'s
  `[Unreleased]` section shifts every line below it by seven; this log's
  repeated pair of self-citations into that file's "The implementer" and
  "Citation scanning is paragraph-joined" bullets move by the same offset.
  A new docs-consistency test (INSTALL-AGENT.md's package-README section
  pointers) was appended to the end of `test/docs-consistency.test.ts`,
  after every existing citation site in that file, so no other citation
  into it shifted. `model-preselection.md`, `review-gate-and-waivers.md`,
  and `run-state-lifecycle-and-markers.md` also list both edited files as
  sources and are re-stamped alongside `subagent-contracts-superset.md`;
  their claims about `test/docs-consistency.test.ts` and `CHANGELOG.md`
  were re-read against the edit and stay accurate, since neither edit
  removed, moved, or renumbered anything an existing citation depends on.
...
- 2026-09-26T06:49:29Z (README restructure, second pass): `packages/orchestrator-workflow/README.md`
  ...
  `docs/verification-sets.md`; the per-harness file table, the read-only
  posture paragraph, and the Bash-residual honesty paragraph to
  `docs/harnesses.md`; the model-preselection default table, the
  `--routing` JSON example, the Codex defaults table, the opencode
  model-resolution paragraph, and the entire "Effort tiers" section
  (both tables plus the opencode-effort prose) to
  `docs/model-routing-reference.md`; the entire "Operator-level install"
  section to `docs/operator-install.md`. Each `docs-consistency.test.ts`
  ...
  `test/knowledge-sites.test.ts` now scans `docs/verification-sets.md`
  in place of the JSON example's old README.md location. `package.json`
  `files` gained the nine new reference docs (not `docs/okf`, the
  curated knowledge bundle, which stays out of the npm package); the
  `CHANGELOG.md` `[Unreleased]` section gained one entry, shifting
  every citation into it below its insertion point by 9 lines.
  Re-pointed, one file moving from a line in the README to a line in
  its new home: `install-fence-mechanics.md`'s one `README.md` citation
  moved into `docs/harnesses.md`; `model-preselection.md`'s three
  `README.md` citations moved into `docs/model-routing-reference.md`;
  `run-state-lifecycle-and-markers.md`'s three `README.md` citations
  stayed in `README.md`, at their new (shifted) lines, since that
  content did not move this round. Also re-pointed: every citation into
  `test/docs-consistency.test.ts` carried by `model-preselection.md`,
  `review-gate-and-waivers.md`, `run-state-lifecycle-and-markers.md`,
  and `subagent-contracts-superset.md` (dozens, mechanically re-pointed
  by locating each citation's own anchor text at its new line, since the
  test file's line count grew by two net and individual regions shifted
  by different small amounts once `prettier --write` reformatted the
  renamed variables), and the two citations into `CHANGELOG.md` shared
  across those same docs, whose one shared insertion point pushed both
  down by nine lines. All five re-checked and re-stamped; the bundle
  anchor check reports zero findings.
...
- 2026-09-25T12:15:34Z (task 2348e6f1, knowledge manifest hardening, stored-value
  ...
  stored as well as on the value as written, so a prefix that
  normalisation strips (`./C:x`, `a/../C:x`, `docs/../C:/x`) can no longer
  store a drive or absolute path, and every stored value is accepted again
  on read. Claim changed: `install-fence-mechanics.md` says so next to the
  drive-letter rule. Citations into `src/init.ts`, `test/init.test.ts`,
  `README.md` and `CHANGELOG.md` shifted by the edits were re-pointed from
  a line map of the previous and new files, and the sibling-guard allowlist
  geometry in `test/docs-consistency.test.ts` moved with them. Every doc
  whose `sources` list an edited file was re-checked and re-stamped.

- 2026-09-25T12:02:54Z (task b4d8f0e9, hand-off step names documentation impact): the
  Hand off step in `evidence-and-probes.md` (step 9) and its copy in
  `SKILL.md` (step 6) name documentation impact (none with a reason,
  updated paths, or a follow-up) among what the orchestrator records and
  reports; `test/docs-impact.test.ts` pins both clauses and `CHANGELOG.md`
  gains its `[Unreleased]` bullet. No bundle claim changed: no doc
  enumerates the hand-off report items. Citations into the shifted
  `evidence-and-probes.md` lines after step 9's report sentence and into
  the shifted `CHANGELOG.md` lines (the two self-citations included, in the
  live entries and in the earlier entries that quote them) were re-pointed
  through a line map from the base revision, every moved anchor checked by
  the bundle validator. Every doc whose `sources` list an edited file was
  re-checked and re-stamped.
...
- 2026-09-25T11:50:15Z (task e92008cf, released-bullet pins): `test/probe-plans-recovery.test.ts`
  no longer pins the fix-regression trigger or the probe verdict clauses
  against released `CHANGELOG.md` bullets, and its bundle doc copy pins strip
  okf citation parentheticals before matching. `CHANGELOG.md` gains its
  `[Unreleased]` bullet; the two self-citations into it moved with the new
  bullet and were re-pointed here after re-reading both anchors at their new
  lines. Claims changed: `subagent-contracts-superset.md` no longer names the
  CHANGELOG among the sites the probe verdict pin binds. Every doc whose
  `sources` list an edited file was re-checked and re-stamped.

- 2026-09-25T11:46:54Z (task 2348e6f1, knowledge manifest hardening, drive-relative
  paths): a `knowledge` `path` or `repoRoot` starting with a Windows drive
  letter that is not absolute (`C:x`, `C:..`, `C:`) is now rejected as a
  Windows drive path, since Windows resolves it against that drive's
  current directory. Claim changed: `install-fence-mechanics.md` names the
  drive-letter rule next to the backslash rule. Citations into
  `src/init.ts`, `test/init.test.ts`, `README.md` and `CHANGELOG.md`
  shifted by the edits were re-pointed from a line map of the previous and
  new files, with their anchors re-checked, and the sibling-guard allowlist
  geometry in `test/docs-consistency.test.ts` moved with them. Every doc
  whose `sources` list an edited file was re-checked and re-stamped.
...
- 2026-09-25T11:45:56Z (task a0effc8f, bundle-doc intersection edge cases): `contracts.md`
  ...
  workspace bundle's paths are rebased onto its `repoRoot` with the expansion
  run inside that repository. Claims changed: `subagent-contracts-superset.md`
  ...
  (`CHANGELOG.md:718#"The implementer"` and
  `CHANGELOG.md:1095#"Citation scanning is paragraph-joined"`), re-pointed in
...
- 2026-09-25T11:31:02Z (task 2348e6f1, knowledge manifest hardening): a
  `knowledge` `path` or `repoRoot` containing a backslash, or a Windows
  drive path, is now rejected, and a re-install that rewrites the manifest
  prints a note for each invalid hand-edited entry it removes from disk.
  Claim changed: `install-fence-mechanics.md` describes the
  `knowledgeProblems` read field, the re-install note and the backslash
  rule. Every citation into `src/init.ts`, `test/init.test.ts`,
  `README.md` and `CHANGELOG.md` shifted by the edits was re-pointed from a
  line map of the base and head files, with its anchor re-read at the new
  line, and the sibling-guard allowlist geometry in
  `test/docs-consistency.test.ts` moved with it. Every doc whose `sources`
  list an edited file was re-checked and re-stamped.
...
- 2026-09-25T11:22:23Z (task 433b78d5, bundle gate in CI wording): `bundle-gate-in-ci.md`
  ...
  in the bundle docs is heading-anchored to a released section, so no claim
  changed. The new bullet moves the two live self-citations of this log to
  `CHANGELOG.md:718#"The implementer"` and
  `CHANGELOG.md:1095#"Citation scanning is paragraph-joined"`, re-pointed in
  the live sites and in the earlier entries that quote them after re-reading
  both anchors at the new lines. The docs whose `sources` list `CHANGELOG.md`
...
- 2026-09-25T10:02:27Z (task e5ec01f3, bundle gate in CI): a new skill reference
  ...
  `CHANGELOG.md` gains its `[Unreleased]` bullet and `test/init.test.ts` its
  installed-file count. No bundle claim changed: no doc cites the new
  reference, `SKILL.md`, or the changed test line. The two self-citations
  into `CHANGELOG.md` moved with the new bullet and were re-pointed here
  after re-reading both anchors at their new lines. Every doc whose
  `sources` list an edited file was re-checked and re-stamped.
...
- 2026-09-25T08:28:29Z (task ee0a02ac, knowledge-bundle manifest field):
  ...
  configured path, unconfigured `docs/okf/`); `explorer.md`, `implementer.md`,
  `reviewer.md`, `task-slicer.md`, `evidence-and-probes.md` (three sites),
  `02-tasks.md`, `06-handoff.md` and `README.md` (two sites) now name the
  configured knowledge-bundle list instead of hard-coding `docs/okf/` as the
  sole locator. `install-fence-mechanics.md`, `model-preselection.md`,
  `operator-install-and-registry.md`, `review-gate-and-waivers.md`,
  `run-state-lifecycle-and-markers.md` and `subagent-contracts-superset.md`
  citations into the shifted files were re-pointed to their new lines and
  every moved anchor re-read; two allowlist entries in
  `test/docs-consistency.test.ts` whose `claim` text quoted the pre-shift
  line numbers were updated to the new ones. Re-checked and re-stamped;
  the `06-handoff.md` comment quoted in `run-state-lifecycle-and-markers.md`
  was missed here and is corrected in the entry above.

- 2026-09-25T07:56:21Z (task 7e6dd342, noted-ref wording): `evidence-and-probes.md` step 6
  now exempts a noted ref only while it stays at its sha, treats an
  unresolvable mover as a misfire, and marks the limits list as examples;
  the paragraph grew by five lines and citations below it were re-pointed,
  every moved anchor re-read. `review-gate-and-waivers.md`,
  `run-state-lifecycle-and-markers.md` and `subagent-contracts-superset.md`
...
- 2026-09-25T06:18:59Z (task 7e6dd342, redesign round): the `outward` marker was
  ...
  a push to the default branch, or a merge into it); every other outward
  action always needs per-action operator confirmation. The canonical
  action-class token list was replaced by that two-class sentence in
  `agents-md-section.md`; the value grammar (one definition site,
  `run-state-and-harness.md`) now also says more than one `outward` line
  means `none` and an unknown token is reported. A reader-side grant
  condition was added in the policy section, `SKILL.md`, and
  `run-state-and-harness.md`: a class counts only with an
  operator-instruction record in `03-decisions.md` sourced from a session
  message, a new run starts at `none`, and an untraceable class on resume is
  not granted. The orchestrator's cross-check lists the remote's refs by
  sha instead of the task branch by name. Every subagent prompt and both
  output contracts make reporting a performed outward action mandatory;
  `advisor.md` gained the never-perform line. The okf outward section in
  `run-state-lifecycle-and-markers.md` was rewritten to the narrow design
  and its heading changed from "fails OPEN" to "fails closed (no grant)",
  with the meaning spelled out. Citations into the touched assets and
  `CHANGELOG.md` across `log.md`, `model-preselection.md`,
  `review-gate-and-waivers.md`, `run-state-lifecycle-and-markers.md`, and
  `subagent-contracts-superset.md` were re-pointed; no re-pointed claim
  changed. `install-fence-mechanics.md` (unanchored `agents-md-section.md`
  source, content-only rewording inside the fence), `model-preselection.md`,
  `review-gate-and-waivers.md`, `run-state-lifecycle-and-markers.md`, and
...
- 2026-09-25T05:42:46Z (task 7e6dd342, review fix round): the Outward-facing
  ...
  `agents-md-section.md`) with a per-token ignore-unknown rule; the rule's
  scope was reworded everywhere so no "unless" can be read as lifting
  orchestrator-only, even when a class is durably authorized; an outward
  action is now defined by boundary (any write outside the local checkout
  and the run directory) with the class list as examples; the marker
  gained a provenance rule (a class enters it only on the operator's
  explicit instruction, recorded in `03-decisions.md`, never by a
  subagent or the orchestrator's own judgment); the orchestrator's
  mechanical cross-check was corrected from an ahead-count comparison
  (which a push without `-u` or a from-sha worktree defeats) to a
  run-base commit-range comparison plus a remote check for the task
  branch; recovering a report of a performed-but-unauthorized outward
  action now routes to the operator and `03-decisions.md`, not just the
  misfire rule; `explorer.md` and `task-slicer.md` each gained a
  never-perform-an-outward-action line; and the marker's own
  `<!-- outward: classes = none -->` line shape now matches the kit's
  other plain-record marker, `review-round-escalation`. A new
  `## The outward marker: a record, fails OPEN` section was added to
  `run-state-lifecycle-and-markers.md` (pure append, no earlier citation in
  that file moved). Citations into `agents-md-section.md`, `SKILL.md`,
  `implementer.md`, `reviewer.md`, `explorer.md`, `task-slicer.md`,
  `00-goal.md`, `06-handoff.md`, `evidence-and-probes.md`,
  `review-and-recovery.md`, `run-state-and-harness.md`, and `CHANGELOG.md`
  across `log.md`, `model-preselection.md`, `review-gate-and-waivers.md`,
  `run-state-lifecycle-and-markers.md`, and `subagent-contracts-superset.md`
  were re-pointed to the shifted line numbers; no re-pointed claim changed.
...
- 2026-09-25T05:05:40Z (task 7e6dd342): the new Outward-facing actions rule
  ...
  template, a `Sent / Drafted Outward` section to the `06-handoff.md`
  template, and prose to `contracts.md`, `evidence-and-probes.md`,
  `review-and-recovery.md`, `run-state-and-harness.md`, and `CHANGELOG.md`.
  Citations into these files across `log.md`, `model-preselection.md`,
  `review-gate-and-waivers.md`, `run-state-lifecycle-and-markers.md`, and
  `subagent-contracts-superset.md` were re-pointed to the shifted line
  numbers; no cited claim changed. `model-preselection.md`,
  `review-gate-and-waivers.md`, `run-state-lifecycle-and-markers.md`, and
  `subagent-contracts-superset.md` re-checked and re-stamped.
...
- 2026-09-24T06:48:31Z (release 0.40.1): the `[Unreleased]` notes moved under
  ...
  the new heading were re-pointed by two lines. No bundle doc cited the
  `[Unreleased]` heading, so no heading citation changed.
  `review-gate-and-waivers.md`, `run-state-lifecycle-and-markers.md` and
  `subagent-contracts-superset.md` re-checked and re-stamped.
...
- 2026-09-24T06:16:59Z (task 19a63460): rebased onto the 0.37.0 CHANGELOG restore; its
  ...
  self-citations to `CHANGELOG.md:718#"The implementer"` and
  `CHANGELOG.md:1095#"Citation scanning is paragraph-joined"`, re-pointed in the
...
- 2026-09-24T06:01:54Z (agent-dx tracker task 19a63460): `implementer.md`,
  ...
  effective config and scripts, and preflight executable identity/
  definition, at the tree the set executes in and defers repository
  identity to the path rule that follows, instead of comparing repository
  identity with the tree the role runs in next to a path rule that
  compares with the tree the diff comes from. `CHANGELOG.md` gained an
  `[Unreleased]` entry for it, shifting the two citations below its new
  heading by eight lines. `subagent-contracts-superset.md` re-checked and
  re-stamped.
...
- 2026-09-24T05:54:19Z (task 61599d23): restored the `## [0.37.0]` section
  ...
  had applied to an already-released section. `test/probe-plans-recovery.test.ts`
  no longer asserts the ceremony-rule constants against a CHANGELOG bullet
  (that bullet is now the released 0.37.0 wording again, and the 0.39.0
  bullet only summarizes the tightening rather than repeating it verbatim);
  it keeps the bundle-doc assertion as the live copy pin.
  `review-gate-and-waivers.md`'s own "Ceremony rules" section was reworded
  to say the pin runs against the references and that section, not the
  CHANGELOG bullet, and to note the 0.37.0 entry stays as released.
  `review-gate-and-waivers.md`, `run-state-lifecycle-and-markers.md`, and
...
- 2026-09-23T21:10:00Z (release 0.40.0): the `[Unreleased]` notes moved under
  `## [0.40.0] - 2026-09-23`. Line citations into `CHANGELOG.md` below
  the new heading were re-pointed by two lines. No bundle doc cited the
  `[Unreleased]` heading, so no heading citation changed.
  `review-gate-and-waivers.md`, `run-state-lifecycle-and-markers.md` and
  `subagent-contracts-superset.md` re-checked and re-stamped.

- 2026-09-23T16:23:01Z (issue #335): `implementer.md`, `reviewer.md`,
  `contracts.md`, and `evidence-and-probes.md` now state, in the same
  wording at the implementer/reviewer sites, that a verification set named
  by reference plus its frozen digest and repository identity is the
  orchestrator's approval of every argv resolved from that frozen snapshot,
  with a digest mismatch withdrawing the approval and the safety intent
  preserved for anything outside the frozen snapshot. `CHANGELOG.md` gained
  an `[Unreleased]` entry for it. Citations into these four files and into
  `CHANGELOG.md` were re-pointed across `review-gate-and-waivers.md`,
...
- 2026-09-23T11:03:24Z (T-006): added the bounded `single` escalation
  ...
  findings, are recorded in the run evidence.
...
- 2026-09-22T07:45:00.000Z (agent-dx tracker task
  ...
  halt rule's cross-reference to `introduced_by_delta: yes`/`unknown`
  through one shared test constant (`HALT_SCOPE_PHRASE`); extended the
  subagent misfire rule to name a missing `class_closure` field on any
  round after the task's first; added `closed: true | false` to the
  `class_closure` output-contract field in both copies, with an unclosed
  site named in `risks`; dropped `not_applicable` from the 04 template's
  Class Closure row enum, since a row exists only for a round that fixed
  a finding; gave the reviewer prompt the run-mode-`single` comparison
  target for its independent class-enumeration search and an ordinary
  severity floor for an omitted site. Two new tests pin `class_closure`'s
  exact sub-field names and fixed order in both copies (mirroring the
  existing `mutation_probes` pins), and `extractClassClosureBlock` was
  lifted to module level beside `extractMutationProbesBlock`. This
  bundle re-verify re-stamped `subagent-contracts-superset.md`
  (new "Class closure field" section), `review-gate-and-waivers.md` (a
  paragraph on the scoped halt), `model-preselection.md` and
  `run-state-lifecycle-and-markers.md` (citation re-points only), and
  fixed a pre-existing citation-geometry drift in the
  citation-sibling-drift guard's own allowlist in
  `test/docs-consistency.test.ts` that unrelated line-number movement had
  broken (a model-preselection.md pair of citations that had collapsed
  onto the same target line, one review-gate-and-waivers.md entry whose
  own doc position had shifted, and the subagent-contracts-superset.md
  entries whose target lines moved). Six entries added here, including
  the two for the guard's own `cursor = idx;` and
  `not implementation instructions` hits, were later removed again: those
  hits were not pre-existing gaps, they were this task's own citation
  collapses, and an entry certifying one is the failure this array's own
  process comment names. Nine mutation probes
  (`agent-primitives probe -p`, `--pre 'npm run build'`, whole-file
  `npx vitest run test/docs-consistency.test.ts` or
  `test/template-markers.test.ts`): the `introduced_by_delta` scope
  removed from step 8's halt sentence, killed by the scope-agreement
  test; the same scope removed from the Round-2 halt rule's
  cross-reference instead, killed; a `class_closure` sub-field renamed
  identically in both copies (`sites` to `hits`), killed by the exact
  sub-field-name pin; the `class_closure`-omission clause removed from
  the misfire rule, killed; `closed` removed from both copies, killed;
  replayed from the earlier pass: the `class_closure` block removed from
  the implementer contract reference copy, killed; the halt-at-first-
  repeated sentence removed from step 8, killed; the Class Closure row
  removed from the 04 template, killed; the reviewer's independent
  class-enumeration and class-match sentences removed, killed.
  PKG-VERIFY(orchestrator-workflow) green (1313/1313).
  A later pass in the same task corrected five citations that had been
  re-pointed by searching an anchor's text and taking an occurrence,
  which is how each of them landed on a sibling line carrying the same
  text: the second and third `expectPointerMention(section)` mentions in
  the entry below, which had collapsed back onto the first of the three
  `it` blocks; `subagent-contracts-superset.md`'s scope-boundary pin, now
  naming the assertion rather than the `it` title a neighbouring sentence
  already cites; its slicer-output pin, now naming the test that proves no
  subagent-input field is absent from the slicer schema rather than the
  envelope-fields test; and its `02-tasks.md` section-order pin, now
  naming that test's own cursor-advance statement rather than the
  field-order test's. Each was derived from the site its own sentence
  names, then checked against the enclosing `it` title at the base
  revision. The same pass extended the shared halt-scope constant to the
  signal's other clause (a class a previous round's fix already
  addressed), so step 8 and the Round-2 halt rule's cross-reference state
  both clauses from that one constant; stated once, in the implementer
  prompt, what `class_closure` looks like where a task has no finding to
  fix yet; pinned the 04 template's Class Closure and Mutation Probes
  marker rows against their own header columns and routed an unclosed
  site of a row's class to Risks / Notes, which the reviewer's run-mode
  `single` sentence now names beside the row; and closed the collapse
...
- 2026-09-19T11:22:53.000Z (agent-dx tracker task 0665044e, pandora run
  ...
  build metadata in release headings, so `1.0.0+build-1` matches the
  package version and a mismatched tag reports the version mismatch.
  Underscores remain invalid under the deliberate SemVer identifier
  decision, pinned by a fixture. The CHANGELOG references in this bundle
  include two live line-and-anchor citations; both targets were mechanically
  re-pointed after the new Unreleased bullet. Re-verified the three docs
  whose `sources` list CHANGELOG.md against source commit
  b2a3cd79291ccb66b105726a741b89beb0d4cdaa and re-stamped them after that
  commit.
...
- 2026-09-19T04:59:36.000Z (agent-dx tracker task 5b66d626-4220-44df-b52d-
  ...
  the `mode` marker in `00-goal.md` and on the reference's new final "Run
  mode" section, with anchored citations into the template, the reference,
  the `04-implementation-summary.md` template and the two new pins at the
  end of `test/template-markers.test.ts`; its grep gotcha gained one
  sentence appended to an existing line. Every source edit was placed so
  that no cited line moved: the marker and its comment sit below the two
  cited run-base lines of the template, the reference section and the
  template section are appended after the last cited line of their files,
  the two pointers in the reference's Intent and in `SKILL.md` extend
  existing lines, and the new pins follow the last cited line of the test
  file. The one exception is the package CHANGELOG, whose new Unreleased
  bullet pushed everything below the Unreleased heading down by seventeen
  lines; this log's own relative citations into it (two anchors, repeated
  across earlier entries) were re-pointed mechanically by that offset and
...
- 2026-09-18T04:42:03.000Z (agent-dx tracker task 3c7bf237-f587-40bb-b1dc-
  ...
  parses an allowlist entry's `claim` as citation-shaped tokens (`line
  N`, `lines N and M`, a bare number after `uncited`, and a bare
  `:N`/`:N-M` not preceded by a path character) instead of a
  digit-substring search, fails closed on a blank enumeration, checks
  every `line`/`uncited` token against the entry's own recorded lines,
  and checks every `:N`/`:N-M` token against the entry's own range, its
  own recorded lines (a bare single-number token only), or a real
  sibling citation the doc's own citation scan finds into `entry.real`;
  this pure re-stamp-free growth added a new helper block, a new
  fixture test, and a docCitations-typed variable well after every
  existing citation this bundle makes into that test file, pushing
  every citation at or after line 7638 (pre-edit) down by 193 lines.
  Re-read all ten of `log.md`'s own citations into that test file
  against the edited file (the only bundle doc citing that far down;
  ...
  A follow-up implementer pass on review findings reworded the two comments that still
  described the pre-sibling-citation own-lines-only rule, renamed the
  sanity test title to match, and extended the fixture test with three
  near-miss identity checks (right file wrong line, right start wrong
  end, right range wrong file), a claim resolving purely on a real
  sibling citation with no own-line token, and the previously-unexercised
  `lines N and M` / `uncited N and M` two-number branches (wrong-second
  ...
  commit.
  A fourth pass closed the two holes a reviewer had found in that table by
  hand (no row carried a WRONG start with the entry's own end, so the
  `start` half of the own-range identity survived the whole file, and no
  ...
  row, a conjunct naming a row that does not exist, and a row no conjunct
  claims all fail. New rows pin the length floor as a boundary pair (40
  characters rejected, 41 accepted), the mirrored own-range start, a bare
  `:N` resolving through the own-line fallback with no doc at all, and the
  `secondCitationLine` dimension the shared fixture entry had never set;
  the two cases the previous pass kept as standalone assertions are rows
  now, so the table is the whole coverage statement, and the row loop
  reports every mismatching row instead of throwing on the first, so a
  mutation probe's failure output names every row its mutant flips. The
  one boolean site deliberately left out is the `docCitations ===
  undefined` memoization guard, whose neutralised form is an equivalent
  mutant. This pushed every citation at or after line 7305 (pre-edit) down
  by 403 lines, and all ten of `log.md`'s own citations into that test
  file were re-read against the edited file and re-stamped again in the
  same commit.
...
- 2026-09-16T05:18:16Z (docs cleanup, agent-dx tracker task 8a55e082, review
  ...
  entry in `test/docs-consistency.test.ts` for the doc's line-1082
  paragraph moved `paragraphLine`/`secondCitationLine` from 441/445 to
  440/444, and the entry for its line-1238 paragraph moved from 645/651
  to 638/646 (the new
...
- 2026-09-13T05:42:55Z (reviewer delta attribution, agent-dx task
  ...
  named base build plus replay and remains in the ordinary review gate; only
  `yes` and `unknown` count toward bounded halt/escalation guidance. The
  legacy five-cell table and placeholder remain byte-compatible with the
  completeness reader; concrete finding rows record attribution
  parenthetically in their Description field. Fixture coverage pins the full
  legacy table schema and placeholder row. Re-verified the affected bundle
  documents, re-pointed their shifted source citations, and re-stamped their
  timestamps.
...
- 2026-09-12T10:21:32Z (implementer report derivation and foreground returns,
  ...
  killed, and restored. Repointed citations and re-stamped affected documents.

- 2026-09-12T10:08:46Z (result enum and forward-pointer guard, agent-dx task
  318a70ad, review round 2): replaced the fabricated Set-membership check with
  the real forward-pointer validator. A fixture now contains `see Nonexistent
  thing (9.9.9) below` without its heading and asserts the validator's exact
  missing-heading diagnostic; the live parenthesized heading remains covered.
  Replayed the parenthesis, both contract-enum, and validator-bypass probes;
  each was killed and restored.

- 2026-09-12T09:54:49Z (result enum and forward-pointer guard, agent-dx task
  318a70ad): changed the implementer output-contract `result` placeholder in
  both source copies to `killed | survived | not_applicable`. The exact-field
  guard now independently rejects a missing enum value in either copy, in
  addition to its cross-copy equality check. The bundle-doc forward-pointer
  guard accepts the live `Mutation probes requirement (0.16.0)` heading and
  rejects `Nonexistent thing (9.9.9)`. Re-wrapped the orphaned SKILL.md line,
  re-stamped the affected bundle documents, and re-pointed their citations after
  the test shift. Verification is recorded with this task's commit.

- 2026-09-12T07:30:00Z (mutation probe definition fields, agent-dx task
  06330af2, review round 2 fixes): review round 2 found five gaps. (1)
  MEDIUM docs: `subagent-contracts-superset.md` carried four "see
  Mutation probe definition fields and expectation split below" forward
  pointers to a section that did not exist. Added that heading (exactly
  that title) covering the five sub-fields task 06330af2 grew the
  contract by (`file`, `anchor`, `before`, `after`, `expectation`), the
  `met`/`violated`/`not_applicable` split scoped to a measured result,
  the new `reason` rule (see 2 below), the by-definition replay and
  reviewer-skip rules (cross-referenced to the Fix-round mutation probe
  replay section rather than restated), and the pins that hold them,
  each claim anchored into `SKILL.md`, `implementer.md`, the template,
  and the tests. Extended the bundle-doc-prose pin
  (`test/docs-consistency.test.ts`) with a generic scan: every "see
  <Name> below" reference in the file must name an existing heading (the
  pre-existing "see Motivation below" convention, a same-subsection
  paragraph lead-in rather than a heading, is excluded by name). (2)
  MEDIUM correctness: the two `not_applicable` verdicts (no reapplicable
  definition versus target text no longer present) were distinguished
  only by prose, with no field a misfire check could look for. Added an
  eleventh `mutation_probes` sub-field, `reason` (both copies): free
  text, required exactly when `result` is `not_applicable`, empty
  otherwise, carrying one of the two existing canonical strings. Updated
  the fixed-order pin (ten sub-fields to eleven), the exact-sub-field-name
  pin, the field-enumeration pin, the acceptance-baseline producer-shape
  key list, and the `04-implementation-summary.md` template's Mutation
  Probes table (added a Reason column between Expectation and Restored
  Verified, the same position as the yaml field); the existing
  columns-vs-contract agreement test derives both lists and passed
  unmodified once both sides carried the new field/column. (3) MEDIUM
  docs: the CHANGELOG's `[Unreleased]` `mutation_probes` bullet still
  described `expectation: met | violated` with no `not_applicable` case
  and no `reason` field. Updated the bullet's prose and added a
  fourth-copy pin (the same anchor-and-slice pattern already used for the
  replay-rule bullet) asserting the three-value enum, the measured-result
  scoping, the `reason` field, and its two canonical strings. (4) LOW
  tests: SKILL.md step 6's single-line-excerpt sentence had no
  cross-copy pin; added one checking the same key phrases (single-line
  excerpt, multi-line, unescaped `|`, patch/diff, fenced block) appear in
  both `SKILL.md` and the template. (5) LOW maintainability: the round-1
  commits had added 7 em dashes to this file's prose; replaced with
  commas, colons, and parentheses in the lines this branch added.

  Doc-placement decision (finding 6): the ten partial
  `assets/skill/SKILL.md:N#` citations this branch (both rounds) had
  added to this file were unresolved-ambiguous under `okf-kit check`
  (repo-wide basename search finds three same-named `SKILL.md` files, one
  per package). Converted all ten to the full repo-root-relative path
  form (`packages/orchestrator-workflow/assets/skill/SKILL.md:N#"..."`),
  which resolves uniquely in `okf-kit check`. This did surface warnings
  in older entries, as predicted: an ambiguous citation is "not
  evaluated" at all, so bare continuations and same-suffix bare
  ...
  self-reference to that doc's own `[0.18.0]` CHANGELOG citation had
  drifted with this round's new section insertion and was re-pointed to
  ...
  direct consequence of this round's one-line insertion into
  `test/acceptance-baseline.test.ts`'s probe-keys array) moved the
  "original contract, keep the original checklist semantics" citation,
  formerly numbered 386-392 (cited from both `log.md` and
  `subagent-contracts-superset.md`), onto a bare `` );` `` line;
  re-pointed to `387-393`, the same two-`expect()`-call span one line
  later. The notice count for the whole `docs/okf` bundle went from 30 to 11 (0
  errors, 0 warnings throughout, under both `check --json` and `check
  --json --require-anchors`); the remaining 11 are all pre-existing bare
  `init.ts:N`/`cli.ts:N` forms and three further bare `SKILL.md:N` forms
  this round did not touch, out of this round's scope.

  Re-pointing mechanics: the `reason` field additions to `SKILL.md` and
  `implementer.md`, the new bundle-doc section, the four new tests, and
  the CHANGELOG bullet rewrite shifted line numbers throughout
  `test/docs-consistency.test.ts`, `SKILL.md`, `implementer.md`,
  `CHANGELOG.md`, `subagent-contracts-superset.md` itself, and (by one
  line, from the acceptance-baseline producer-shape key list edit)
  `test/acceptance-baseline.test.ts`. Every citation into those six files
  across the whole `docs/okf` bundle (`log.md`,
  `subagent-contracts-superset.md`, `model-preselection.md`,
  `review-gate-and-waivers.md`, `run-state-lifecycle-and-markers.md`) was
  re-pointed by anchor-text search against the post-edit files, including
  two multi-candidate
  groups resolved by context rather than nearest-line guessing: the three
  per-harness "pointer rule applies unchanged" citations in
  `run-state-lifecycle-and-markers.md` (Claude Code, opencode, OpenAI
  Codex bullets, each ending on a distinct line) and the "Commits field"
  section's own sibling-citation group in `subagent-contracts-superset.md`.
  That second group's opening/closing citation (`expect(implementerMd)
  .toContain(clause);`) moved from :1194 to :1209; the matching
  `docs-consistency.test.ts` sibling-guard allowlist entry (anchorKey
  `b19680bb`) was updated in place (start/end, paragraphLine,
  secondCitationLine, and the claim text) rather than left to read as a
  dead exemption. One re-point slip (a doubled `packages/
  orchestrator-workflow/` path prefix introduced while converting one of
  the ten bare citations above) was caught by the log.md citation-guard
  test and corrected before commit.

  Verified: the full package suite (`npm test`, 840/840, up from 836 by
  the four new tests above), `typecheck`, `typecheck:test`, and
  `format:check`, all clean on the committed tree. `okf-kit check --json`
  and `okf-kit check --json --require-anchors` against `docs/okf`: 0
  errors, 0 warnings (11 notices, all pre-existing and out of scope, see
  above).

- 2026-09-12T06:30:04Z (mutation probe definition fields, agent-dx task
  06330af2, review round 1 fixes): review round 1 found the prior round's
  re-stamp and re-point pass had moved citations without updating the
  prose they anchor, plus three correctness/maintainability gaps in the
  contract itself. Fixed in `assets/agents/implementer.md` and
  `assets/skill/SKILL.md` (mirrored copies): `expectation` is `met |
  ...
  (`implementer.md:286#"expectation: met | violated | not_applicable"`,
  ...
  carry a distinguishing reason string, `no definition recorded` and
  `target text no longer present` respectively, in all four places the
  ...
  prose paragraph)
  (`implementer.md:121#"no definition recorded"`,
  `packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:109#"no definition recorded"`). The prompt's
  ...
  (`implementer.md:101#"records whether"`,
  `packages/orchestrator-workflow/assets/agents/implementer.md:101#"records whether"`). The verify/mutation-
  ...
  (`implementer.md:163#"take the definition fields from that"`).
  ...
  (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:135#"Each row's Before/After"`).
  ...
  `--expect`-absence entry in the existing tool-agnostic-guidance test,
  and a template-markers.test.ts pin on the single-line-excerpt note; the
  YAML sub-field-block pin's expected string now reads `expectation: met
  | violated | not_applicable`. The reviewer's missing pin (that the
  `04-implementation-summary.md` template's Mutation Probes columns and
  the `mutation_probes` contract's sub-field list cannot drift apart
  independently) is now a dedicated test deriving both lists
  programmatically (the template's header row minus `Round`, the YAML
  block's sub-field names minus `mutation_probes` itself) and asserting
  they agree.

  `docs/okf/subagent-contracts-superset.md`'s "Mutation probes
  requirement (0.16.0)" section gained a one-line forward pointer where
  it enumerated the original four sub-fields, since that enumeration is
  no longer the current shape. Its "Fix-round mutation probe replay"
  section had two sentences that were flatly wrong today, not just
  incomplete, because round 1's re-point pass had already moved their
  own citations onto the current (post-this-round) wording without
  updating the sentences describing them: "a replayed probe whose mutant
  now survives or can no longer be applied is a regression signal: both
  copies say so" (false for the survives half, once `expectation` can
  legitimately be `met` alongside `result: survived`), and step 7 naming
  the replayed-and-killed probes "together with their `mutant` and
  `verified_applied_via` values" (drops the by-mutant-definition rule
  step 7 actually states). Both are now framed as "at this point" /
  historical, with a pointer to this round's task id for the current
  wording, rather than left describing a superseded rule in the present
  tense. The pin citing that section's "further pin locks the five
  `mutation_probes` sub-fields" now says ten, with the same historical
  framing. Re-anchoring the citations alone was not enough evidence this
  would not recur: `test/docs-consistency.test.ts` gained a pin reading
  the bundle doc's own raw prose, asserting it states the ten-sub-field
  count (not five) and the current `expectation`-keyed regression-signal
  wording (not the pre-`expectation` "both copies say so" wording), so a
  future re-point-only pass that moves the citations but not the prose
  fails this test instead of shipping silently wrong again.

  Collateral, undocumented in the prior round: two older entries below
  this one ("review-method run, round-2 review lows closed by the
  orchestrator" and "sibling-guard allowlist claim off-by-one fixed")
  had a leading colon stripped from four and twelve historical
  line-number mentions respectively (sixteen total), digits and meaning
  unchanged. Measured reason, now recorded there: a bare colon-digit
  group in backticks parses as an okf-kit continuation citation
  inheriting the nearest preceding citation's path, so left colon-
  prefixed those sixteen mentions resolved, wrongly, against
  `implementer.md` past its own end; `okf-kit check` reproduces exactly
  16 `range-exceeds-file` warnings with the colons restored and zero
  without them.

  Verified: the full package suite (`npm test`, 836/836), `typecheck`,
  `typecheck:test`, and `format:check`, all clean on the committed tree.
  Re-pointed every citation this round's `SKILL.md`/`implementer.md`
  edits shifted across `subagent-contracts-superset.md`,
  `run-state-lifecycle-and-markers.md`, `review-gate-and-waivers.md`,
  `model-preselection.md`, and this file (including four bare/ambiguous
  `SKILL.md:<n>#"..."` citations this file's own stricter log-citation
  test resolves unconditionally to this package, unlike okf-kit's
  ambiguous-and-skipped treatment of the same bare form); a handful
  landing on text the diff itself changed were re-anchored by hand.
  Fixed two citation-sibling-drift regressions the new tests introduced
  by reusing a generic `const clause` name already tracked as a
  duplicate-citation pair elsewhere in this file's own allowlist: renamed
  the new local to `expectationScopeClause`, and re-pointed the
  `subagent-contracts-superset.md` allowlist entries (`03317257`,
...
- 2026-09-12T05:32:21Z (mutation probe definition fields, agent-dx task
  ...
  free-text `mutant` label; a pandora dogfood round could not replay a
  prior round's probes because no reapplicable definition existed.
  `assets/agents/implementer.md` and `assets/skill/SKILL.md` (mirrored
  copies) each gained four new sub-fields, `file`, `anchor`, `before`,
  `after`
  (`implementer.md:95#"anchor, before, after, verified_applied_via, result, expectation,"`,
  `packages/orchestrator-workflow/assets/agents/implementer.md:95#"anchor, before, after, verified_applied_via, result, expectation,"`),
  ...
  actual observed outcome (killed/survived) independently of `--expect`,
  so a routine negative-control probe now legitimately returns `result:
  survived`. Added `expectation: met | violated` beside `result`
  and qualified the replay regression-signal rule everywhere it appears
  (SKILL.md step 6, SKILL.md's output-contract prose paragraph, and
  ...
  alone is not
  (`implementer.md:101#"records whether"`,
  `packages/orchestrator-workflow/assets/agents/implementer.md:99#"are the exact text swapped there"`). The
  ...
  (`CHANGELOG.md:718#"The implementer"`).
  ...
  every citation this change shifted in `subagent-contracts-superset.md`
  and `run-state-lifecycle-and-markers.md` (pure offset from the diff of
  `SKILL.md`/`implementer.md`/`test/docs-consistency.test.ts`) and in this
  ...
  rather than only re-numbered (`packages/orchestrator-workflow/assets/agents/implementer.md:112#"rather than omitting the field."`,
  `packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:112#"A replayed probe whose"`,
  `implementer.md:124#"A replayed probe whose"`). Two pre-existing
  ...
  string-anchored citations.

  Also dropped a leading colon from four historical line-number mentions
  in the "review-method run, round-2 review lows closed by the
  orchestrator" entry below, and from twelve more (two lists of six) in
  the "sibling-guard allowlist claim off-by-one fixed" entry further
  below; both entries named these bare digit groups in parentheses,
  describing lines a still-earlier round had touched. Neither edit
  renumbers or reworks those entries: the digits and their meaning are
  unchanged, only the colon each one used to start with is gone. Measured
  reason: with the colon left in place, okf-kit reads a bare colon-digit
  group as a continuation citation inheriting the nearest preceding
  citation's path in the same paragraph, so it resolved each of these
  sixteen, wrongly, against `implementer.md` and reported it past the end
  of that 164-line file. Restoring all sixteen colons and running
  `okf-kit check --json packages/orchestrator-workflow/docs/okf`
  reproduces exactly 16 range-exceeds-file warnings; removing them again
  returns the suite to clean, confirming the colon, not the digits, was
  the trigger.
...
- 2026-09-09T05:19:57Z (five review-round-2 lows on the shared
  ...
  (1) the fallback containment conjunct's trailing `` `${sep}` ``
  survived dropping it; the `..`-segment fixture gained a sibling-prefix
  assertion (`${repoRoot}-other/README.md`, an existing path sharing
  `repoRoot` as a string prefix without being inside it) that only fails
  with the suffix present. (2) the log guard's own
  `.filter((m) => !PATH_SHAPED_BEFORE_RE.test(...))` had no fixture of
  its own (only the sibling guard's use was pinned); added a
  `.toml`-style fixture whose own `:N-M#"..."` tail must not be reported
  as a forbidden continuation. (3) reworded the comment naming the
  removed `PATH_SHAPED_BEFORE_MATCH_RE` symbol as history, without the
  dead identifier. (5) the `/etc/hosts` fixture now asserts the file
  exists before asserting it does not resolve, so an absent file fails
  loudly instead of passing for the wrong reason. Re-pointing the five
  new lines added ahead of the `node:fs` import shifted every citation
  into `test/docs-consistency.test.ts` below it across four bundle docs
  (`review-gate-and-waivers.md`, `model-preselection.md`,
  `subagent-contracts-superset.md`, this file's own two non-anchored
...
- 2026-09-08T11:05:00Z (citation scanning goes paragraph-joined, the
  ...
  `CHANGELOG.md:1095#"Citation scanning is paragraph-joined"`. Verified on
...
- 2026-09-07T13:36:36Z (citation-sibling-drift guard, review round 4, task
  ...
  trusting the prior round's verdicts. None of the 14 entries certified
  real drift, but the pass found two inaccurate claims and one further
  citation mis-pairing invisible to the guard itself.
  (M1) Rule (a)'s match compared only `finding.citationLines[1]` against
  `entry.secondCitationLine`; a `return true;` mutant of that one line
  survives, and a two-citation entry's cleared repeat was indistinguishable
  from a finding whose group grew a THIRD, unreviewed repeat sharing the
  same second line. The match now also requires
  `finding.citationLines.length === 2`
  (`packages/orchestrator-workflow/test/docs-consistency.test.ts`, the
  `siblingGuardFindingMatchesAllowlist` function), pinned by a new
  bundle-independent fixture covering both the differing-second-line and
  the three-repeat cases. The geometry re-check's duplicate-citation
  branch had the same blind spot from the other side: it only asked
  whether SOME citation exists at the recorded line, true of any citation
  the extractor produces by construction. Replaced with a check that sorts
  the group's own citations into document order and requires the recorded
  `paragraphLine`/`secondCitationLine` to sit at positions 0 and 1; a
  mutant reducing the old guard to `if (false)` survived until this
  fixture existed.
  (L2) The match and the geometry re-check both gained `paragraphLine`,
  the doc line of the finding's own first citation. Before this, an entry
  was keyed by (doc, kind, real target, range, anchorKey, recorded
  ...
  different evidence
  (`packages/orchestrator-workflow/src/init.ts:1153-1155#"composeOpencodeAgent(role, modelValue, defaultEffortLine),"`,
  ...
  (`packages/orchestrator-workflow/src/init.ts:675#"reasoningEffort: ${TIER_DEFS[tier].effort}"`);
  ...
  only non-quote-crossing substring distinguishing it from its sibling
  line 1826 is the bare word `reasoningEffort`, which recurs 15 times in
  `init.test.ts`, over this file's own 3-occurrence-file-wide cap; the
  claim was corrected instead of the citation.
  ...
  now cites
  `packages/orchestrator-workflow/src/init.ts:850-859#"variantPath}: now untracked after the full"`
  ...
  `packages/orchestrator-workflow/src/init.ts:839-841#"${relativePath}: now untracked after the full -> ${profile} profile downgrade; run"`.
  ...
  in the paragraph, no anchor text recurring nearby. M4's mis-pairing was
  exactly this shape: both citations passed every existing check,
  including this guard, because nothing about either one, read alone or
  against its paragraph's siblings, looked wrong; only re-reading the
  target file against the sentence's own claim surfaced it.
  Verification. `npm test`, `npm run typecheck` and `npm run
  typecheck:test` are green, including this round's new fixtures; `npm
  run format:check` is clean on `test/docs-consistency.test.ts` (the two
  pre-existing red files, `decision-authority.test.ts` and
  `template-markers.test.ts`, are untouched by this round). `npx
  okf-kit@0.10.0 check --json --require-anchors docs/okf` measured before
  this round's edits: 0 errors, 0 warnings, 17 notices; measured again
...
- 2026-09-07T12:57:17Z (citation-sibling-drift guard, review round 3, task
  ...
  changed the mechanism rather than only the entries. Structural cause
  named by D-017: whoever implemented or re-pointed a citation also
  classified its own guard's hits, and an entry's justification was free
  prose no test could check, so a wrong "coincidence" verdict shipped with
  the suite green. Three changes.
  (R1) An entry now records the geometry it was cleared against and that
  ...
  to a cleared one, or a repeat that moves to another doc line, fails the
  per-doc check instead of inheriting the old verdict. A separate test
  re-reads those recorded lines against the current files without going
  through the guard at all (the anchor text is re-derived from the doc's
  own citation, since the array stores a hash rather than the literal
  text, for the file-wide-occurrence reason recorded in round 2), so an
  entry whose situation no longer exists goes red even if the guard itself
  were broken and produced no findings. Two bundle-independent fixtures
  pin both halves.
  (R2) `reason` is replaced by `claim`: one sentence naming what the
  citing sentence describes and why the cited line, not the uncited
  sibling, is that sentence's evidence, written so a reviewer can falsify
  it by reading exactly the two lines the entry names. The process the
  entries depend on is recorded here rather than in the entries: an
  allowlist entry is accepted only on an independent review
  classification, never on the reading of whoever wrote or re-pointed the
  citation. `docs/okf/index.md`'s Maintenance section and the CHANGELOG
  `[Unreleased]` entry both state that rule; neither carries hit counts
  any more (round-2 finding F2 -- the CHANGELOG still named 7 and 11,
  which had been stale since round 2 re-measured them).
  (R3) Re-pointed the pair: `install-fence-mechanics.md` and
  ...
  its citing paragraph: rule (a) reports 5 hits, rule (b) reports 10 at
  window 20 and 8 at window 10. Allowlisted as 14 entries (5
  duplicate-citation, 9 wrong-sibling-anchor; one wrong-sibling entry
  matches two raw hits that differ only in the citing doc's own spelling
  of the path), down from round 2's 16.
  ...
  local anchor guard caps an anchor at 3 file-wide occurrences, so no
  range ending there can carry a compliant anchor. The widened range
  covers the marker-mention survival assertion the old one truncated; the
  no-conflict half stays covered by the same bullet's `writers.ts`
  ...
  supports; widened to
  `packages/orchestrator-workflow/assets/skill/references/run-state-and-harness.md:82#"the files as the run progresses. The newest run"`.
  ...
  alongside it,
  `packages/orchestrator-workflow/test/acceptance-baseline.test.ts:272-278#"else assertCriteriaShape(block);"`
  and
  `packages/orchestrator-workflow/test/acceptance-baseline.test.ts:404-409#"original contract, keep the original checklist semantics"`.
  ...
  occurrence 15 lines outside the cited range with a genuine sibling range
  also cited, and asserts rule (b) fires at the guard's window and stays
  silent at the round-1 value of 10. The fence-skipping pass toggled on
  any delimiter line with no balance check, so one stray fence would have
  marked the rest of a doc as fenced, dropped every citation after it and
  left that doc reporting a clean zero; it now throws, with a fixture, and
  a companion assertion requires every bundle doc to yield at least one
  citation.
  Log hygiene. Replaced this doc's own round-2 suite total with a delta
  verdict (round-2 finding F3; the Maintenance convention above forbids
  totals). Rewrote the round-2 entry's 14 bare-basename citations
  (`init.ts`, `init.test.ts`, `SKILL.md`) as repo-relative paths (F6).
  Side effect worth recording: once a doc pins one repo-relative path, the
  checker also resolves that doc's other multi-segment citations of the
  same suffix, which surfaced an older entry's historical inventory of how
  some citations read at that time -- it was being validated against
  today's file and reported a stale start line. Rewrote that inventory as
  prose, the same remedy and the same reason as the historical CHANGELOG
  mention recorded in the round-2 entry above. The CHANGELOG citation this
  doc carries was re-pointed by the round-3 `[Unreleased]` bullet's own
  length, and two self-citations into `subagent-contracts-superset.md`
  were re-pointed by the two lines the slicer-superset re-point added.
  Verification. `npm test`, `npm run typecheck` and
  `npm run typecheck:test` are green, including the round's new fixtures;
  `npm run format:check` is clean on `test/docs-consistency.test.ts` (the
  two pre-existing red files, `decision-authority.test.ts` and
  `template-markers.test.ts`, are untouched by this round).
  `npx okf-kit@0.10.0 check --json --require-anchors docs/okf` measured
  before this round's edits: 0 errors, 0 warnings, 38 notices; measured
  after the round's commits: 0 errors, 0 warnings, 17 notices. The drop
  below the pre-round baseline of 24 is the F6 re-points plus the
  resolution side effect above; all 17 remaining notices are this doc's
...
- 2026-09-07T11:57:20Z (citation-sibling-drift guard, review round 2, task
  ...
  fenced blocks, so this is a no-op today). `npm test` (green, including
  this round's four new pins), `npm run typecheck`, and
  `npm run typecheck:test` are all green;
  ...
  re-measured: 0 errors, 0 warnings, 38 notices. The 14 new notices were
  ...
  introduced, the same notice class this doc already carried before the
  round, not a new one; review round 3 re-pointed them to repo-relative
  paths (see the round-3 entry above) and the count returned to the
  pre-round baseline.
...
- 2026-09-06T21:33:40Z (task 0e17fb63, review round 2 fix round, two
  ...
  `packages/orchestrator-workflow/assets/skill/references/run-state-and-harness.md:211#"pointer rule from Run state applies unchanged."`.
...
- 2026-09-06T21:10:29Z (task 0e17fb63, review round 1 fix round, six
  ...
  `test/docs-consistency.test.ts:513#"expectPointerMention(section)"`
  twice instead of walking on to the second and third `it` block; restored
  the second and third mentions to
  `test/docs-consistency.test.ts:523#"expectPointerMention(section)"`
  and
  `test/docs-consistency.test.ts:533#"expectPointerMention(section)"`
...
- 2026-08-18: re-verified and re-stamped subagent-contracts-superset.md and
  ...
  (`test/docs-consistency.test.ts:1037#"expect(skillBlock).toBe(reviewerBlock);"`); the doc now states the true
...
- 2026-08-18: re-verified and re-stamped review-gate-and-waivers.md,
  ...
  SKILL.md line 79 "grounding-mcp 0.6.0 docs" pointer (actually line 82) had
  ...
  the ones this diff broke. `subagent-contracts-superset.md` also
...
- 2026-08-19: re-verified and re-stamped install-fence-mechanics.md and
  ...
  diff hunk header), so every citation before that line in both docs (the
  ranges then reading 100-106, 275-306, 464-509, 511-534, 721-758 and
  921-941) needed no change, while the two citations past it (then reading
  995-1007 and 1009-1016, the opencode-absent-binary tests) shifted by
  ...
  (Review round 3 of task agent-dx 9f72ae6d rewrote these eight ranges from
  citation syntax into prose: they are a historical inventory of how the
  docs' citations read at that time, not live evidence pointers, and once
  round 3's repo-relative re-points let the checker resolve this doc's
  `test/init.test.ts` chain, they were being validated against today's file
  instead. Same remedy, same reason as the historical `CHANGELOG.md`
  mention two entries above.)
...
- 2026-08-20 (review round 1, 7 findings: H1, M1-M4, L1-L3): re-verified and
  ...
  actually changed (M1's label derivation, M2's tripwire/pin additions). The
  underlying mechanics they document are untouched, so a mechanical
...
- 2026-08-24: fix-round-3 review found 0 high, 3 medium, 7 low findings
  ...
  also collapsed two long single-argument `.indexOf(...)` calls
...
section); subagent-contracts-superset.md:329's `[0.18.0]` citation now
...
- 2026-09-11 (review-method axis round 2, pandora task 226c532c): closed
  R1's two High and two Medium findings plus its two Lows. `reviewer.md`'s
  `normal`/`rigorous` obligation-table rows now state an explicit floor
  (normal adds nothing beyond, and suspends none of, the Check list and
  Rules below it) and fix the "already required above" direction to
  "below" in both rows. SKILL.md's own completeness-gate claim (no
  enforcing reader existed) is reworded to the hand-check the orchestrator
  performs until a grounding-mcp reader parses the marker, matched in the
  CHANGELOG's own description of the field; the cross-repo follow-up id
  stays out of `SKILL.md` (an instruction file placement-guard covers) and
  lives only in the CHANGELOG and this bundle. The CHANGELOG's evidence
  pointer is rewritten to the corrected sweep counts (AC-005 r2 -> r3,
  `03-decisions.md` D-003): R1, R5, R11, R13 and R14 found the
  Critical/Highs by probing; R4, R6 and R9 returned nothing on the default
  method and R8 only two informational lows; R5 withdrew. Added a "Review-
  method axis: method_applied and withdrawn" section to
  `subagent-contracts-superset.md` following the Recurrence-field pattern,
  ...
  contract copies and the CHANGELOG's own description of it.

  Both edited assets (`reviewer.md`, `SKILL.md`) are pure-insertion diffs
  at different offsets (reviewer.md: +1 line after old line 41; SKILL.md:
  +3 lines after old line 552, both before the new anchored citations were
  authored, then a placement-guard fix reworded SKILL.md's marker sentence
  without changing its line count); every citation into them from this
  bundle and `CHANGELOG.md`'s own two literal `CHANGELOG.md` line
  citations was re-derived by line-diff mapping and verified to resolve.
  `test/docs-consistency.test.ts` gained a new describe block (a pure
  insertion after old line 1567, 54 lines once formatted by `prettier
  --write`, not the 56 first drafted before formatting collapsed two
  multi-line `it()` calls); every downstream citation into that file
  across `subagent-contracts-superset.md`, `model-preselection.md`,
  `review-gate-and-waivers.md`, and this file's own citations was
  re-derived against the actual post-format line numbers, not assumed
  from the pre-format insertion size. Two of this file's own literal
  `CHANGELOG.md:<n>` mentions inside historical, already-closed accounts
  (the misstating-the-past account and the round-1-blind-find-and-replace
  correction account) and two more marked "deliberately historical --
  frozen to base commit" were left untouched even though a first blind
  pass moved them; this is exactly the mistake those accounts themselves
  narrate, and reverting it here does not undo their own closure.

  A first draft of the SKILL.md reword named the cross-repo follow-up id
  inline; `node packages/slop-detector/dist/cli.js check . --pack
  placement-slop --config slop.config.yml` from the repo root caught it
  as an opaque id in an instruction file (`assets/**/*.md` is covered;
  `docs/okf/**` and `CHANGELOG.md` are not), fixed by moving the id out of
  `SKILL.md` and into the CHANGELOG/bundle only; re-run clean (625 files
  scanned, 0 violations). `okf-kit check --json --require-anchors
  docs/okf` (the CI-pinned 0.10.0, exact `ci.yml` flag set) measured 0
  errors, 0 warnings, 17 notices both on the round-1 base commit and after
  this round's edits, with an identical finding set (bare-filename
...
- 2026-09-12 (task 0085ea91, round 1 + round 2 fix round, coverage-citation
  ...
  (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:98#"not backed by a check it actually ran as"`,
...
by later rounds, `CHANGELOG.md:718#"The implementer"`) and from line
556 to line 575 (moved again to line 673,
`CHANGELOG.md:1095#"Citation scanning is paragraph-joined"`); both were
...
round, `CHANGELOG.md:718#"The implementer"`) and from line 575 to line 579
(moved again to line 673, `CHANGELOG.md:1095#"Citation scanning is paragraph-joined"`); both were
...
later change, `CHANGELOG.md:718#"The implementer"`) and from line 579 to
line 598 (moved again to line 673,
`CHANGELOG.md:1095#"Citation scanning is paragraph-joined"`), in the two
...
later change, `CHANGELOG.md:718#"The implementer"`) and from line 598 to
line 637 (moved again to line 673,
`CHANGELOG.md:1095#"Citation scanning is paragraph-joined"`), in the live
...
## 2026-09-18 (agent-dx 20a01c2f, probe verdict fields)

`assets/agents/implementer.md` (the normative site for output-field
semantics), `assets/skill/references/contracts.md` (verbatim copy) and step 6
of `assets/skill/references/evidence-and-probes.md` (the orchestrator's
cross-check) each gained sentences appended to an existing line, so none of
the three files moved a cited line: the prompt stays 221 lines, contracts.md
303 and the workflow 314.
[subagent-contracts-superset.md](subagent-contracts-superset.md) gained a
section quoting the three sentences with three anchored citations; the tests
...
later change, `CHANGELOG.md:718#"The implementer"`) and from line 637 to
...
The independent review found the copy rule unsatisfiable for a runner that
states only a kill verdict, since `expectation` follows from the probe's
...
## 2026-09-18 (orchestrator-workflow 0.37.0 release cut)
...
from line 294 to line 296 (`CHANGELOG.md:718#"The implementer"`) and from
...
citations resolve into the shifted range. agent-primitives and slop-detector
carry unreleased entries of their own and were left as they are.

## 2026-09-19 (orchestrator-workflow 0.38.0 release cut)

Cut the `[Unreleased]` section under a new `[0.38.0] - 2026-09-19` heading,
keeping an empty `[Unreleased]` heading above it, and bumped `package.json`
and `package-lock.json` to 0.38.0 (minor under 0.x: the run mode with its
marker, its normative section, the policy section and README that follow
it, and the reviewer's probe replay in run mode `single`; no file under
`src/` changed since 0.37.0). Consumer-visible: the AGENTS.md policy section
changed, so an existing install receives it only with its next `init` or
`apply`; every rendered reviewer prompt gained the replay duty, which is
inert unless a briefing names run mode `single`; the `00-goal.md` template
gained the `mode` marker and the `04-implementation-summary.md` template an
Integration section, both of which an unedited install picks up on its next
`init`. The released section was also split into three bullets, one per
merged change, and wrapped to the file's usual width, with one clause added
that names the two reworded policy-section sentences the note had left out;
apart from the opening words of the second bullet the text is otherwise
...
(`CHANGELOG.md:718#"The implementer"`) and from line 690 to line 718
(`CHANGELOG.md:1095#"Citation scanning is paragraph-joined"`); both were
...
## 2026-09-20 (orchestrator-workflow 0.38.1 release cut)

Cut the `[Unreleased]` section under a new `[0.38.1] - 2026-09-20` heading,
keeping an empty `[Unreleased]` heading above it, and bumped `package.json`
and `package-lock.json` to 0.38.1 (patch: the default-mode sequence
qualifiers in the skill were aligned with the Run mode section, and the
repository's release-changelog lint accepts hyphens in build-metadata
identifiers; no file under `src/` changed since 0.38.0, and the AGENTS.md
policy section is unchanged). The test-only guard work merged since 0.38.0
(rule (c) of the citation-sibling-drift guard) ships in no tarball file and
has its own entry above. The new heading, a re-wrap of the section's second
bullet to the file's usual width, a qualifier marking its first bullet as a
...
(`CHANGELOG.md:718#"The implementer"`) and from line 723 to line 728
(`CHANGELOG.md:1095#"Citation scanning is paragraph-joined"`); both were
re-pointed mechanically after checking that each anchor sits on its new
line, in the live sites and in the earlier entries that quote them.
Re-stamped the bundle docs whose `sources:` list `CHANGELOG.md` after
confirming that none of their own citations resolve into the shifted range.
okf-kit 0.14.0, agent-primitives 0.7.0 and slop-detector 0.4.0 were cut in
the same release change; the okf-kit pins under `.github/workflows/` and
the README example moved to 0.14.0 with it.

## 2026-09-23 (agent-dx cb4ff78f, probe verdict rule completion)

Re-stamped the three source-linked invariant documents at
2026-09-23T09:48:19Z after the probe-verdict rule gained its explicit
result-only and no-verdict paths. Recomputed the affected citations into
the implementer and reviewer prompts, workflow, and contracts from their
anchor text after rewrapping, including this log's shifted CHANGELOG
citations. The bundle section quotes the eleven pinned claims, and the
CHANGELOG and tests bind those copies to the normative sites. The focused
build and probe-plan, init, documentation-consistency, and single-mode
replay suites passed; the source audit compared contract YAML blocks and
the policy fence against the base byte for byte. The astra-old-eight run's
T-005 implementation report holds the verification and mutation evidence.
...
and the shared authority-model comparison sentence shifted every citation
into implementer.md, reviewer.md, task-slicer.md, and contracts.md at or
after the edited lines; the approval-scope clause pins in
verification-sets.test.ts added no prose to those files and shifted
nothing. Every shifted citation was re-pointed to the anchor text's own
...
## 2026-09-23T17:49:55Z (agent-dx #335, snapshot sub-field and the compared-values clause)

The verification_set shape in contracts.md's Subagent input contract and
both task-slicer output copies (contracts.md and task-slicer.md) gained a
`snapshot` sub-field beside `digest` and `repository_identity`, naming the
run-local path of the frozen snapshot record; `test/verification-sets.test.ts`
extended its existing shape assertion to cover it. The shared
authority-model comparison sentence in implementer.md, reviewer.md, and
contracts.md gained a trailing clause, identical at all three sites, that
the compared values are the ones recorded in that snapshot at the path
`verification_set.snapshot` names, and that evidence-and-probes.md's
Verification sets section defines what counts as a "script" for the
comparison. evidence-and-probes.md's Freeze paragraph now states that the
orchestrator records the snapshot at a run-local path it names in the
...
..." sentence, which read as though a revision or dirty-state difference
alone withdrew the approval, was reworded to mirror implementer.md's "Use
the frozen run-local snapshot (...)" phrasing and states explicitly that
a revision or dirty-state difference alone does not withdraw the
approval. CHANGELOG.md's `[Unreleased]` entry for #335 gained a bullet
naming the compare-before-run rule (including the own-diff-is-outside-
the-approval clause) and the new `snapshot` sub-field, which it had not
named before. This entry's own predecessor above, dated 2026-09-23
"verification-set authority model and digest fields", named its
citation-shifting cause as "the criterion-4 scoping-clause pins", a
tracker-criterion-numbered phrase that does not belong in this file;
reworded to "the approval-scope clause pins in verification-sets.test.ts"
and corrected to say those pins added no prose to the shifted files and
therefore shifted nothing themselves, since only the digest/
repository_identity fields and the comparison sentence actually moved
any line.

The implementer.md and reviewer.md edits above shifted every citation
into those two files at or after the edited lines; the contracts.md and
evidence-and-probes.md edits shifted citations into those two files at or
after their own edited lines. CHANGELOG.md also gained a new
`[Unreleased]` bullet for #335 (the compare-before-run rule and the new
`snapshot` sub-field, named above), which shifted every citation into
CHANGELOG.md at or after the insertion. Every shifted citation was
re-pointed to the anchor text's own current line, confirmed against the
anchor text actually present at the new line rather than assumed from the
diff alone, across `subagent-contracts-superset.md`,
`review-gate-and-waivers.md`, and this log's own self-citations.
`subagent-contracts-superset.md`'s own prose also gained two sentences
naming the `snapshot` sub-field and the compared-values clause, each with
its own citation into contracts.md; that insertion shifted the doc's own
later content, which in turn shifted every citation this log and
CHANGELOG.md hold into it, and the `SIBLING_GUARD_BUNDLE_ALLOWLIST`
entries in `test/docs-consistency.test.ts` that record that doc's
paragraph geometry (`paragraphLine`, `secondCitationLine`, and the three
duplicate-citation entries' `claim` prose, which spells the same line
numbers out in words). Every shifted `paragraphLine`/`secondCitationLine`
was re-derived from the doc's current text, not assumed from the diff.
...
The implementer and reviewer prompts now keep run-internal identifiers out
of code, comments, tests, and commit messages (#337), and
evidence-and-probes.md gains an appended Run-internal identifiers section
with a documented check. The prompt sentences were appended to existing
lines and the new section sits at the end of its file, so no citation into
...
(`CHANGELOG.md:718#"The implementer"`) and from line 903 to line 919
(`CHANGELOG.md:1095#"Citation scanning is paragraph-joined"`); both were
