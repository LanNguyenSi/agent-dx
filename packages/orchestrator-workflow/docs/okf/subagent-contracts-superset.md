...
- Explorer: `packages/orchestrator-workflow/assets/skill/SKILL.md:325-325#"risk: low | medium | high"`
  (`## Explorer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/explorer.md:48-70#"recommendation:"`.
- Implementer: `packages/orchestrator-workflow/assets/skill/SKILL.md:381-381#"commits:"`
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/SKILL.md:429-429#"matches_implementer_claim: matched | mismatched |"`
...
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/SKILL.md:471-471#"- T-001"`
...
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/SKILL.md:499-499#"would_change_recommendation_if:"`
...
- Subagent input contract (the shape the orchestrator sends when delegating,
  ...
  `packages/orchestrator-workflow/assets/skill/SKILL.md:349-349#"format: structured"`; there is no
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:471-471#"- T-001"`): `id, title,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:349-349#"format: structured"`) requires:
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:479-479#"inventing new field values."`: "The
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:565-565#"with it. Treat a misfire as a failed spawn: resume or"` (`## Subagent
...
1. Contract-parse failure: the output does not parse against the role's
   contract (`SKILL.md:558#"against its role's output contract, including an"`). Since a same-day R2 fix-round on 0.16.0
   ...
   task assignment asked for a commit (`SKILL.md:561#"task assignment asked for a commit. When a subagent returns"`);
   see [Commits field](#commits-field) below for the field itself.
2. Near-instant return with no tool activity (`SKILL.md:562#"with no tool activity, treat that as a misfire signal rather than"`). This is a
   ...
   was answerable from the context supplied with it (`SKILL.md:565#"with it. Treat a misfire as a failed spawn: resume or"`).
...
completed step (`SKILL.md:568#"completed step. For the near-instant, no-tool-activity"`). Since 0.18.0, for the near-instant,
...
(`SKILL.md:571#"since resume keeps the subagent's prior turn in context"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`SKILL.md:576#"the resume attempt itself misfires the same way. This"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`SKILL.md:575#"resolved on the first resume attempt; fall back to a"`) — a same-day review-fix round (0.18.0)
...
constrained respawn instead (`SKILL.md:581#"watchdog stall as outside this preference. Record every"`). Record every misfire in
`03-decisions.md` (`SKILL.md:582#"This matters most for review: a misfired review is not a"`). Review-gate consequence, stated
explicitly: a misfired review is not a review and never satisfies the review
gate, since review is never skipped (`SKILL.md:583#"review and never satisfies the review gate, since"`). Review-gate
...
a second time" down to just the outcome (`SKILL.md:580#"explicitly constrained respawn produced a"`), since the
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:241-241#"lint): only claims that could vary run to run trigger"` (step 7,
...
`SKILL.md:429#"matches_implementer_claim: matched | mismatched |"` and `reviewer.md:105-108#"residual_risks:"`); `matches_implementer_claim`
...
result, restored_verified`, `SKILL.md:373#"restored_verified:"` and `implementer.md:82-86#"restored_verified:"`).
A paragraph immediately after `SKILL.md`'s contract block
(`SKILL.md:390#"reported'."`) and a matching bullet in the installed prompt
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:374-374#"replayed: false | true"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:87-87#"replayed: false | true"`),
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:252-252#"orchestrator's reviewer briefing names the replayed probes"`);
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:419-419#"recurrence: new | repeated"` and
`packages/orchestrator-workflow/assets/agents/reviewer.md:104-104#"recurrence: new | repeated"`, same field, same
...
(`SKILL.md:440#"Review-round escalation budget's trigger."`). The
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:250-250#"trigger (see below) without re-deriving it by hand."`).
...
copies identically (`packages/orchestrator-workflow/assets/skill/SKILL.md:381-381#"commits:"`
and `packages/orchestrator-workflow/assets/agents/implementer.md:94-94#"commits:"`,
...
the task branch, in order (`SKILL.md:401#"produced on the task branch, in the order produced"`
...
"forgot to report" (`SKILL.md:403#"omitting the field, so 'did not commit' is"`);
...
evidence" (`SKILL.md:561#"task assignment asked for a commit. When a subagent returns"`
