...
timestamp: 2026-09-06T20:33:16Z
sources:
...
(`packages/orchestrator-workflow/assets/agents/reviewer.md:85#"A reviewer recommendation is not orchestrator acceptance"`). The generated
...
- Explorer: `packages/orchestrator-workflow/assets/skill/SKILL.md:372#"risk: low | medium | high"`
  (`## Explorer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/explorer.md:48-70#"recommendation:"`.
- Implementer: `packages/orchestrator-workflow/assets/skill/SKILL.md:421#"## Implementer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/implementer.md:110#"role: implementer"`.
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/SKILL.md:496#"## Reviewer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/reviewer.md:128#"role: reviewer"`. Both
...
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/SKILL.md:536#"## Task slicer output contract"`
...
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/SKILL.md:606#"would_change_recommendation_if:"`
...
- Subagent input contract (the shape the orchestrator sends when delegating,
  ...
  `packages/orchestrator-workflow/assets/skill/SKILL.md:390#"## Subagent input contract"`; there is no
...
(`packages/orchestrator-workflow/test/docs-consistency.test.ts:986#"expect(skillBlock).toBe(reviewerBlock);"`); the
implementer pair gained a byte-for-byte `mutation_probes` field equality
test in 0.16.0
(`packages/orchestrator-workflow/test/docs-consistency.test.ts:1055#"expect(skillBlock).toBe(implementerBlock);"`),
reinforced by an exact-sub-field-name pin added in a same-day R2 fix-round
(`test/docs-consistency.test.ts:1102#"expect(implementerMd).toContain(field);"`) that catches a mutant the plain
...
SKILL.md")` block (`test/docs-consistency.test.ts:2445#"explorer, task-slicer, implementer, reviewer, advisor"`) only pinned
...
(`test/docs-consistency.test.ts:2473#"expect(skillBlock).toBe(advisorBlock);"`) extracts the yaml block from
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:536#"## Task slicer output contract"`): `id, title,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:390#"## Subagent input contract"`) requires
...
rule at `packages/orchestrator-workflow/assets/skill/SKILL.md:582#"rather than inventing new field"`, the
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:181#"task will not edit."`;
...
requirement (`packages/orchestrator-workflow/test/docs-consistency.test.ts:4218#"requires reference sites to be annotated in the existing task fields"`).
...
(`test/docs-consistency.test.ts:807#"^ {4}${field}:"`): it regex-extracts top-level fields
...
(`test/docs-consistency.test.ts:821#"expect(slicerFields).toEqual(skillFields);"`); the original field order
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:672#"with it. Treat a misfire as a failed spawn: resume or"` (`## Subagent
...
1. Contract-parse failure: the output does not parse against the role's
   contract (`SKILL.md:665#"against its role's output contract, including an"`). Since a same-day R2 fix-round on 0.16.0
   ...
   task assignment asked for a commit (`SKILL.md:668#"task assignment asked for a commit. When a subagent returns"`);
   see [Commits field](#commits-field) below for the field itself.
2. Near-instant return with no tool activity (`SKILL.md:669#"with no tool activity, treat that as a misfire signal rather than"`). This is a
   ...
   was answerable from the context supplied with it (`SKILL.md:672#"with it. Treat a misfire as a failed spawn: resume or"`).
...
completed step (`SKILL.md:675#"completed step. For the near-instant, no-tool-activity"`). Since 0.18.0, for the near-instant,
...
(`SKILL.md:678#"since resume keeps the subagent's prior turn in context"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`SKILL.md:683#"the resume attempt itself misfires the same way. This"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`SKILL.md:682#"resolved on the first resume attempt; fall back to a"`) — a same-day review-fix round (0.18.0)
...
constrained respawn instead (`SKILL.md:688#"watchdog stall as outside this preference. Record every"`). Record every misfire in
`03-decisions.md` (`SKILL.md:689#"This matters most for review: a misfired review is not a"`). Review-gate consequence, stated
explicitly: a misfired review is not a review and never satisfies the review
gate, since review is never skipped (`SKILL.md:690#"review and never satisfies the review gate, since"`). Review-gate
...
a second time" down to just the outcome (`SKILL.md:687#"explicitly constrained respawn produced a"`), since the
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:280#"lint): only claims that could vary run to run trigger"` (step 7,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:281#"GitHub Actions shell replay named in step 6 is a second, explicitly"`;
`packages/orchestrator-workflow/assets/agents/reviewer.md:116#"shell replay above is a second, explicitly non-probabilistic trigger for"`). The
installed `packages/orchestrator-workflow/assets/agents/reviewer.md:115#"lint) do not trigger this."`
...
`SKILL.md:523#"matches_implementer_claim: matched | mismatched |"` and `reviewer.md:141#"residual_risks:"`); `matches_implementer_claim`
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:211#"for real, observe the named test fail, restore,"` (step 6, added
...
claim-only-what-was-measured rule (`SKILL.md:214#"unverified. On any round after the task's first"`): treat a verification
...
result, restored_verified`, `SKILL.md:451#"restored_verified:"` and `implementer.md:134#"restored_verified:"`).
A paragraph immediately after `SKILL.md`'s contract block
(`SKILL.md:480#"reported'."`) and a matching bullet in the installed prompt
(`implementer.md:40#"rather than omitting the field."`) both state the not-applicable signal added in the
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:1055#"expect(skillBlock).toBe(implementerBlock);"` pins
the original 0.16.0 shape: the installed prompt's instruction and field
mention, packages/orchestrator-workflow/test/docs-consistency.test.ts:1030#"an output missing that field when probes were named is treated as a misfire, not evidence", the claim-only-what-was-measured rule, packages/orchestrator-workflow/test/docs-consistency.test.ts:1038#"never claim a run you did not execute", the
misfire-rule sentence, packages/orchestrator-workflow/test/docs-consistency.test.ts:1043#"does not parse against its role's output contract, including an implementer return that omits the", and a byte-for-byte cross-copy equality
check on the field block, packages/orchestrator-workflow/test/docs-consistency.test.ts:1055#"expect(skillBlock).toBe(implementerBlock);".
`test/docs-consistency.test.ts:1109#"expect(implementerMd).toContain(enumeration);"` pins the R2 additions: step 6's
sentence and its claim-only-what-was-measured reference, test/docs-consistency.test.ts:1084#"apply the mutant for real, observe the named test fail, restore, re-verify" (the `it` block that follows it pins the claim-only-what-was-measured reference; the rule name recurs too often in that file to anchor a citation on it), the
not-applicable clause in both copies, test/docs-consistency.test.ts:1095#"expect(implementerMd).toContain(clause);", and two exact-string pins,
test/docs-consistency.test.ts:1102#"expect(implementerMd).toContain(field);", that catch a rename applied identically to both copies — a mutant
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:223#"Record meaningful decisions in"`,
step 6). The installed prompt carries the same rule as its own bullet
(`packages/orchestrator-workflow/assets/agents/implementer.md:47#"four evidence fields plus"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:220#"A replayed probe whose mutant now survives or can no"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:48#"A replayed probe whose mutant now survives or can no"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:452#"replayed: false | true"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:135#"replayed: false | true"`),
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:295#"the replayed probes the implementer reports as killed"`);
`assets/agents/reviewer.md` itself is untouched by this change.

`packages/orchestrator-workflow/test/docs-consistency.test.ts:4318#"On any round after the task's first, the briefing also names"` pins step 6's
instruction, `test/docs-consistency.test.ts:4327#"A replayed probe whose mutant now survives or can no longer be applied is a regression signal"`
pins the regression-signal consequence, and the implementer prompt's
matching rules
(`test/docs-consistency.test.ts:4333#"On any round after the task's first, the assignment also names"`,
`test/docs-consistency.test.ts:4327#"A replayed probe whose mutant now survives or can no longer be applied is a regression signal"`). A byte-for-byte
cross-copy equality check on the `mutation_probes` block including the new
sub-field (`test/docs-consistency.test.ts:4381#"replayed: false | true"`), the step 7 reviewer-briefing
sentence (`test/docs-consistency.test.ts:4387#"the orchestrator's reviewer briefing names the replayed probes"`), and a
...
mistaken for it
(`test/docs-consistency.test.ts:4428#"outputContractBlock).not.toContain"`). A further pin locks the five
`mutation_probes` sub-fields to their fixed order in both copies
(`test/docs-consistency.test.ts:4431#"both copies' mutation_probes block has exactly the five sub-fields in a fixed order"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:513#"recurrence: new | repeated"` and
`packages/orchestrator-workflow/assets/agents/reviewer.md:137#"recurrence: new | repeated"`, same field, same
...
(`SKILL.md:534#"Review-round escalation budget's trigger."`). The
installed `reviewer.md:54#"classify each finding as"`
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:293#"trigger (see below) without re-deriving it by hand."`).
...
copies identically (`packages/orchestrator-workflow/assets/skill/SKILL.md:459#"commits:"`
and `packages/orchestrator-workflow/assets/agents/implementer.md:110#"role: implementer"`,
...
the task branch, in order (`SKILL.md:491#"produced on the task branch, in the order produced"`
and, worded identically in substance in the installed prompt's rule bullet,
`implementer.md:76#"Report the full sha of every commit you produced"`, with
"in order" carried at `implementer.md:77#"order, in the"`).
...
"forgot to report" (`SKILL.md:493#"omitting the field, so 'did not commit' is"`);
...
evidence" (`SKILL.md:668#"task assignment asked for a commit. When a subagent returns"`
and `implementer.md:78#"assignment asked for a commit is treated as a misfire, not"`).

`packages/orchestrator-workflow/test/docs-consistency.test.ts:1169#"expect(implementerMd).toContain(clause);"`
pins the field: the installed prompt's full-sha instruction and field
mention, packages/orchestrator-workflow/test/docs-consistency.test.ts:1135#"an output missing that field when the task assignment asked for a commit is treated as a misfire, not evidence", the misfire-rule sentence, packages/orchestrator-workflow/test/docs-consistency.test.ts:1141#"field even though the task assignment asked for a commit", a dedicated pin
on the "full sha" / "in order" semantics themselves (not only the
surrounding clauses), packages/orchestrator-workflow/test/docs-consistency.test.ts:1148#"in the order produced", a byte-for-byte cross-copy equality check
on the field block, packages/orchestrator-workflow/test/docs-consistency.test.ts:1163#"expect(skillBlock).toBe(implementerBlock);", and the not-applicable `commits: []` clause
in both copies, packages/orchestrator-workflow/test/docs-consistency.test.ts:1169#"expect(implementerMd).toContain(clause);".
