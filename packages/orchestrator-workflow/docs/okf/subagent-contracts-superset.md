...
timestamp: 2026-09-12T04:28:06Z
sources:
...
- Explorer: `packages/orchestrator-workflow/assets/skill/SKILL.md:389#"risk: low | medium | high"`
  (`## Explorer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/explorer.md:48-70#"recommendation:"`.
- Implementer: `packages/orchestrator-workflow/assets/skill/SKILL.md:438#"## Implementer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/implementer.md:115#"role: implementer"`.
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/SKILL.md:513#"## Reviewer output contract"`
...
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/SKILL.md:566#"## Task slicer output contract"`
...
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/SKILL.md:636#"would_change_recommendation_if:"`
...
- Subagent input contract (the shape the orchestrator sends when delegating,
  ...
  `packages/orchestrator-workflow/assets/skill/SKILL.md:407#"## Subagent input contract"`; there is no
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:566#"## Task slicer output contract"`): `id, title,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:407#"## Subagent input contract"`) requires
...
rule at `packages/orchestrator-workflow/assets/skill/SKILL.md:612#"rather than inventing new field"`, the
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:702#"with it. Treat a misfire as a failed spawn: resume or"` (`## Subagent
...
1. Contract-parse failure: the output does not parse against the role's
   contract (`SKILL.md:695#"against its role's output contract, including an"`). Since a same-day R2 fix-round on 0.16.0
   ...
   task assignment asked for a commit (`SKILL.md:698#"task assignment asked for a commit. When a subagent returns"`);
   see [Commits field](#commits-field) below for the field itself.
2. Near-instant return with no tool activity (`SKILL.md:699#"with no tool activity, treat that as a misfire signal rather than"`). This is a
   ...
   was answerable from the context supplied with it (`SKILL.md:702#"with it. Treat a misfire as a failed spawn: resume or"`).
...
completed step (`SKILL.md:705#"completed step. For the near-instant, no-tool-activity"`). Since 0.18.0, for the near-instant,
...
(`SKILL.md:708#"since resume keeps the subagent's prior turn in context"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`SKILL.md:713#"the resume attempt itself misfires the same way. This"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`SKILL.md:712#"resolved on the first resume attempt; fall back to a"`); a same-day review-fix round (0.18.0)
...
constrained respawn instead (`SKILL.md:718#"watchdog stall as outside this preference. Record every"`). Record every misfire in
`03-decisions.md` (`SKILL.md:719#"This matters most for review: a misfired review is not a"`). Review-gate consequence, stated
explicitly: a misfired review is not a review and never satisfies the review
gate, since review is never skipped (`SKILL.md:720#"review and never satisfies the review gate, since"`). Review-gate
...
a second time" down to just the outcome (`SKILL.md:717#"explicitly constrained respawn produced a"`), since the
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:289#"lint): only claims that could vary run to run trigger"` (step 7,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:290#"Actions shell replay named in step 6 is a second, explicitly"`;
...
`SKILL.md:540#"matches_implementer_claim: matched | mismatched |"` and `reviewer.md:164#"residual_risks:"`); `matches_implementer_claim`
...
result, restored_verified`, `SKILL.md:468#"restored_verified:"` and `implementer.md:139#"restored_verified:"`).
A paragraph immediately after `SKILL.md`'s contract block
(`SKILL.md:497#"reported'."`) and a matching bullet in the installed prompt
(`implementer.md:43#"rather than omitting the field."`) both state the not-applicable signal added in the
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:227#"Record meaningful decisions in"`,
step 6). The installed prompt carries the same rule as its own bullet
(`packages/orchestrator-workflow/assets/agents/implementer.md:50#"four evidence fields plus"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:224#"A replayed probe whose mutant now survives or can no"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:51#"A replayed probe whose mutant now survives or can no"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:469#"replayed: false | true"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:140#"replayed: false | true"`),
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:309#"reports as killed together with their"`);
...
instruction, `test/docs-consistency.test.ts:4688#"A replayed probe whose mutant now survives or can no longer be applied is a regression signal"`
pins the regression-signal consequence, and the implementer prompt's
matching rules
(`test/docs-consistency.test.ts:4694#"On any round after the task's first, the assignment also names"`,
`test/docs-consistency.test.ts:4703#"signal: report it as such"`). A byte-for-byte
cross-copy equality check on the `mutation_probes` block including the new
sub-field (`test/docs-consistency.test.ts:4742#"replayed: false | true"`), the step 7 reviewer-briefing
sentence (`test/docs-consistency.test.ts:4748#"the orchestrator's reviewer briefing names the replayed probes"`), and a
...
mistaken for it
(`test/docs-consistency.test.ts:4789#"outputContractBlock).not.toContain"`). A further pin locks the five
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:530#"recurrence: new | repeated"` and
...
(`SKILL.md:555#"Review-round escalation budget's trigger."`). The
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:306#"trigger (see below) without re-deriving it by hand."`).
...
copies identically (`packages/orchestrator-workflow/assets/skill/SKILL.md:476#"commits:"`
and `packages/orchestrator-workflow/assets/agents/implementer.md:115#"role: implementer"`,
...
the task branch, in order (`SKILL.md:508#"produced on the task branch, in the order produced"`
and, worded identically in substance in the installed prompt's rule bullet,
`implementer.md:81#"Report the full sha of every commit you produced"`, with
"in order" carried at `implementer.md:82#"order, in the"`).
...
"forgot to report" (`SKILL.md:510#"omitting the field, so 'did not commit' is"`);
...
evidence" (`SKILL.md:698#"task assignment asked for a commit. When a subagent returns"`
and `implementer.md:83#"assignment asked for a commit is treated as a misfire, not"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:541#"method_applied: normal | rigorous | adversarial"`
and
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:264#"never substitutes for it: do not pair"`);
...
and
`packages/orchestrator-workflow/assets/skill/SKILL.md:560#"when nothing was withdrawn. Until a"`).
...
both copies
(`packages/orchestrator-workflow/assets/skill/SKILL.md:563#"matches the briefing's"`).
