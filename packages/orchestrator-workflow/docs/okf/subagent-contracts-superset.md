...
timestamp: 2026-09-05T21:05:58Z
sources:
...
- Explorer: `packages/orchestrator-workflow/assets/skill/SKILL.md:349#"risk: low | medium | high"`
  (`## Explorer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/explorer.md:48-70#"recommendation:"`.
- Implementer: `packages/orchestrator-workflow/assets/skill/SKILL.md:405#"commits:"`
  vs. `packages/orchestrator-workflow/assets/agents/implementer.md:57#"commits:"`.
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/SKILL.md:453#"matches_implementer_claim: matched | mismatched |"`
  vs. `packages/orchestrator-workflow/assets/agents/reviewer.md:123#"reproduction:"`. Both
...
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/SKILL.md:495#"- T-001"`
  (`## Task slicer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/task-slicer.md:71#"open_questions:"`.
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/SKILL.md:523#"would_change_recommendation_if:"`
...
- Subagent input contract (the shape the orchestrator sends when delegating,
  ...
  `packages/orchestrator-workflow/assets/skill/SKILL.md:373#"format: structured"`; there is no
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:495#"- T-001"`): `id, title,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:373#"format: structured"`) requires:
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:503#"inventing new field values."`: "The
...
`packages/orchestrator-workflow/assets/agents/task-slicer.md:36#"will not edit."`).
...
`packages/orchestrator-workflow/assets/agents/task-slicer.md:32#"and must not touch — not implementation instructions."`).

## Subagent misfire rule (0.11.0, evidence relocated 0.24.0)

`packages/orchestrator-workflow/assets/skill/SKILL.md:589#"with it. Treat a misfire as a failed spawn: resume or"` (`## Subagent
...
1. Contract-parse failure: the output does not parse against the role's
   contract (`SKILL.md:582#"against its role's output contract, including an"`). Since a same-day R2 fix-round on 0.16.0
   ...
   task assignment asked for a commit (`SKILL.md:585#"task assignment asked for a commit. When a subagent returns"`);
   see [Commits field](#commits-field) below for the field itself.
2. Near-instant return with no tool activity (`SKILL.md:586#"with no tool activity, treat that as a misfire signal rather than"`). This is a
   ...
   was answerable from the context supplied with it (`SKILL.md:589#"with it. Treat a misfire as a failed spawn: resume or"`).
...
completed step (`SKILL.md:592#"completed step. For the near-instant, no-tool-activity"`). Since 0.18.0, for the near-instant,
...
(`SKILL.md:595#"since resume keeps the subagent's prior turn in context"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`SKILL.md:600#"the resume attempt itself misfires the same way. This"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`SKILL.md:599#"resolved on the first resume attempt; fall back to a"`) — a same-day review-fix round (0.18.0)
...
constrained respawn instead (`SKILL.md:605#"watchdog stall as outside this preference. Record every"`). Record every misfire in
`03-decisions.md` (`SKILL.md:606#"This matters most for review: a misfired review is not a"`). Review-gate consequence, stated
explicitly: a misfired review is not a review and never satisfies the review
gate, since review is never skipped (`SKILL.md:607#"review and never satisfies the review gate, since"`). Review-gate
...
a second time" down to just the outcome (`SKILL.md:604#"explicitly constrained respawn produced a"`), since the
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:257#"lint): only claims that could vary run to run trigger"` (step 7,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:258#"GitHub Actions shell replay named in step 6 is a second, explicitly"`;
`packages/orchestrator-workflow/assets/agents/reviewer.md:96#"shell replay above is a second, explicitly non-probabilistic trigger for"`). The
installed `packages/orchestrator-workflow/assets/agents/reviewer.md:95#"lint) do not trigger this."`
...
`SKILL.md:453#"matches_implementer_claim: matched | mismatched |"` and `reviewer.md:121#"residual_risks:"`); `matches_implementer_claim`
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:196#"for real, observe the named test fail, restore,"` (step 6, added
...
claim-only-what-was-measured rule (`SKILL.md:199#"unverified. On any round after the task's first"`): treat a verification
...
result, restored_verified`, `SKILL.md:397#"restored_verified:"` and `implementer.md:92#"restored_verified:"`).
A paragraph immediately after `SKILL.md`'s contract block
(`SKILL.md:414#"reported'."`) and a matching bullet in the installed prompt
(`implementer.md:27#"rather than omitting the field."`) both state the not-applicable signal added in the
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:208#"Record meaningful decisions in"`,
step 6). The installed prompt carries the same rule as its own bullet
(`packages/orchestrator-workflow/assets/agents/implementer.md:34#"four evidence fields plus"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:205#"A replayed probe whose mutant now survives or can no"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:35#"A replayed probe whose mutant now survives or can no"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:398#"replayed: false | true"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:93#"replayed: false | true"`),
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:272#"the replayed probes the implementer reports as killed"`);
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:443#"recurrence: new | repeated"` and
`packages/orchestrator-workflow/assets/agents/reviewer.md:117#"recurrence: new | repeated"`, same field, same
...
(`SKILL.md:464#"Review-round escalation budget's trigger."`). The
installed `reviewer.md:35#"classify each finding as"`
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:270#"trigger (see below) without re-deriving it by hand."`).
...
copies identically (`packages/orchestrator-workflow/assets/skill/SKILL.md:405#"commits:"`
and `packages/orchestrator-workflow/assets/agents/implementer.md:57#"commits:"`,
...
the task branch, in order (`SKILL.md:425#"produced on the task branch, in the order produced"`
and, worded identically in substance in the installed prompt's rule bullet,
`implementer.md:54#"Report the full sha of every commit you produced"`, with
"in order" carried at `implementer.md:55#"order, in the"`).
...
"forgot to report" (`SKILL.md:427#"omitting the field, so 'did not commit' is"`);
...
evidence" (`SKILL.md:585#"task assignment asked for a commit. When a subagent returns"`
and `implementer.md:56#"assignment asked for a commit is treated as a misfire, not"`).
