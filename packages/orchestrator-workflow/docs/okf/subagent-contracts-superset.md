...
timestamp: 2026-09-25T05:05:40Z
sources:
...
- Implementer: `packages/orchestrator-workflow/assets/skill/references/contracts.md:101#"## Implementer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/implementer.md:239#"role: implementer"`.
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/references/contracts.md:182#"## Reviewer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/reviewer.md:277#"role: reviewer"`. Both
...
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/references/contracts.md:274#"## Task slicer output contract"`
  (`## Task slicer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/task-slicer.md:68#"role: task_slicer"`.
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/references/contracts.md:349#"would_change_recommendation_if:"`
...
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:274#"## Task slicer output contract"`): `id, title,
...
rule at `packages/orchestrator-workflow/assets/skill/references/contracts.md:325#"rather than inventing new field"`, the
...
`packages/orchestrator-workflow/assets/agents/task-slicer.md:58#"will not edit."`).
...
`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:17#"context supplied with it. Treat a misfire as a failed spawn: resume or"` (`## Subagent
...
2. Near-instant return with no tool activity (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:14#"with no tool activity, treat that as a misfire signal rather than"`). This is a
   ...
   was answerable from the context supplied with it (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:17#"with it. Treat a misfire as a failed spawn: resume or"`).
...
completed step (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:20#"completed step. For the near-instant, no-tool-activity"`). Since 0.18.0, for the near-instant,
...
(`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:23#"since resume keeps the subagent's prior turn in context"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:28#"the resume attempt itself misfires the same way. This"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:27#"resolved on the first resume attempt; fall back to a"`); a same-day review-fix round (0.18.0)
...
constrained respawn instead (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:33#"watchdog stall as outside this preference. Record every"`). Record every misfire in
`03-decisions.md` (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:34#"This matters most for review: a misfired review is not a"`). Review-gate consequence, stated
explicitly: a misfired review is not a review and never satisfies the review
gate, since review is never skipped (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:35#"review and never satisfies the review gate, since"`). Review-gate
...
a second time" down to just the outcome (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:32#"explicitly constrained respawn produced a"`), since the
...
`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:198-199#"only claims that could vary run to run trigger it."` (detailed workflow,
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:200#"The GitHub Actions shell replay named in step 6 is a second, explicitly"`;
`packages/orchestrator-workflow/assets/agents/reviewer.md:218#"shell replay above is a second, explicitly non-probabilistic trigger for"`). The
installed `packages/orchestrator-workflow/assets/agents/reviewer.md:217#"lint) do not trigger this."`
...
`packages/orchestrator-workflow/assets/skill/references/contracts.md:211#"matches_implementer_claim: matched | mismatched |"` and `reviewer.md:291#"residual_risks:"`); `matches_implementer_claim`
...
result, restored_verified`, `packages/orchestrator-workflow/assets/skill/references/contracts.md:137#"restored_verified:"` and `implementer.md:269#"restored_verified:"`)
...
(`implementer.md:94#"rather than omitting the field."`) states the not-applicable signal added in the
...
(`packages/orchestrator-workflow/assets/agents/implementer.md:97#"On any round after the task's first, the assignment also names"`).
...
and
`packages/orchestrator-workflow/assets/agents/implementer.md:106#"A replayed probe whose"`).
...
and
`packages/orchestrator-workflow/assets/agents/implementer.md:270#"replayed: false | true"`),
...
contract itself
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:218#"implementer reports as killed together with their mutant definition"`);
...
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:200#"recurrence: new | repeated"` and
`packages/orchestrator-workflow/assets/agents/reviewer.md:286#"recurrence: new | repeated"`, same field, same
...
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:262#"Review-round escalation budget's trigger."`). The
...
`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:215#"Review-round escalation budget's trigger (see below) without re-deriving it"`).
...
(`implementer.md:86#"otherwise (for example when the mutant"`).
...
(`implementer.md:88#"text, required when"`).
...
(`implementer.md:92#"(a replayed probe whose mutant can no longer be"`).
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:217#"the orchestrator's reviewer briefing names the replayed probes the"`). Run mode `single` inverts this permission: the orchestrator is then the author of the probes, so the same step defines once what a named probe is (its full definition or a resolved immutable plan-and-result reference, never an id alone), requires the reviewer to replay every named orchestrator probe, and to report, in the existing `reproduction` field, whether each replayed verdict matches the recorded one, a mismatch also setting `matches_implementer_claim: mismatched` (packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:224#"that skip permission does not apply"). The reviewer prompt carries the duty in its own words, because a reviewer runs without the skill, applies it under any `review_method` (its `normal` row lists the replay among the obligations no method suspends), and keeps it inert unless the briefing names the mode (packages/orchestrator-workflow/assets/agents/reviewer.md:271#"Without that mode line in the briefing this obligation"); `contracts.md` only points to the step and adds no output field (packages/orchestrator-workflow/assets/skill/references/contracts.md:272#"no output field is added for it"). `test/single-probe-replay.test.ts` binds the prompt and the pointer to the rule's wording through one constant and checks every rendered reviewer variant.
...
in `summary`
(`packages/orchestrator-workflow/assets/agents/implementer.md:112#"On any round after the task's first, when the round fixes a review"`).
...
and `packages/orchestrator-workflow/assets/agents/implementer.md:271#"class_closure:"`,
...
(`packages/orchestrator-workflow/assets/agents/implementer.md:123#"when every site the round found (by search or by source-level"`).
The round also runs one mutation probe per review finding it fixed, in
addition to any probe the assignment names
(`implementer.md:126#"Run one mutation probe per"`).
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:274#"Halt at the first"`);
the Round-2 halt rule's own cross-reference to step 8 states the identical
scope
(`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:50#"Step 8 of the detailed workflow states the operational"`),
...
and `packages/orchestrator-workflow/assets/agents/implementer.md:239#"role: implementer"`,
...
`implementer.md:187#"Report the full sha of every commit you produced"`, with
"in order" carried at `implementer.md:188#"order, in the"`).
...
"forgot to report" (`implementer.md:190#"evidence. When the task produced no commit, return"`);
...
and `implementer.md:189#"assignment asked for a commit is treated as a misfire, not"`).
...
finding first
(`packages/orchestrator-workflow/assets/agents/implementer.md:194#"Before committing, when slop-detector is available run"`),
...
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:212#"method_applied: normal | rigorous | adversarial"`
and
`packages/orchestrator-workflow/assets/agents/reviewer.md:298#"method_applied: normal | rigorous | adversarial"`,
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:175-176#"reviewer tier, a budget mismatch that names probes without the effort to run"`);
...
and
`packages/orchestrator-workflow/assets/skill/references/contracts.md:272#"when nothing was withdrawn."`).
...
(`packages/orchestrator-workflow/assets/agents/implementer.md:148#"reacted to the mutant under the runner's pass predicate"`):
...
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:162#"reacted to the mutant under the runner's pass predicate"`).
...
(`packages/orchestrator-workflow/assets/agents/reviewer.md:233#"probes you run, apply the implementer's verdict-copy"`):
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:243#"the probe, the replayed verdict or"`):
