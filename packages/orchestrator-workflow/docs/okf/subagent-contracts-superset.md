...
(`packages/orchestrator-workflow/test/docs-consistency.test.ts:797-810#"expect(skillBlock).toBe(reviewerBlock);"`); the
implementer pair gained a byte-for-byte `mutation_probes` field equality
test in 0.16.0
(`packages/orchestrator-workflow/test/docs-consistency.test.ts:872-884#"expect(skillBlock).toBe(implementerBlock);"`),
reinforced by an exact-sub-field-name pin added in a same-day R2 fix-round
(`test/docs-consistency.test.ts:927-931#"expect(implementerMd).toContain(field);"`) that catches a mutant the plain
...
`packages/orchestrator-workflow/CHANGELOG.md:745-768#0.10.0` (0.10.0) records that the
...
(`test/docs-consistency.test.ts:636-646#"expect(slicerFields).toEqual(skillFields);"`); the original field order
(`id, title, goal, relevant_files, ... dependencies, risk`) survives around
the newer fields (`test/docs-consistency.test.ts:649-672#"cursor = idx;"`); `02-tasks.md`'s
...
Motivation, `packages/orchestrator-workflow/CHANGELOG.md:722-739#0.11.0` (0.11.0): a
...
(`packages/orchestrator-workflow/CHANGELOG.md:432-484#0.18.0`, agent-tasks task
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:367-401#"never satisfies the review gate"` pins
...
`test/docs-consistency.test.ts:421-482#"did not resolve on resume; only a fresh, explicitly"` pins the 0.18.0 extension, this
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:181-188#"lint): only claims that could vary run to run trigger"` (step 7,
...
Motivation, `packages/orchestrator-workflow/CHANGELOG.md:636-661#0.14.0` (0.14.0): the
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:136-155#"for real, observe the named test fail, restore,"` (step 6, added
...
claim-only-what-was-measured rule (`SKILL.md:155-158#"unverified. Record meaningful decisions in"`): treat a verification
...
Motivation, `packages/orchestrator-workflow/CHANGELOG.md:545-587#0.16.0` (0.16.0 plus
