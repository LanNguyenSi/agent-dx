...
(`packages/orchestrator-workflow/assets/agents/reviewer.md:143#"A reviewer recommendation is not orchestrator acceptance"`). The generated
...
- Implementer: `packages/orchestrator-workflow/assets/skill/references/contracts.md:95#"## Implementer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/implementer.md:227#"role: implementer"`.
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/references/contracts.md:173#"## Reviewer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/reviewer.md:258#"role: reviewer"`. Both
...
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/references/contracts.md:263#"## Task slicer output contract"`
...
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/references/contracts.md:337#"would_change_recommendation_if:"`
...
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:263#"## Task slicer output contract"`): `id, title,
...
rule at `packages/orchestrator-workflow/assets/skill/references/contracts.md:313#"rather than inventing new field"`, the
...
`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:15#"context supplied with it. Treat a misfire as a failed spawn: resume or"` (`## Subagent
...
2. Near-instant return with no tool activity (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:12#"with no tool activity, treat that as a misfire signal rather than"`). This is a
   ...
   was answerable from the context supplied with it (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:15#"with it. Treat a misfire as a failed spawn: resume or"`).
...
completed step (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:18#"completed step. For the near-instant, no-tool-activity"`). Since 0.18.0, for the near-instant,
...
(`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:21#"since resume keeps the subagent's prior turn in context"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:26#"the resume attempt itself misfires the same way. This"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:25#"resolved on the first resume attempt; fall back to a"`); a same-day review-fix round (0.18.0)
...
constrained respawn instead (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:31#"watchdog stall as outside this preference. Record every"`). Record every misfire in
`03-decisions.md` (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:32#"This matters most for review: a misfired review is not a"`). Review-gate consequence, stated
explicitly: a misfired review is not a review and never satisfies the review
gate, since review is never skipped (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:33#"review and never satisfies the review gate, since"`). Review-gate
...
a second time" down to just the outcome (`packages/orchestrator-workflow/assets/skill/references/review-and-recovery.md:30#"explicitly constrained respawn produced a"`), since the
...
`packages/orchestrator-workflow/assets/agents/reviewer.md:199#"shell replay above is a second, explicitly non-probabilistic trigger for"`). The
installed `packages/orchestrator-workflow/assets/agents/reviewer.md:198#"lint) do not trigger this."`
...
`packages/orchestrator-workflow/assets/skill/references/contracts.md:200#"matches_implementer_claim: matched | mismatched |"` and `reviewer.md:272#"residual_risks:"`); `matches_implementer_claim`
...
result, restored_verified`, `packages/orchestrator-workflow/assets/skill/references/contracts.md:131#"restored_verified:"` and `implementer.md:257#"restored_verified:"`)
...
(`implementer.md:82#"rather than omitting the field."`) states the not-applicable signal added in the
...
(`packages/orchestrator-workflow/assets/agents/implementer.md:85#"On any round after the task's first, the assignment also names"`).
...
and
`packages/orchestrator-workflow/assets/agents/implementer.md:94#"A replayed probe whose"`).
...
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:132#"replayed: false | true"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:258#"replayed: false | true"`),
...
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:189#"recurrence: new | repeated"` and
`packages/orchestrator-workflow/assets/agents/reviewer.md:267#"recurrence: new | repeated"`, same field, same
...
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:251#"Review-round escalation budget's trigger."`). The
installed `reviewer.md:99#"classify each finding as"`
...
(`implementer.md:74#"otherwise (for example when the mutant"`).
...
(`implementer.md:76#"text, required when"`).
...
(`implementer.md:80#"(a replayed probe whose mutant can no longer be"`).
...
in `summary`
(`packages/orchestrator-workflow/assets/agents/implementer.md:100#"On any round after the task's first, when the round fixes a review"`).
The result is reported in the implementer output contract's
`class_closure` field, added to both copies identically
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:133#"class_closure:"`
and `packages/orchestrator-workflow/assets/agents/implementer.md:259#"class_closure:"`,
...
(`packages/orchestrator-workflow/assets/agents/implementer.md:111#"when every site the round found (by search or by source-level"`).
The round also runs one mutation probe per review finding it fixed, in
addition to any probe the assignment names
(`implementer.md:114#"Run one mutation probe per"`).
...
(`packages/orchestrator-workflow/assets/agents/reviewer.md:109#"a site your search finds that the implementer's report omits"`).
...
(`reviewer.md:112#"there is no separate implementer"`).
...
(`reviewer.md:101#"Class match, not site match, decides this"`).
...
copies identically (`packages/orchestrator-workflow/assets/skill/references/contracts.md:145#"commits:"`
and `packages/orchestrator-workflow/assets/agents/implementer.md:227#"role: implementer"`,
...
`implementer.md:175#"Report the full sha of every commit you produced"`, with
"in order" carried at `implementer.md:176#"order, in the"`).
...
"forgot to report" (`implementer.md:178#"evidence. When the task produced no commit, return"`);
...
and `implementer.md:177#"assignment asked for a commit is treated as a misfire, not"`).
...
finding first
(`packages/orchestrator-workflow/assets/agents/implementer.md:182#"Before committing, when slop-detector is available run"`),
...
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:201#"method_applied: normal | rigorous | adversarial"`
and
`packages/orchestrator-workflow/assets/agents/reviewer.md:279#"method_applied: normal | rigorous | adversarial"`,
...
and
`packages/orchestrator-workflow/assets/skill/references/contracts.md:261#"when nothing was withdrawn."`).
...
(`packages/orchestrator-workflow/assets/agents/implementer.md:136#"reacted to the mutant under the runner's pass predicate"`):
...
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:153#"reacted to the mutant under the runner's pass predicate"`).
...
(`packages/orchestrator-workflow/assets/agents/reviewer.md:214#"probes you run, apply the implementer's verdict-copy"`):
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:235#"the probe, the replayed verdict or"`):
