...
- Explorer: `packages/orchestrator-workflow/assets/skill/references/contracts.md:1#"## Explorer output contract"`
  ...
  `packages/orchestrator-workflow/assets/agents/explorer.md:51-73#"recommendation:"`.
- Implementer: `packages/orchestrator-workflow/assets/skill/references/contracts.md:101#"## Implementer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/implementer.md:245#"role: implementer"`.
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/references/contracts.md:182#"## Reviewer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/reviewer.md:281#"role: reviewer"`. Both
...
- Task-slicer:
  ...
  `packages/orchestrator-workflow/assets/agents/task-slicer.md:71#"role: task_slicer"`.
...
`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:207-208#"only claims that could vary run to run trigger it."` (detailed workflow,
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:209#"The GitHub Actions shell replay named in step 6 is a second, explicitly"`;
`packages/orchestrator-workflow/assets/agents/reviewer.md:222#"shell replay above is a second, explicitly non-probabilistic trigger for"`). The
installed `packages/orchestrator-workflow/assets/agents/reviewer.md:221#"lint) do not trigger this."`
...
`packages/orchestrator-workflow/assets/skill/references/contracts.md:211#"matches_implementer_claim: matched | mismatched |"` and `reviewer.md:295#"residual_risks:"`); `matches_implementer_claim`
...
result, restored_verified`, `packages/orchestrator-workflow/assets/skill/references/contracts.md:137#"restored_verified:"` and `implementer.md:275#"restored_verified:"`)
...
(`implementer.md:100#"rather than omitting the field."`) states the not-applicable signal added in the
...
(`packages/orchestrator-workflow/assets/agents/implementer.md:103#"On any round after the task's first, the assignment also names"`).
...
and
`packages/orchestrator-workflow/assets/agents/implementer.md:112#"A replayed probe whose"`).
...
and
`packages/orchestrator-workflow/assets/agents/implementer.md:276#"replayed: false | true"`),
...
contract itself
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:227#"implementer reports as killed together with their mutant definition"`);
...
`packages/orchestrator-workflow/assets/agents/reviewer.md:290#"recurrence: new | repeated"`, same field, same
...
`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:224#"Review-round escalation budget's trigger (see below) without re-deriving it"`).
...
(`implementer.md:92#"otherwise (for example when the mutant"`).
...
(`implementer.md:94#"text, required when"`).
...
(`implementer.md:98#"(a replayed probe whose mutant can no longer be"`).
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:226#"the orchestrator's reviewer briefing names the replayed probes the"`). Run mode `single` inverts this permission: the orchestrator is then the author of the probes, so the same step defines once what a named probe is (its full definition or a resolved immutable plan-and-result reference, never an id alone), requires the reviewer to replay every named orchestrator probe, and to report, in the existing `reproduction` field, whether each replayed verdict matches the recorded one, a mismatch also setting `matches_implementer_claim: mismatched` (packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:233#"that skip permission does not apply"). The reviewer prompt carries the duty in its own words, because a reviewer runs without the skill, applies it under any `review_method` (its `normal` row lists the replay among the obligations no method suspends), and keeps it inert unless the briefing names the mode (packages/orchestrator-workflow/assets/agents/reviewer.md:275#"Without that mode line in the briefing this obligation"); `contracts.md` only points to the step and adds no output field (packages/orchestrator-workflow/assets/skill/references/contracts.md:272#"no output field is added for it"). `test/single-probe-replay.test.ts` binds the prompt and the pointer to the rule's wording through one constant and checks every rendered reviewer variant.
...
in `summary`
(`packages/orchestrator-workflow/assets/agents/implementer.md:118#"On any round after the task's first, when the round fixes a review"`).
...
and `packages/orchestrator-workflow/assets/agents/implementer.md:277#"class_closure:"`,
...
(`packages/orchestrator-workflow/assets/agents/implementer.md:129#"when every site the round found (by search or by source-level"`).
The round also runs one mutation probe per review finding it fixed, in
addition to any probe the assignment names
(`implementer.md:132#"Run one mutation probe per"`).
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:283#"Halt at the first"`);
...
and `packages/orchestrator-workflow/assets/agents/implementer.md:245#"role: implementer"`,
...
`implementer.md:193#"Report the full sha of every commit you produced"`, with
"in order" carried at `implementer.md:194#"order, in the"`).
...
"forgot to report" (`implementer.md:196#"evidence. When the task produced no commit, return"`);
...
and `implementer.md:195#"assignment asked for a commit is treated as a misfire, not"`).
...
finding first
(`packages/orchestrator-workflow/assets/agents/implementer.md:200#"Before committing, when slop-detector is available run"`),
...
and
`packages/orchestrator-workflow/assets/agents/reviewer.md:302#"method_applied: normal | rigorous | adversarial"`,
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:184-185#"reviewer tier, a budget mismatch that names probes without the effort to run"`);
...
(`packages/orchestrator-workflow/assets/agents/implementer.md:154#"reacted to the mutant under the runner's pass predicate"`):
...
(`packages/orchestrator-workflow/assets/agents/reviewer.md:237#"probes you run, apply the implementer's verdict-copy"`):
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:252#"the probe, the replayed verdict or"`):
