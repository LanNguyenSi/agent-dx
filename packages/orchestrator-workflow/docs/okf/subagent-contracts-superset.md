...
`packages/orchestrator-workflow/src/models.ts:8-14#"export const ROLES: Role[] = ["` (`Role` type at `:1-6`)
= `explorer, task-slicer, implementer, reviewer, advisor` (five since
0.21.0, four before it), narratively mirrored at
`packages/orchestrator-workflow/assets/skill/SKILL.md:28-63#"## Roles"` (`## Roles`).
...
the profile-scoping mechanics — `MINIMAL_PROFILE_ROLES`, `src/models.ts:42-45#"const MINIMAL_PROFILE_ROLES: ReadonlySet<Role> = new"`,
...
`packages/orchestrator-workflow/src/models.ts:22-26#"export const READ_ONLY_ROLES: ReadonlySet<Role> = new"` (advisor added
...
which `packages/orchestrator-workflow/test/docs-consistency.test.ts:513-520#"describe("`
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:55-63#"Where the harness supports subagent definitions, the"`). Per-role
default models (`DEFAULT_MODELS`,
`packages/orchestrator-workflow/src/models.ts:80-86#"export const DEFAULT_MODELS: Record<Role, string> = {"`) are out of this doc's
...
- Explorer: `packages/orchestrator-workflow/assets/skill/SKILL.md:225-250#"## Explorer output contract"`
  (`## Explorer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/explorer.md:47-70#"status: done | partial | blocked"`.
- Implementer: `packages/orchestrator-workflow/assets/skill/SKILL.md:273-301#"## Implementer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/implementer.md:36-62#"status: done | partial | blocked"`.
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/SKILL.md:310-333#"## Reviewer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/reviewer.md:60-81#"the implementer's claim in the"`. Both
  copies gained a `reproduction` field in 0.14.0; see
  [Reproduction requirement](#reproduction-requirement-0140) below.
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/SKILL.md:339-371#"## Task slicer output contract"`
  (`## Task slicer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/task-slicer.md:36-66#"status: done | partial | blocked"`.
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/SKILL.md:378-400#"## Advisor output contract"`
  (`## Advisor output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/advisor.md:53-73#"status: done | partial | blocked"`. Direct
...
- Subagent input contract (the shape the orchestrator sends when delegating,
  ...
  `packages/orchestrator-workflow/assets/skill/SKILL.md:252-271#"## Subagent input contract"`; there is no
...
(`packages/orchestrator-workflow/test/docs-consistency.test.ts:797-811#"the reproduction field is byte-for-byte identical"`); the
implementer pair gained a byte-for-byte `mutation_probes` field equality
test in 0.16.0
(`packages/orchestrator-workflow/test/docs-consistency.test.ts:872-885#"both implementer output contracts carry an identical"`),
reinforced by an exact-sub-field-name pin added in a same-day R2 fix-round
(`test/docs-consistency.test.ts:927-932#"both copies pin the mutation_probes field block by its"`) that catches a mutant the plain
...
SKILL.md")` block (`test/docs-consistency.test.ts:1923-1999#"describe("`) only pinned
...
(`test/docs-consistency.test.ts:2010-2026#"describe("`) extracts the yaml block from
...
`packages/orchestrator-workflow/CHANGELOG.md:713-736#0.10.0` (0.10.0) records that the
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:339-371#"## Task slicer output contract"`): `id, title,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:252-271#"## Subagent input contract"`) requires:
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:373-376#"The orchestrator copies each task's goal,"`: "The
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:536-743#"describe("`
...
(`test/docs-consistency.test.ts:596-634#"no field required by the subagent input contract is"`): it regex-extracts top-level fields
...
(`test/docs-consistency.test.ts:636-647#"both slicer output copies carry the same task fields in"`); the original field order
(`id, title, goal, relevant_files, ... dependencies, risk`) survives around
the newer fields (`test/docs-consistency.test.ts:649-674#"SKILL.md's task slicer output contract keeps id, title,"`); `02-tasks.md`'s
sections map 1:1 to the fields in order
(`test/docs-consistency.test.ts:681-700#"02-tasks.md sections map 1:1 to the slicer output"`); and `task-slicer.md` must frame
`allowed_changes`/`forbidden_changes` as scope boundaries for the
implementer, not implementation instructions
(`test/docs-consistency.test.ts:738-742#"task-slicer.md frames allowed/forbidden changes as"`, prompt text at
`packages/orchestrator-workflow/assets/agents/task-slicer.md:27-29#"forbidden changes, dependencies, and risk. Allowed"`).

## Subagent misfire rule (0.11.0, evidence relocated 0.24.0)

`packages/orchestrator-workflow/assets/skill/SKILL.md:438-447#"## Subagent misfire rule"` (`## Subagent
...
1. Contract-parse failure: the output does not parse against the role's
   contract (`SKILL.md:440-441#"A subagent return is a misfire, not evidence, when its"`). Since a same-day R2 fix-round on 0.16.0
   ...
   mutation probes to run (`SKILL.md:442-443#"omits the"`).
2. Near-instant return with no tool activity (`SKILL.md:443-444#"mutation probes to run. When a subagent returns"`). This is a
   ...
   was answerable from the context supplied with it (`SKILL.md:445-447#"against the contract with extra suspicion, and accept"`).
...
completed step (`SKILL.md:447-449#"with it. Treat a misfire as a failed spawn: resume or"`). Since 0.18.0, for the near-instant,
...
(`SKILL.md:449-452#"completed step. For the near-instant, no-tool-activity"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`SKILL.md:456-457#"resolved on the first resume attempt; fall back to a"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`SKILL.md:453-456#"starts cold and risks the same misfire again. Every"`) — a same-day review-fix round (0.18.0)
...
constrained respawn instead (`SKILL.md:457-462#"the resume attempt itself misfires the same way. This"`). Record every misfire in
`03-decisions.md` (`SKILL.md:462-463#"watchdog stall as outside this preference. Record every"`). Review-gate consequence, stated
...
a second time" down to just the outcome (`SKILL.md:459-461#"mid-run watchdog stall (the subagent goes idle partway"`), since the
incident count is now evidence rather than rule text too. `SKILL.md:93-94#"change (the merge-base with the remote default branch);"`
...
Motivation, `packages/orchestrator-workflow/CHANGELOG.md:690-707#0.11.0` (0.11.0): a
...
(`packages/orchestrator-workflow/CHANGELOG.md:400-452#0.18.0`, agent-tasks task
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:367-403#"describe("` pins
...
`test/docs-consistency.test.ts:421-485#"describe("` pins the 0.18.0 extension, this
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:181-188#"rows as the template never having been filled in. When"` (step 7,
...
installed `packages/orchestrator-workflow/assets/agents/reviewer.md:51-56#"- Review the diff against its stated goal; if the goal"`
...
`SKILL.md:328-332#"reproduction:"` and `reviewer.md:76-80#"acceptance_recommendation: accept | accept_with_notes |"`); `matches_implementer_claim`
...
Motivation, `packages/orchestrator-workflow/CHANGELOG.md:604-629#0.14.0` (0.14.0): the
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:136-155#"6. **Delegate implementation.** Send each implementer"` (step 6, added
...
claim-only-what-was-measured rule (`SKILL.md:155-158#"for real, observe the named test fail, restore,"`): treat a verification
...
result, restored_verified`, `SKILL.md:290-294#"mutation_probes:"` and `implementer.md:51-55#"mutation_probes:"`).
A paragraph immediately after `SKILL.md`'s contract block
(`SKILL.md:303-308#"When the task assignment names mutation probes to run,"`) and a matching bullet in the installed prompt
(`implementer.md:16-21#"- When the task assignment names mutation probes to"`) both state the not-applicable signal added in the
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:846-886#"describe("` pins
...
`test/docs-consistency.test.ts:904-940#"describe("` pins the R2 additions: step 6's
...
Motivation, `packages/orchestrator-workflow/CHANGELOG.md:513-555#0.16.0` (0.16.0 plus
