...
timestamp: 2026-09-12T04:49:30Z
sources:
...
- Explorer: `packages/orchestrator-workflow/assets/skill/SKILL.md:387#"risk: low | medium | high"`
  (`## Explorer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/explorer.md:48-70#"recommendation:"`.
- Implementer: `packages/orchestrator-workflow/assets/skill/SKILL.md:436#"## Implementer output contract"`
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/SKILL.md:511#"## Reviewer output contract"`
...
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/SKILL.md:564#"## Task slicer output contract"`
...
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/SKILL.md:634#"would_change_recommendation_if:"`
...
- Subagent input contract (the shape the orchestrator sends when delegating,
  ...
  `packages/orchestrator-workflow/assets/skill/SKILL.md:405#"## Subagent input contract"`; there is no
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:564#"## Task slicer output contract"`): `id, title,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:405#"## Subagent input contract"`) requires
...
rule at `packages/orchestrator-workflow/assets/skill/SKILL.md:610#"rather than inventing new field"`, the
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:700#"with it. Treat a misfire as a failed spawn: resume or"` (`## Subagent
...
1. Contract-parse failure: the output does not parse against the role's
   contract (`SKILL.md:693#"against its role's output contract, including an"`). Since a same-day R2 fix-round on 0.16.0
   ...
   task assignment asked for a commit (`SKILL.md:696#"task assignment asked for a commit. When a subagent returns"`);
   see [Commits field](#commits-field) below for the field itself.
2. Near-instant return with no tool activity (`SKILL.md:697#"with no tool activity, treat that as a misfire signal rather than"`). This is a
   ...
   was answerable from the context supplied with it (`SKILL.md:700#"with it. Treat a misfire as a failed spawn: resume or"`).
...
completed step (`SKILL.md:703#"completed step. For the near-instant, no-tool-activity"`). Since 0.18.0, for the near-instant,
...
(`SKILL.md:706#"since resume keeps the subagent's prior turn in context"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`SKILL.md:711#"the resume attempt itself misfires the same way. This"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`SKILL.md:710#"resolved on the first resume attempt; fall back to a"`); a same-day review-fix round (0.18.0)
...
constrained respawn instead (`SKILL.md:716#"watchdog stall as outside this preference. Record every"`). Record every misfire in
`03-decisions.md` (`SKILL.md:717#"This matters most for review: a misfired review is not a"`). Review-gate consequence, stated
explicitly: a misfired review is not a review and never satisfies the review
gate, since review is never skipped (`SKILL.md:718#"review and never satisfies the review gate, since"`). Review-gate
...
a second time" down to just the outcome (`SKILL.md:715#"explicitly constrained respawn produced a"`), since the
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:288#"lint): only claims that could vary run to run trigger"` (step 7,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:289#"Actions shell replay named in step 6 is a second, explicitly"`;
...
`SKILL.md:538#"matches_implementer_claim: matched | mismatched |"` and `reviewer.md:164#"residual_risks:"`); `matches_implementer_claim`
...
result, restored_verified`, `SKILL.md:466#"restored_verified:"` and `implementer.md:139#"restored_verified:"`).
A paragraph immediately after `SKILL.md`'s contract block
(`SKILL.md:495#"reported'."`) and a matching bullet in the installed prompt
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:467#"replayed: false | true"`
and
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:307#"reports as killed together with their"`);
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:528#"recurrence: new | repeated"` and
...
(`SKILL.md:553#"Review-round escalation budget's trigger."`). The
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:304#"trigger (see below) without re-deriving it"`).
...
copies identically (`packages/orchestrator-workflow/assets/skill/SKILL.md:474#"commits:"`
...
the task branch, in order (`SKILL.md:506#"produced on the task branch, in the order produced"`
...
"forgot to report" (`SKILL.md:508#"omitting the field, so 'did not commit' is"`);
...
evidence" (`SKILL.md:696#"task assignment asked for a commit. When a subagent returns"`
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:539#"method_applied: normal | rigorous | adversarial"`
and
...
and
`packages/orchestrator-workflow/assets/skill/SKILL.md:558#"when nothing was withdrawn. Until a"`).
...
both copies
(`packages/orchestrator-workflow/assets/skill/SKILL.md:561#"matches the briefing's"`).
