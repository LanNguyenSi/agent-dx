...
timestamp: 2026-09-06T05:42:00Z
sources:
...
- Implementer: `packages/orchestrator-workflow/assets/skill/SKILL.md:417#"## Implementer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/implementer.md:101#"role: implementer"`.
...
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/SKILL.md:532#"## Task slicer output contract"`
...
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/SKILL.md:602#"would_change_recommendation_if:"`
...
- Subagent input contract (the shape the orchestrator sends when delegating,
  ...
  `packages/orchestrator-workflow/assets/skill/SKILL.md:386#"## Subagent input contract"`; there is no
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:532#"## Task slicer output contract"`): `id, title,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:386#"## Subagent input contract"`) requires
...
rule at `packages/orchestrator-workflow/assets/skill/SKILL.md:578#"rather than inventing new field"`, the
...
requirement (`packages/orchestrator-workflow/test/docs-consistency.test.ts:4148#"requires reference sites to be annotated in the existing task fields"`).
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:668#"with it. Treat a misfire as a failed spawn: resume or"` (`## Subagent
...
1. Contract-parse failure: the output does not parse against the role's
   contract (`SKILL.md:661#"against its role's output contract, including an"`). Since a same-day R2 fix-round on 0.16.0
   ...
   task assignment asked for a commit (`SKILL.md:664#"task assignment asked for a commit. When a subagent returns"`);
   see [Commits field](#commits-field) below for the field itself.
2. Near-instant return with no tool activity (`SKILL.md:665#"with no tool activity, treat that as a misfire signal rather than"`). This is a
   ...
   was answerable from the context supplied with it (`SKILL.md:668#"with it. Treat a misfire as a failed spawn: resume or"`).
...
completed step (`SKILL.md:671#"completed step. For the near-instant, no-tool-activity"`). Since 0.18.0, for the near-instant,
...
(`SKILL.md:674#"since resume keeps the subagent's prior turn in context"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`SKILL.md:679#"the resume attempt itself misfires the same way. This"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`SKILL.md:678#"resolved on the first resume attempt; fall back to a"`) — a same-day review-fix round (0.18.0)
...
constrained respawn instead (`SKILL.md:684#"watchdog stall as outside this preference. Record every"`). Record every misfire in
`03-decisions.md` (`SKILL.md:685#"This matters most for review: a misfired review is not a"`). Review-gate consequence, stated
explicitly: a misfired review is not a review and never satisfies the review
gate, since review is never skipped (`SKILL.md:686#"review and never satisfies the review gate, since"`). Review-gate
...
a second time" down to just the outcome (`SKILL.md:683#"explicitly constrained respawn produced a"`), since the
...
result, restored_verified`, `SKILL.md:447#"restored_verified:"` and `implementer.md:125#"restored_verified:"`).
A paragraph immediately after `SKILL.md`'s contract block
(`SKILL.md:476#"reported'."`) and a matching bullet in the installed prompt
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:448#"replayed: false | true"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:126#"replayed: false | true"`),
...
mistaken for it
(`test/docs-consistency.test.ts:4358#"outputContractBlock).not.toContain"`). A further pin locks the five
`mutation_probes` sub-fields to their fixed order in both copies
(`test/docs-consistency.test.ts:4361#"both copies' mutation_probes block has exactly the five sub-fields in a fixed order"`).
...
(`SKILL.md:530#"Review-round escalation budget's trigger."`). The
installed `reviewer.md:49#"classify each finding as"`
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:289#"trigger (see below) without re-deriving it by hand."`).
...
copies identically (`packages/orchestrator-workflow/assets/skill/SKILL.md:455#"commits:"`
and `packages/orchestrator-workflow/assets/agents/implementer.md:101#"role: implementer"`,
...
the task branch, in order (`SKILL.md:487#"produced on the task branch, in the order produced"`
and, worded identically in substance in the installed prompt's rule bullet,
`implementer.md:67#"Report the full sha of every commit you produced"`, with
"in order" carried at `implementer.md:68#"order, in the"`).
...
"forgot to report" (`SKILL.md:489#"omitting the field, so 'did not commit' is"`);
...
evidence" (`SKILL.md:664#"task assignment asked for a commit. When a subagent returns"`
and `implementer.md:69#"assignment asked for a commit is treated as a misfire, not"`).
