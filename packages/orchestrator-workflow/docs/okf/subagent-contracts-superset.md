...
(`packages/orchestrator-workflow/assets/agents/reviewer.md:132#"A reviewer recommendation is not orchestrator acceptance"`). The generated
...
SKILL.md")` block (`test/docs-consistency.test.ts:2776#"explorer, task-slicer, implementer, reviewer, advisor"`) only pinned
...
(`test/docs-consistency.test.ts:2804#"expect(skillBlock).toBe(advisorBlock);"`) extracts the yaml block from
...
requirement (`packages/orchestrator-workflow/test/docs-consistency.test.ts:4856#"requires reference sites to be annotated in the existing task fields"`).
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:4956#"On any round after the task's first, the briefing also names"` pins step 6's
instruction, `test/docs-consistency.test.ts:4968#"or which can no longer be applied (reason:"`
pins the regression-signal consequence, and the implementer prompt's
matching rules
(`test/docs-consistency.test.ts:4974#"On any round after the task's first, the assignment also names"`,
`test/docs-consistency.test.ts:4986#"resolve it before the next reviewer spawn"`). A byte-for-byte
cross-copy equality check on the `mutation_probes` block including the new
sub-field (`test/docs-consistency.test.ts:5026#"replayed: false | true"`), the step 7 reviewer-briefing
sentence (`test/docs-consistency.test.ts:5032#"the orchestrator's reviewer briefing names the replayed probes"`), and a
...
mistaken for it
(`test/docs-consistency.test.ts:5073#"outputContractBlock).not.toContain"`). A further pin locks the eleven
...
in `summary`
(`packages/orchestrator-workflow/assets/agents/implementer.md:90#"On any round after the task's first, when the round fixes a review"`).
...
(`packages/orchestrator-workflow/test/docs-consistency.test.ts:1414#"built from one shared constant"`).
...
(`packages/orchestrator-workflow/CHANGELOG.md:#[Unreleased]`).
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:1852#"both copies carry the method_applied field with the three-method enum"`
pins the field in both copies,
`test/docs-consistency.test.ts:1858#"both copies carry the withdrawn field with its description/reason sub-fields"`
pins the `withdrawn` sub-fields, a byte-for-byte cross-copy equality
check on the combined block
(`test/docs-consistency.test.ts:1864#"the method_applied/withdrawn block is byte-for-byte identical between SKILL.md and reviewer.md"`),
the reviewer.md unnamed-method default
(`test/docs-consistency.test.ts:1876#"reviewer.md states rigorous as the default when the briefing names no method"`),
and the detailed workflow reference selection-rule sentence including the
never-`adversarial`-on-`-medium` constraint
(`test/docs-consistency.test.ts:1881#"detailed workflow reference states the review-method selection rule by risk class, including the never-adversarial-on-medium constraint"`).
