...
timestamp: 2026-09-05T06:21:55Z
sources:
...
- Explorer: `packages/orchestrator-workflow/assets/skill/SKILL.md:266-287#"risk: low | medium | high"`
  (`## Explorer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/explorer.md:48-70#"recommendation:"`.
- Implementer: `packages/orchestrator-workflow/assets/skill/SKILL.md:314-342#"commits:"`
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/SKILL.md:359-382#"matches_implementer_claim: matched | mismatched |"`
...
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/SKILL.md:395-424#"- T-001"`
...
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/SKILL.md:434-452#"would_change_recommendation_if:"`
...
- Subagent input contract (the shape the orchestrator sends when delegating,
  ...
  `packages/orchestrator-workflow/assets/skill/SKILL.md:293-311#"format: structured"`; there is no
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:395-424#"- T-001"`): `id, title,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:293-311#"format: structured"`) requires:
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:429-432#"inventing new field values."`: "The
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:508-518#"with it. Treat a misfire as a failed spawn: resume or"` (`## Subagent
...
1. Contract-parse failure: the output does not parse against the role's
   contract (`SKILL.md:510-511#"against its role's output contract, including an"`). Since a same-day R2 fix-round on 0.16.0
   ...
   task assignment asked for a commit (`SKILL.md:513-514#"task assignment asked for a commit. When a subagent returns"`);
   see [Commits field](#commits-field) below for the field itself.
2. Near-instant return with no tool activity (`SKILL.md:515#"with no tool activity, treat that as a misfire signal rather than"`). This is a
   ...
   was answerable from the context supplied with it (`SKILL.md:516-518#"with it. Treat a misfire as a failed spawn: resume or"`).
...
completed step (`SKILL.md:519-521#"completed step. For the near-instant, no-tool-activity"`). Since 0.18.0, for the near-instant,
...
(`SKILL.md:521-524#"since resume keeps the subagent's prior turn in context"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`SKILL.md:528-529#"the resume attempt itself misfires the same way. This"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`SKILL.md:525-528#"resolved on the first resume attempt; fall back to a"`) — a same-day review-fix round (0.18.0)
...
constrained respawn instead (`SKILL.md:529-534#"watchdog stall as outside this preference. Record every"`). Record every misfire in
`03-decisions.md` (`SKILL.md:534-535#"This matters most for review: a misfired review is not a"`). Review-gate consequence, stated
explicitly: a misfired review is not a review and never satisfies the review
gate, since review is never skipped (`SKILL.md:535-536#"review and never satisfies the review gate, since"`). Review-gate
...
a second time" down to just the outcome (`SKILL.md:531-533#"explicitly constrained respawn produced a"`), since the
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:214-221#"lint): only claims that could vary run to run trigger"` (step 7,
...
`SKILL.md:378-382#"matches_implementer_claim: matched | mismatched |"` and `reviewer.md:86-89#"residual_risks:"`); `matches_implementer_claim`
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:169-188#"for real, observe the named test fail, restore,"` (step 6, added
...
claim-only-what-was-measured rule (`SKILL.md:188-191#"unverified. Record meaningful decisions in"`): treat a verification
...
result, restored_verified`, `SKILL.md:331-335#"restored_verified:"` and `implementer.md:60-64#"restored_verified:"`).
A paragraph immediately after `SKILL.md`'s contract block
(`SKILL.md:346-351#"reported'."`) and a matching bullet in the installed prompt
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:372#"recurrence: new | repeated"` and
...
(`SKILL.md:392-393#"Review-round escalation budget's trigger."`). The
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:223-226#"trigger (see below) without re-deriving it by hand."`).
...
copies identically (`packages/orchestrator-workflow/assets/skill/SKILL.md:342#"commits:"`
...
the task branch, in order (`SKILL.md:354#"produced on the task branch, in the order produced"`
...
"forgot to report" (`SKILL.md:355-356#"omitting the field, so 'did not commit' is"`);
...
evidence" (`SKILL.md:514#"task assignment asked for a commit. When a subagent returns"`
