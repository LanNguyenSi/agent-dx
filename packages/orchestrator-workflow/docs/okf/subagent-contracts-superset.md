...
timestamp: 2026-09-05T21:53:31Z
sources:
...
which `packages/orchestrator-workflow/test/docs-consistency.test.ts:653#"out of this kit's scope"`
...
- Explorer: `packages/orchestrator-workflow/assets/skill/SKILL.md:362#"risk: low | medium | high"`
  (`## Explorer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/explorer.md:48-70#"recommendation:"`.
- Implementer: `packages/orchestrator-workflow/assets/skill/SKILL.md:425#"commits:"`
  vs. `packages/orchestrator-workflow/assets/agents/implementer.md:61#"commits:"`.
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/SKILL.md:473#"matches_implementer_claim: matched | mismatched |"`
  vs. `packages/orchestrator-workflow/assets/agents/reviewer.md:127#"reproduction:"`. Both
...
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/SKILL.md:522#"- T-001"`
  (`## Task slicer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/task-slicer.md:82#"open_questions:"`.
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/SKILL.md:552#"would_change_recommendation_if:"`
...
- Subagent input contract (the shape the orchestrator sends when delegating,
  ...
  `packages/orchestrator-workflow/assets/skill/SKILL.md:393#"format: structured"`; there is no
...
(`packages/orchestrator-workflow/test/docs-consistency.test.ts:960#"expect(skillBlock).toBe(reviewerBlock);"`); the
implementer pair gained a byte-for-byte `mutation_probes` field equality
test in 0.16.0
(`packages/orchestrator-workflow/test/docs-consistency.test.ts:1029#"expect(skillBlock).toBe(implementerBlock);"`),
reinforced by an exact-sub-field-name pin added in a same-day R2 fix-round
(`test/docs-consistency.test.ts:1076#"expect(implementerMd).toContain(field);"`) that catches a mutant the plain
...
SKILL.md")` block (`test/docs-consistency.test.ts:2419#"explorer, task-slicer, implementer, reviewer, advisor"`) only pinned
...
(`test/docs-consistency.test.ts:2447#"expect(skillBlock).toBe(advisorBlock);"`) extracts the yaml block from
...
compares evidence against the same baseline identity. This added shape applies
only after explicit adoption, so an old run is neither migrated nor rejected
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:522#"- T-001"`): `id, title,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:393#"format: structured"`) requires:
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:530#"rather than inventing new field"`: "The
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:179#"task will not edit."`;
`packages/orchestrator-workflow/assets/agents/task-slicer.md:40#"will not edit."`).
...
requirement (`packages/orchestrator-workflow/test/docs-consistency.test.ts:4122#"requires reference sites to be annotated in the existing task fields"`).

The load-bearing check enforces this, and its assertion pins the wording
(`packages/orchestrator-workflow/test/docs-consistency.test.ts:891#"not implementation instructions"`).
The check derives the *required* field set from
the live subagent-input yaml block itself rather than hardcoding it
(`test/docs-consistency.test.ts:780#"^ {4}${field}:"`): it regex-extracts top-level fields
...
(`test/docs-consistency.test.ts:794#"expect(slicerFields).toEqual(skillFields);"`); the original field order
(`id, title, goal, relevant_files, ... dependencies, risk`) survives around
the newer fields (`test/docs-consistency.test.ts:821#"cursor = idx;"`); `02-tasks.md`'s
sections map 1:1 to the fields in order
(`test/docs-consistency.test.ts:821#"cursor = idx;"`); and `task-slicer.md` must frame
`allowed_changes`/`forbidden_changes` as scope boundaries for the
implementer, not implementation instructions
(`test/docs-consistency.test.ts:888#"not implementation instructions"`, prompt text at
`packages/orchestrator-workflow/assets/agents/task-slicer.md:36#"and must not touch — not implementation instructions."`).

## Subagent misfire rule (0.11.0, evidence relocated 0.24.0)

`packages/orchestrator-workflow/assets/skill/SKILL.md:618#"with it. Treat a misfire as a failed spawn: resume or"` (`## Subagent
...
1. Contract-parse failure: the output does not parse against the role's
   contract (`SKILL.md:611#"against its role's output contract, including an"`). Since a same-day R2 fix-round on 0.16.0
   ...
   task assignment asked for a commit (`SKILL.md:614#"task assignment asked for a commit. When a subagent returns"`);
   see [Commits field](#commits-field) below for the field itself.
2. Near-instant return with no tool activity (`SKILL.md:615#"with no tool activity, treat that as a misfire signal rather than"`). This is a
   ...
   was answerable from the context supplied with it (`SKILL.md:618#"with it. Treat a misfire as a failed spawn: resume or"`).
...
completed step (`SKILL.md:621#"completed step. For the near-instant, no-tool-activity"`). Since 0.18.0, for the near-instant,
...
(`SKILL.md:624#"since resume keeps the subagent's prior turn in context"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`SKILL.md:629#"the resume attempt itself misfires the same way. This"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`SKILL.md:628#"resolved on the first resume attempt; fall back to a"`) — a same-day review-fix round (0.18.0)
...
constrained respawn instead (`SKILL.md:634#"watchdog stall as outside this preference. Record every"`). Record every misfire in
`03-decisions.md` (`SKILL.md:635#"This matters most for review: a misfired review is not a"`). Review-gate consequence, stated
explicitly: a misfired review is not a review and never satisfies the review
gate, since review is never skipped (`SKILL.md:636#"review and never satisfies the review gate, since"`). Review-gate
...
a second time" down to just the outcome (`SKILL.md:633#"explicitly constrained respawn produced a"`), since the
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:534#"never satisfies the review gate"` pins
the 0.11.0 rule clause-by-clause: section heading present, packages/orchestrator-workflow/test/docs-consistency.test.ts:504#"## Subagent misfire rule", both
detection signals named verbatim, packages/orchestrator-workflow/test/docs-consistency.test.ts:511#"returns near-instantly with no tool activity", the scoping language that
prevents false-positive misfires, packages/orchestrator-workflow/test/docs-consistency.test.ts:517#"only if it is contract-valid and the assignment was answerable from the context supplied with it", the resume-or-respawn response
plus the non-evidence rule, packages/orchestrator-workflow/test/docs-consistency.test.ts:524#"never fold the non-contract output into run state or count it as a completed step", the `03-decisions.md` record
requirement, packages/orchestrator-workflow/test/docs-consistency.test.ts:529#"Record every misfire in", and the review-gate consequence sentence, packages/orchestrator-workflow/test/docs-consistency.test.ts:534#"never satisfies the review gate".
`test/docs-consistency.test.ts:613#"did not resolve on resume; only a fresh, explicitly"` pins the 0.18.0 extension, this
fix-round's hardening, and the 0.24.0 evidence removal in one `describe`
block: the resume-over-respawn preference, test/docs-consistency.test.ts:559#"For the near-instant, no-tool-activity signal specifically, prefer resume over a fresh respawn", the
repeat-the-assignment mechanic, test/docs-consistency.test.ts:565#"send the same subagent a message that explicitly repeats the original assignment rather than a generic retry", why resume beats a fresh respawn,
test/docs-consistency.test.ts:571#"resume keeps the subagent's prior turn in context while a fresh spawn starts cold and risks the same misfire again", the parenthetical signal definition, test/docs-consistency.test.ts:577#"(a return within seconds, zero tool calls, harness or system boilerplate instead of the output contract)", the claim-binding
to recorded outcomes, test/docs-consistency.test.ts:583#"whose outcome was recorded has resolved on the first resume attempt", the conditional respawn fallback, test/docs-consistency.test.ts:589#"fall back to a fresh respawn only if the resume attempt itself misfires the same way",
a negative-pin test that the incident tally and the model-correlation
passage no longer appear (test/docs-consistency.test.ts:599#"see the per-role model preferences", replacing the review-round-1 positive
pins on that passage the 0.24.0 pass removed along with the prose), the
watchdog scope carve-out, test/docs-consistency.test.ts:607#"treat a watchdog stall as outside this preference", and its own resolution detail,
test/docs-consistency.test.ts:613#"did not resolve on resume; only a fresh, explicitly constrained respawn produced a contract-valid review".

## Reproduction requirement (0.14.0)

`packages/orchestrator-workflow/assets/skill/SKILL.md:270#"lint): only claims that could vary run to run trigger"` (step 7,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:271#"GitHub Actions shell replay named in step 6 is a second, explicitly"`;
`packages/orchestrator-workflow/assets/agents/reviewer.md:100#"shell replay above is a second, explicitly non-probabilistic trigger for"`). The
installed `packages/orchestrator-workflow/assets/agents/reviewer.md:99#"lint) do not trigger this."`
...
`SKILL.md:473#"matches_implementer_claim: matched | mismatched |"` and `reviewer.md:125#"residual_risks:"`); `matches_implementer_claim`
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:209#"for real, observe the named test fail, restore,"` (step 6, added
...
claim-only-what-was-measured rule (`SKILL.md:212#"unverified. On any round after the task's first"`): treat a verification
...
result, restored_verified`, `SKILL.md:417#"restored_verified:"` and `implementer.md:96#"restored_verified:"`).
A paragraph immediately after `SKILL.md`'s contract block
(`SKILL.md:434#"reported'."`) and a matching bullet in the installed prompt
(`implementer.md:31#"rather than omitting the field."`) both state the not-applicable signal added in the
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:1029#"expect(skillBlock).toBe(implementerBlock);"` pins
the original 0.16.0 shape: the installed prompt's instruction and field
mention, packages/orchestrator-workflow/test/docs-consistency.test.ts:1004#"an output missing that field when probes were named is treated as a misfire, not evidence", the claim-only-what-was-measured rule, packages/orchestrator-workflow/test/docs-consistency.test.ts:1012#"never claim a run you did not execute", the
misfire-rule sentence, packages/orchestrator-workflow/test/docs-consistency.test.ts:1017#"does not parse against its role's output contract, including an implementer return that omits the", and a byte-for-byte cross-copy equality
check on the field block, packages/orchestrator-workflow/test/docs-consistency.test.ts:1029#"expect(skillBlock).toBe(implementerBlock);".
`test/docs-consistency.test.ts:1083#"expect(implementerMd).toContain(enumeration);"` pins the R2 additions: step 6's
sentence and its claim-only-what-was-measured reference, test/docs-consistency.test.ts:1058#"apply the mutant for real, observe the named test fail, restore, re-verify" (the `it` block that follows it pins the claim-only-what-was-measured reference; the rule name recurs too often in that file to anchor a citation on it), the
not-applicable clause in both copies, test/docs-consistency.test.ts:1069#"expect(implementerMd).toContain(clause);", and two exact-string pins,
test/docs-consistency.test.ts:1076#"expect(implementerMd).toContain(field);", that catch a rename applied identically to both copies — a mutant
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:221#"Record meaningful decisions in"`,
step 6). The installed prompt carries the same rule as its own bullet
(`packages/orchestrator-workflow/assets/agents/implementer.md:38#"four evidence fields plus"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:218#"A replayed probe whose mutant now survives or can no"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:39#"A replayed probe whose mutant now survives or can no"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:418#"replayed: false | true"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:97#"replayed: false | true"`),
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:285#"the replayed probes the implementer reports as killed"`);
`assets/agents/reviewer.md` itself is untouched by this change.

`packages/orchestrator-workflow/test/docs-consistency.test.ts:4221#"On any round after the task's first, the briefing also names"` pins step 6's
instruction, `test/docs-consistency.test.ts:4230#"A replayed probe whose mutant now survives or can no longer be applied is a regression signal"`
pins the regression-signal consequence, and the implementer prompt's
matching rules
(`test/docs-consistency.test.ts:4236#"On any round after the task's first, the assignment also names"`,
`test/docs-consistency.test.ts:4230#"A replayed probe whose mutant now survives or can no longer be applied is a regression signal"`). A byte-for-byte
cross-copy equality check on the `mutation_probes` block including the new
sub-field (`test/docs-consistency.test.ts:4284#"replayed: false | true"`), the step 7 reviewer-briefing
sentence (`test/docs-consistency.test.ts:4290#"the orchestrator's reviewer briefing names the replayed probes"`), and a
negative pin scoped to `reviewer.md`'s output-contract yaml block, that it
gains no `replayed` field
(`test/docs-consistency.test.ts:4299#"outputContractBlock).not.toContain"`). A further pin locks the five
`mutation_probes` sub-fields to their fixed order in both copies
(`test/docs-consistency.test.ts:4302#"both copies' mutation_probes block has exactly the five sub-fields in a fixed order"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:463#"recurrence: new | repeated"` and
`packages/orchestrator-workflow/assets/agents/reviewer.md:121#"recurrence: new | repeated"`, same field, same
...
(`SKILL.md:484#"Review-round escalation budget's trigger."`). The
installed `reviewer.md:39#"classify each finding as"`
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:283#"trigger (see below) without re-deriving it by hand."`).
...
copies identically (`packages/orchestrator-workflow/assets/skill/SKILL.md:425#"commits:"`
and `packages/orchestrator-workflow/assets/agents/implementer.md:61#"commits:"`,
...
the task branch, in order (`SKILL.md:445#"produced on the task branch, in the order produced"`
and, worded identically in substance in the installed prompt's rule bullet,
`implementer.md:58#"Report the full sha of every commit you produced"`, with
"in order" carried at `implementer.md:59#"order, in the"`).
...
"forgot to report" (`SKILL.md:447#"omitting the field, so 'did not commit' is"`);
...
evidence" (`SKILL.md:614#"task assignment asked for a commit. When a subagent returns"`
and `implementer.md:60#"assignment asked for a commit is treated as a misfire, not"`).

`packages/orchestrator-workflow/test/docs-consistency.test.ts:1143#"expect(implementerMd).toContain(clause);"`
pins the field: the installed prompt's full-sha instruction and field
mention, packages/orchestrator-workflow/test/docs-consistency.test.ts:1109#"an output missing that field when the task assignment asked for a commit is treated as a misfire, not evidence", the misfire-rule sentence, packages/orchestrator-workflow/test/docs-consistency.test.ts:1115#"field even though the task assignment asked for a commit", a dedicated pin
on the "full sha" / "in order" semantics themselves (not only the
surrounding clauses), packages/orchestrator-workflow/test/docs-consistency.test.ts:1122#"in the order produced", a byte-for-byte cross-copy equality check
on the field block, packages/orchestrator-workflow/test/docs-consistency.test.ts:1137#"expect(skillBlock).toBe(implementerBlock);", and the not-applicable `commits: []` clause
in both copies, packages/orchestrator-workflow/test/docs-consistency.test.ts:1143#"expect(implementerMd).toContain(clause);".
