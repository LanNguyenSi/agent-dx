...
timestamp: 2026-09-12T06:24:08Z
sources:
...
- Explorer: `packages/orchestrator-workflow/assets/skill/SKILL.md:400#"risk: low | medium | high"`
  (`## Explorer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/explorer.md:48-70#"recommendation:"`.
- Implementer: `packages/orchestrator-workflow/assets/skill/SKILL.md:449#"## Implementer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/implementer.md:133#"role: implementer"`.
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/SKILL.md:541#"## Reviewer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/reviewer.md:151#"role: reviewer"`. Both
...
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/SKILL.md:594#"## Task slicer output contract"`
...
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/SKILL.md:664#"would_change_recommendation_if:"`
...
- Subagent input contract (the shape the orchestrator sends when delegating,
  ...
  `packages/orchestrator-workflow/assets/skill/SKILL.md:418#"## Subagent input contract"`; there is no
...
SKILL.md")` block (`test/docs-consistency.test.ts:2524#"explorer, task-slicer, implementer, reviewer, advisor"`) only pinned
...
(`test/docs-consistency.test.ts:2552#"expect(skillBlock).toBe(advisorBlock);"`) extracts the yaml block from
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:594#"## Task slicer output contract"`): `id, title,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:418#"## Subagent input contract"`) requires
...
rule at `packages/orchestrator-workflow/assets/skill/SKILL.md:640#"rather than inventing new field"`, the
...
requirement (`packages/orchestrator-workflow/test/docs-consistency.test.ts:4597#"requires reference sites to be annotated in the existing task fields"`).
...
`test/acceptance-baseline.test.ts:256-263#"else assertCriteriaShape(block);"`;
`test/acceptance-baseline.test.ts:386-392#"original contract, keep the original checklist semantics"`); and `task-slicer.md` must frame
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:730#"with it. Treat a misfire as a failed spawn: resume or"` (`## Subagent
...
1. Contract-parse failure: the output does not parse against the role's
   contract (`SKILL.md:723#"against its role's output contract, including an"`). Since a same-day R2 fix-round on 0.16.0
   ...
   task assignment asked for a commit (`SKILL.md:726#"task assignment asked for a commit. When a subagent returns"`);
   see [Commits field](#commits-field) below for the field itself.
2. Near-instant return with no tool activity (`SKILL.md:727#"with no tool activity, treat that as a misfire signal rather than"`). This is a
   ...
   was answerable from the context supplied with it (`SKILL.md:730#"with it. Treat a misfire as a failed spawn: resume or"`).
...
completed step (`SKILL.md:733#"completed step. For the near-instant, no-tool-activity"`). Since 0.18.0, for the near-instant,
...
(`SKILL.md:736#"since resume keeps the subagent's prior turn in context"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`SKILL.md:741#"the resume attempt itself misfires the same way. This"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`SKILL.md:740#"resolved on the first resume attempt; fall back to a"`); a same-day review-fix round (0.18.0)
...
constrained respawn instead (`SKILL.md:746#"watchdog stall as outside this preference. Record every"`). Record every misfire in
`03-decisions.md` (`SKILL.md:747#"This matters most for review: a misfired review is not a"`). Review-gate consequence, stated
explicitly: a misfired review is not a review and never satisfies the review
gate, since review is never skipped (`SKILL.md:748#"review and never satisfies the review gate, since"`). Review-gate
...
a second time" down to just the outcome (`SKILL.md:745#"explicitly constrained respawn produced a"`), since the
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:298#"lint): only claims that could vary run to run trigger"` (step 7,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:299#"Actions shell replay named in step 6 is a second, explicitly"`;
...
`SKILL.md:568#"matches_implementer_claim: matched | mismatched |"` and `reviewer.md:164#"residual_risks:"`); `matches_implementer_claim`
...
claim-only-what-was-measured rule (`SKILL.md:213#"not backed by a check it actually ran as"`): treat a verification
...
result, restored_verified`, `SKILL.md:484#"restored_verified:"` and `implementer.md:162#"restored_verified:"`)
at 0.16.0 (later grown to ten sub-fields; see Mutation probe definition
fields and expectation split below).
A paragraph immediately after `SKILL.md`'s contract block
(`SKILL.md:520#"reported'."`) and a matching bullet in the installed prompt
(`implementer.md:51#"rather than omitting the field."`) both state the not-applicable signal added in the
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:232#"Record meaningful decisions in"`,
step 6). The installed prompt carries the same rule as its own bullet
(`packages/orchestrator-workflow/assets/agents/implementer.md:54#"On any round after the task's first, the assignment also names"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:227#"A replayed probe whose"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:63#"A replayed probe whose"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:485#"replayed: false | true"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:163#"replayed: false | true"`),
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:4697#"On any round after the task's first, the briefing also names"` pins step 6's
instruction, `test/docs-consistency.test.ts:4709#"or which can no longer be applied (reason:"`
pins the regression-signal consequence, and the implementer prompt's
matching rules
(`test/docs-consistency.test.ts:4715#"On any round after the task's first, the assignment also names"`,
`test/docs-consistency.test.ts:4727#"resolve it before the next reviewer spawn"`). A byte-for-byte
cross-copy equality check on the `mutation_probes` block including the new
sub-field (`test/docs-consistency.test.ts:4766#"replayed: false | true"`), the step 7 reviewer-briefing
sentence (`test/docs-consistency.test.ts:4772#"the orchestrator's reviewer briefing names the replayed probes"`), and a
...
mistaken for it
(`test/docs-consistency.test.ts:4813#"outputContractBlock).not.toContain"`). A further pin locks the ten
`mutation_probes` sub-fields to their fixed order in both copies (grown
from five at this rule's own introduction to ten under task 06330af2,
see Mutation probe definition fields and expectation split below)
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:558#"recurrence: new | repeated"` and
`packages/orchestrator-workflow/assets/agents/reviewer.md:160#"recurrence: new | repeated"`, same field, same
...
(`SKILL.md:583#"Review-round escalation budget's trigger."`). The
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:314#"trigger (see below) without re-deriving it"`).
...
copies identically (`packages/orchestrator-workflow/assets/skill/SKILL.md:492#"commits:"`
and `packages/orchestrator-workflow/assets/agents/implementer.md:133#"role: implementer"`,
...
the task branch, in order (`SKILL.md:536#"produced on the task branch, in the order produced"`
and, worded identically in substance in the installed prompt's rule bullet,
`implementer.md:99#"Report the full sha of every commit you produced"`, with
"in order" carried at `implementer.md:100#"order, in the"`).
...
"forgot to report" (`SKILL.md:538#"omitting the field, so 'did not commit' is"`);
...
evidence" (`SKILL.md:726#"task assignment asked for a commit. When a subagent returns"`
and `implementer.md:101#"assignment asked for a commit is treated as a misfire, not"`).

`packages/orchestrator-workflow/test/docs-consistency.test.ts:1194#"expect(implementerMd).toContain(clause);"`
pins the field: the installed prompt's full-sha instruction and field
mention, packages/orchestrator-workflow/test/docs-consistency.test.ts:1160#"an output missing that field when the task assignment asked for a commit is treated as a misfire, not evidence", the misfire-rule sentence, packages/orchestrator-workflow/test/docs-consistency.test.ts:1166#"field even though the task assignment asked for a commit", a dedicated pin
on the "full sha" / "in order" semantics themselves (not only the
surrounding clauses), packages/orchestrator-workflow/test/docs-consistency.test.ts:1173#"in the order produced", a byte-for-byte cross-copy equality check
on the field block, packages/orchestrator-workflow/test/docs-consistency.test.ts:1188#"expect(skillBlock).toBe(implementerBlock);", and the not-applicable `commits: []` clause
in both copies, packages/orchestrator-workflow/test/docs-consistency.test.ts:1194#"expect(implementerMd).toContain(clause);".
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:569#"method_applied: normal | rigorous | adversarial"`
and
`packages/orchestrator-workflow/assets/agents/reviewer.md:171#"method_applied: normal | rigorous | adversarial"`,
...
failed break attempts. SKILL.md step 7 states the selection rule by risk
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:274#"never substitutes for it: do not pair"`);
...
and
`packages/orchestrator-workflow/assets/skill/SKILL.md:588#"when nothing was withdrawn. Until a"`).
...
both copies
(`packages/orchestrator-workflow/assets/skill/SKILL.md:591#"matches the briefing's"`).
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:1600#"both copies carry the method_applied field with the three-method enum"`
pins the field in both copies,
`test/docs-consistency.test.ts:1606#"both copies carry the withdrawn field with its description/reason sub-fields"`
pins the `withdrawn` sub-fields, a byte-for-byte cross-copy equality
check on the combined block
(`test/docs-consistency.test.ts:1612#"the method_applied/withdrawn block is byte-for-byte identical between SKILL.md and reviewer.md"`),
the reviewer.md unnamed-method default
(`test/docs-consistency.test.ts:1624#"reviewer.md states rigorous as the default when the briefing names no method"`),
and the SKILL.md step 7 selection-rule sentence including the
never-`adversarial`-on-`-medium` constraint
(`test/docs-consistency.test.ts:1629#"SKILL.md step 7 states the review-method selection rule by risk class, including the never-adversarial-on-medium constraint"`).
