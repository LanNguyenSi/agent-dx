...
- Explorer: `packages/orchestrator-workflow/assets/skill/SKILL.md:270-291#"risk: low | medium | high"`
  (`## Explorer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/explorer.md:48-70#"recommendation:"`.
- Implementer: `packages/orchestrator-workflow/assets/skill/SKILL.md:318-346#"commits:"`
  vs. `packages/orchestrator-workflow/assets/agents/implementer.md:45-71#"commits:"`.
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/SKILL.md:363-386#"matches_implementer_claim: matched | mismatched |"`
...
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/SKILL.md:399-428#"- T-001"`
...
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/SKILL.md:438-456#"would_change_recommendation_if:"`
...
- Subagent input contract (the shape the orchestrator sends when delegating,
  ...
  `packages/orchestrator-workflow/assets/skill/SKILL.md:297-315#"format: structured"`; there is no
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:399-428#"- T-001"`): `id, title,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:297-315#"format: structured"`) requires:
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:433-436#"inventing new field values."`: "The
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:166-169#"task will not edit."`;
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:512-522#"with it. Treat a misfire as a failed spawn: resume or"` (`## Subagent
...
1. Contract-parse failure: the output does not parse against the role's
   contract (`SKILL.md:514-515#"against its role's output contract, including an"`). Since a same-day R2 fix-round on 0.16.0
   ...
   task assignment asked for a commit (`SKILL.md:517-518#"task assignment asked for a commit. When a subagent returns"`);
   see [Commits field](#commits-field) below for the field itself.
2. Near-instant return with no tool activity (`SKILL.md:519#"with no tool activity, treat that as a misfire signal rather than"`). This is a
   ...
   was answerable from the context supplied with it (`SKILL.md:520-522#"with it. Treat a misfire as a failed spawn: resume or"`).
...
completed step (`SKILL.md:523-525#"completed step. For the near-instant, no-tool-activity"`). Since 0.18.0, for the near-instant,
...
(`SKILL.md:525-528#"since resume keeps the subagent's prior turn in context"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`SKILL.md:532-533#"the resume attempt itself misfires the same way. This"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`SKILL.md:529-532#"resolved on the first resume attempt; fall back to a"`) — a same-day review-fix round (0.18.0)
...
constrained respawn instead (`SKILL.md:533-538#"watchdog stall as outside this preference. Record every"`). Record every misfire in
`03-decisions.md` (`SKILL.md:538-539#"This matters most for review: a misfired review is not a"`). Review-gate consequence, stated
explicitly: a misfired review is not a review and never satisfies the review
gate, since review is never skipped (`SKILL.md:539-540#"review and never satisfies the review gate, since"`). Review-gate
...
a second time" down to just the outcome (`SKILL.md:535-537#"explicitly constrained respawn produced a"`), since the
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:218-225#"lint): only claims that could vary run to run trigger"` (step 7,
...
`SKILL.md:382-386#"matches_implementer_claim: matched | mismatched |"` and `reviewer.md:86-89#"residual_risks:"`); `matches_implementer_claim`
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:173-192#"for real, observe the named test fail, restore,"` (step 6, added
...
claim-only-what-was-measured rule (`SKILL.md:192-195#"unverified. Record meaningful decisions in"`): treat a verification
...
result, restored_verified`, `SKILL.md:335-339#"restored_verified:"` and `implementer.md:60-64#"restored_verified:"`).
A paragraph immediately after `SKILL.md`'s contract block
(`SKILL.md:350-355#"reported'."`) and a matching bullet in the installed prompt
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:376#"recurrence: new | repeated"` and
...
(`SKILL.md:396-397#"Review-round escalation budget's trigger."`). The
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:227-230#"trigger (see below) without re-deriving it by hand."`).
...
copies identically (`packages/orchestrator-workflow/assets/skill/SKILL.md:346#"commits:"`
and `packages/orchestrator-workflow/assets/agents/implementer.md:71#"commits:"`,
...
the task branch, in order (`SKILL.md:358#"produced on the task branch, in the order produced"`
and, worded identically in substance in the installed prompt's rule bullet,
`implementer.md:26#"Report the full sha of every commit you produced"`, with
"in order" carried at `implementer.md:27#"order, in the"`).
...
"forgot to report" (`SKILL.md:359-360#"omitting the field, so 'did not commit' is"`);
...
evidence" (`SKILL.md:518#"task assignment asked for a commit. When a subagent returns"`
and `implementer.md:28#"assignment asked for a commit is treated as a misfire, not"`).
