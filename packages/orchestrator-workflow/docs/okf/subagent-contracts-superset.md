...
timestamp: 2026-09-12T05:35:06Z
sources:
...
- Explorer: `packages/orchestrator-workflow/assets/skill/SKILL.md:394#"risk: low | medium | high"`
  (`## Explorer output contract`) vs.
  `packages/orchestrator-workflow/assets/agents/explorer.md:48-70#"recommendation:"`.
- Implementer: `packages/orchestrator-workflow/assets/skill/SKILL.md:443#"## Implementer output contract"`
  vs. `packages/orchestrator-workflow/assets/agents/implementer.md:125#"role: implementer"`.
...
- Reviewer: `packages/orchestrator-workflow/assets/skill/SKILL.md:531#"## Reviewer output contract"`
...
- Task-slicer:
  `packages/orchestrator-workflow/assets/skill/SKILL.md:584#"## Task slicer output contract"`
...
- Advisor (since 0.21.0):
  `packages/orchestrator-workflow/assets/skill/SKILL.md:654#"would_change_recommendation_if:"`
...
- Subagent input contract (the shape the orchestrator sends when delegating,
  ...
  `packages/orchestrator-workflow/assets/skill/SKILL.md:412#"## Subagent input contract"`; there is no
...
SKILL.md")` block (`test/docs-consistency.test.ts:2507#"explorer, task-slicer, implementer, reviewer, advisor"`) only pinned
...
(`test/docs-consistency.test.ts:2535#"expect(skillBlock).toBe(advisorBlock);"`) extracts the yaml block from
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:584#"## Task slicer output contract"`): `id, title,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:412#"## Subagent input contract"`) requires
...
rule at `packages/orchestrator-workflow/assets/skill/SKILL.md:630#"rather than inventing new field"`, the
...
requirement (`packages/orchestrator-workflow/test/docs-consistency.test.ts:4579#"requires reference sites to be annotated in the existing task fields"`).
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:720#"with it. Treat a misfire as a failed spawn: resume or"` (`## Subagent
...
1. Contract-parse failure: the output does not parse against the role's
   contract (`SKILL.md:713#"against its role's output contract, including an"`). Since a same-day R2 fix-round on 0.16.0
   ...
   task assignment asked for a commit (`SKILL.md:716#"task assignment asked for a commit. When a subagent returns"`);
   see [Commits field](#commits-field) below for the field itself.
2. Near-instant return with no tool activity (`SKILL.md:717#"with no tool activity, treat that as a misfire signal rather than"`). This is a
   ...
   was answerable from the context supplied with it (`SKILL.md:720#"with it. Treat a misfire as a failed spawn: resume or"`).
...
completed step (`SKILL.md:723#"completed step. For the near-instant, no-tool-activity"`). Since 0.18.0, for the near-instant,
...
(`SKILL.md:726#"since resume keeps the subagent's prior turn in context"`); fall back to a fresh respawn only if the resume
attempt itself misfires the same way (`SKILL.md:731#"the resume attempt itself misfires the same way. This"`). Every incident of
this exact signal whose outcome was recorded has resolved on the first
resume attempt (`SKILL.md:730#"resolved on the first resume attempt; fall back to a"`); a same-day review-fix round (0.18.0)
...
constrained respawn instead (`SKILL.md:736#"watchdog stall as outside this preference. Record every"`). Record every misfire in
`03-decisions.md` (`SKILL.md:737#"This matters most for review: a misfired review is not a"`). Review-gate consequence, stated
explicitly: a misfired review is not a review and never satisfies the review
gate, since review is never skipped (`SKILL.md:738#"review and never satisfies the review gate, since"`). Review-gate
...
a second time" down to just the outcome (`SKILL.md:735#"explicitly constrained respawn produced a"`), since the
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:292#"lint): only claims that could vary run to run trigger"` (step 7,
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:293#"Actions shell replay named in step 6 is a second, explicitly"`;
...
`SKILL.md:558#"matches_implementer_claim: matched | mismatched |"` and `reviewer.md:164#"residual_risks:"`); `matches_implementer_claim`
...
result, restored_verified`, `SKILL.md:478#"restored_verified:"` and `implementer.md:154#"restored_verified:"`).
A paragraph immediately after `SKILL.md`'s contract block
(`SKILL.md:511#"reported'."`) and a matching bullet in the installed prompt
(`implementer.md:49#"rather than omitting the field."`) both state the not-applicable signal added in the
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:231#"Record meaningful decisions in"`,
step 6). The installed prompt carries the same rule as its own bullet
(`packages/orchestrator-workflow/assets/agents/implementer.md:51#"On any round after the task's first, the assignment also names"`).
...
and
`packages/orchestrator-workflow/assets/agents/implementer.md:60#"A replayed probe whose"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:479#"replayed: false | true"`
and
`packages/orchestrator-workflow/assets/agents/implementer.md:155#"replayed: false | true"`),
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:311#"reports as killed together with their"`);
`assets/agents/reviewer.md` itself is untouched by this change.

`packages/orchestrator-workflow/test/docs-consistency.test.ts:4679#"On any round after the task's first, the briefing also names"` pins step 6's
instruction, `test/docs-consistency.test.ts:4691#"or which can no longer be applied, is the regression signal"`
pins the regression-signal consequence, and the implementer prompt's
matching rules
(`test/docs-consistency.test.ts:4697#"On any round after the task's first, the assignment also names"`,
`test/docs-consistency.test.ts:4709#"resolve it before the next reviewer spawn"`). A byte-for-byte
cross-copy equality check on the `mutation_probes` block including the new
sub-field (`test/docs-consistency.test.ts:4748#"replayed: false | true"`), the step 7 reviewer-briefing
sentence (`test/docs-consistency.test.ts:4754#"the orchestrator's reviewer briefing names the replayed probes"`), and a
...
mistaken for it
(`test/docs-consistency.test.ts:4795#"outputContractBlock).not.toContain"`). A further pin locks the five
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:548#"recurrence: new | repeated"` and
...
(`SKILL.md:573#"Review-round escalation budget's trigger."`). The
...
`packages/orchestrator-workflow/assets/skill/SKILL.md:308#"trigger (see below) without re-deriving it"`).
...
copies identically (`packages/orchestrator-workflow/assets/skill/SKILL.md:486#"commits:"`
and `packages/orchestrator-workflow/assets/agents/implementer.md:125#"role: implementer"`,
...
the task branch, in order (`SKILL.md:526#"produced on the task branch, in the order produced"`
and, worded identically in substance in the installed prompt's rule bullet,
`implementer.md:91#"Report the full sha of every commit you produced"`, with
"in order" carried at `implementer.md:92#"order, in the"`).
...
"forgot to report" (`SKILL.md:528#"omitting the field, so 'did not commit' is"`);
...
evidence" (`SKILL.md:716#"task assignment asked for a commit. When a subagent returns"`
and `implementer.md:93#"assignment asked for a commit is treated as a misfire, not"`).
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:559#"method_applied: normal | rigorous | adversarial"`
and
...
(`packages/orchestrator-workflow/assets/skill/SKILL.md:268#"never substitutes for it: do not pair"`);
...
and
`packages/orchestrator-workflow/assets/skill/SKILL.md:578#"when nothing was withdrawn. Until a"`).
...
both copies
(`packages/orchestrator-workflow/assets/skill/SKILL.md:581#"matches the briefing's"`).
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:1583#"both copies carry the method_applied field with the three-method enum"`
pins the field in both copies,
`test/docs-consistency.test.ts:1589#"both copies carry the withdrawn field with its description/reason sub-fields"`
pins the `withdrawn` sub-fields, a byte-for-byte cross-copy equality
check on the combined block
(`test/docs-consistency.test.ts:1595#"the method_applied/withdrawn block is byte-for-byte identical between SKILL.md and reviewer.md"`),
the reviewer.md unnamed-method default
(`test/docs-consistency.test.ts:1607#"reviewer.md states rigorous as the default when the briefing names no method"`),
