...
timestamp: 2026-09-25T07:56:21Z
sources:
...
(`packages/orchestrator-workflow/assets/agents/reviewer.md:158#"A reviewer recommendation is not orchestrator acceptance"`). This is a
...
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:221-223#"instead of inferring one from the findings list."`; the installed `reviewer.md:177#"never leave it blank or omit it."` prompt carries the mirrored
...
- Critical: "waived by the operator. The orchestrator never waives a
  ...
  (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:308#"explicitly waived: critical findings require operator"`).
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:117#"rationale."`; `packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:309#"findings require the orchestrator to record a"`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:310#"or critical finding counts as a waiver and follows the"`). A deferred
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:119#"the Accepted Waivers section of"`; `packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:312#"the Accepted Waivers section of"`), instructed to "Mirror
  03-decisions.md"
...
other ineligible finding (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:321#"high/critical or other ineligible finding."`;
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:323#"unchanged and setting Decision to"`; `05-review-findings.md:16#"| Severity | Category | Description | Suggested Fix | Decision |"`).
...
orchestrator-workflow completeness reader verifies" (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:237#"those two are what the orchestrator-workflow completeness reader verifies; for every"`).
Immediately after that quote, detailed workflow reference also carries a 0.13.0 addition
on the same table's placeholder/legend row (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:241#"rows as the template never having been filled in. When"`): replace it
...
reproduction requirement (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:248#"lint): only claims that could vary run to run trigger"`); full treatment is out of scope
...
chosen enum value when finalizing each file (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:357#"non-accepting (fail-closed)."`). Left as
...
the placeholder-row rule (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:248#"lint): only claims that could vary run to run trigger"`): when acceptance rests on such
...
same rule (`reviewer.md:227#"lint) do not trigger this."`), and both output contracts gained a matching
`reproduction: {method, sample_size, result, matches_implementer_claim}`
field (`packages/orchestrator-workflow/assets/skill/references/contracts.md:215#"matches_implementer_claim: matched | mismatched |"`, `reviewer.md:301#"residual_risks:"`); `matches_implementer_claim`
...
findings, and the installed `reviewer.md:177#"never leave it blank or omit it."` prompt carries the mirrored
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:329#"record the Fix-regression decision point"`).
...
Baseline revisions (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:155#"Record a baseline revision only when"`):
...
Docs-only review default (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:226#"For a review round whose entire delta contains only explanatory documentation"`):
