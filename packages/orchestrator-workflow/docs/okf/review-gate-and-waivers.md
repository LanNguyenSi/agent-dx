...
(`SKILL.md:372#"severity: low | medium | high | critical"`, reviewer output contract). Only high and critical block
...
reject` (`SKILL.md:377#"acceptance_recommendation: accept | accept_with_notes |"`; mirrored in the findings template's Acceptance
...
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:100-101#"rationale."`; `SKILL.md:233-234#"findings require the orchestrator to record a"`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`SKILL.md:234-235#"or critical finding counts as a waiver and follows the"`). A deferred
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:102-103#"the Accepted Waivers section of"`; `SKILL.md:236-237#"the Accepted Waivers section of"`), instructed to "Mirror
  03-decisions.md"
...
orchestrator-workflow completeness reader verifies" (`SKILL.md:211-214#"orchestrator-workflow completeness reader verifies."`).
Immediately after that quote, SKILL.md step 7 also carries a 0.13.0 addition
on the same table's placeholder/legend row (`SKILL.md:214-218#"rows as the template never having been filled in. When"`): replace it
...
reproduction requirement (`SKILL.md:218-225#"lint): only claims that could vary run to run trigger"`); full treatment is out of scope
...
chosen enum value when finalizing each file (`SKILL.md:264-268#"non-accepting (fail-closed)."`). Left as
...
(`SKILL.md:539-540#"review and never satisfies the review gate, since"`), pinned by
...
the placeholder-row rule (`SKILL.md:218-225#"lint): only claims that could vary run to run trigger"`): when acceptance rests on such
...
field (`SKILL.md:382-386#"matches_implementer_claim: matched | mismatched |"`, `reviewer.md:86-89#"residual_risks:"`); `matches_implementer_claim`
...
`SKILL.md:389-391#"instead of inferring one from the findings list."` states it and adds the orchestrator's response when it is
...
`SKILL.md:557#"## Review-round escalation budget"` triggers "by the second
...
(`SKILL.md:572#"**Tier or model escalation**"`,
`SKILL.md:578#"**Advisor spawn**"`, `SKILL.md:581#"**Merge-hold**"`: raise
...
(`SKILL.md:584#"Judgment governs which of the three to pick; only that one is chosen and"`).
