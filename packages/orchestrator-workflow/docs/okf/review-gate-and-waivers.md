...
(`SKILL.md:381#"severity: low | medium | high | critical"`, reviewer output contract). Only high and critical block
...
reject` (`SKILL.md:386#"acceptance_recommendation: accept | accept_with_notes |"`; mirrored in the findings template's Acceptance
...
- Critical: "waived by the operator. The orchestrator never waives a
  ...
  (`SKILL.md:233#"explicitly waived: critical findings require operator"`).
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:101#"rationale."`; `SKILL.md:234#"findings require the orchestrator to record a"`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`SKILL.md:235#"or critical finding counts as a waiver and follows the"`). A deferred
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:103#"the Accepted Waivers section of"`; `SKILL.md:237#"the Accepted Waivers section of"`), instructed to "Mirror
  03-decisions.md"
...
other ineligible finding (`SKILL.md:244#"high/critical or other ineligible finding."`;
...
(`SKILL.md:246#"unchanged and setting Decision to"`; `05-review-findings.md:9-11#"| Severity | Category | Description | Suggested Fix | Decision |"`).
...
orchestrator-workflow completeness reader verifies" (`SKILL.md:214#"orchestrator-workflow completeness reader verifies."`).
Immediately after that quote, SKILL.md step 7 also carries a 0.13.0 addition
on the same table's placeholder/legend row (`SKILL.md:218#"rows as the template never having been filled in. When"`): replace it
...
reproduction requirement (`SKILL.md:225#"lint): only claims that could vary run to run trigger"`); full treatment is out of scope
...
chosen enum value when finalizing each file (`SKILL.md:277#"non-accepting (fail-closed)."`). Left as
...
(`SKILL.md:549#"review and never satisfies the review gate, since"`), pinned by
...
the placeholder-row rule (`SKILL.md:225#"lint): only claims that could vary run to run trigger"`): when acceptance rests on such
...
field (`SKILL.md:395#"matches_implementer_claim: matched | mismatched |"`, `reviewer.md:86-89#"residual_risks:"`); `matches_implementer_claim`
...
`SKILL.md:400#"instead of inferring one from the findings list."` states it and adds the orchestrator's response when it is
...
`SKILL.md:566#"## Review-round escalation budget"` triggers "by the second
...
(`SKILL.md:581#"**Tier or model escalation**"`,
`SKILL.md:587#"**Advisor spawn**"`, `SKILL.md:590#"**Merge-hold**"`: raise
...
(`SKILL.md:593#"Judgment governs which of the three to pick; only that one is chosen and"`).
