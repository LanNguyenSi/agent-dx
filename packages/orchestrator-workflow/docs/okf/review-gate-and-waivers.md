...
- Critical: "waived by the operator. The orchestrator never waives a
  ...
  (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:307#"explicitly waived: critical findings require operator"`).
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:117#"rationale."`; `packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:308#"findings require the orchestrator to record a"`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:309#"or critical finding counts as a waiver and follows the"`). A deferred
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:119#"the Accepted Waivers section of"`; `packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:311#"the Accepted Waivers section of"`), instructed to "Mirror
  03-decisions.md"
...
other ineligible finding (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:320#"high/critical or other ineligible finding."`;
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:322#"unchanged and setting Decision to"`; `05-review-findings.md:16#"| Severity | Category | Description | Suggested Fix | Decision |"`).
...
orchestrator-workflow completeness reader verifies" (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:236#"those two are what the orchestrator-workflow completeness reader verifies; for every"`).
Immediately after that quote, detailed workflow reference also carries a 0.13.0 addition
on the same table's placeholder/legend row (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:240#"rows as the template never having been filled in. When"`): replace it
...
reproduction requirement (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:247#"lint): only claims that could vary run to run trigger"`); full treatment is out of scope
...
chosen enum value when finalizing each file (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:355#"non-accepting (fail-closed)."`). Left as
...
the placeholder-row rule (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:247#"lint): only claims that could vary run to run trigger"`): when acceptance rests on such
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:328#"record the Fix-regression decision point"`).
...
Docs-only review default (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:225#"For a review round whose entire delta contains only explanatory documentation"`):
