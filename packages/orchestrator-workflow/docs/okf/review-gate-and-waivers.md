...
(`SKILL.md:554#"severity: low | medium | high | critical"`, reviewer output contract). Only high and critical block
...
reject` (`SKILL.md:559#"acceptance_recommendation: accept | accept_with_notes |"`; mirrored in the findings template's Acceptance
...
- Critical: "waived by the operator. The orchestrator never waives a
  ...
  (`SKILL.md:331#"explicitly waived: critical findings require operator"`).
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:108#"rationale."`; `SKILL.md:332#"findings require the orchestrator to record a"`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`SKILL.md:333#"or critical finding counts as a waiver and follows the"`). A deferred
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:110#"the Accepted Waivers section of"`; `SKILL.md:335#"the Accepted Waivers section of"`), instructed to "Mirror
  03-decisions.md"
...
other ineligible finding (`SKILL.md:344#"high/critical or other ineligible finding."`;
...
pinned in `test/docs-consistency.test.ts:4622#"docs-only closing deltas stay narrowly bounded"`.
...
reproduction requirement (`SKILL.md:298#"lint): only claims that could vary run to run trigger"`); full treatment is out of scope
...
(`SKILL.md:748#"review and never satisfies the review gate, since"`), pinned by
...
the placeholder-row rule (`SKILL.md:298#"lint): only claims that could vary run to run trigger"`): when acceptance rests on such
...
field (`SKILL.md:568#"matches_implementer_claim: matched | mismatched |"`, `reviewer.md:164#"residual_risks:"`); `matches_implementer_claim`
...
`SKILL.md:577#"instead of inferring one from the findings list."` states it and adds the orchestrator's response when it is
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:1221#"the orchestrator asks the reviewer to resupply it"` pins
...
`SKILL.md:765#"## Review-round escalation budget"` triggers "by the second
...
(`SKILL.md:780#"**Tier or model escalation**"`,
`SKILL.md:786#"**Advisor spawn**"`, `SKILL.md:789#"**Merge-hold**"`: raise
...
(`SKILL.md:792#"Judgment governs which of the three to pick; only that one is chosen and"`).
