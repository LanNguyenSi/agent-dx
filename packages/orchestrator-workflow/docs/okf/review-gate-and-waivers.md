...
(`SKILL.md:302`, reviewer output contract). Only high and critical block
...
(`agents-md-section.md:75-78`). Medium and low are "addressed or consciously
accepted at the orchestrator's judgment" (`agents-md-section.md:86-87`); no
...
reject` (`SKILL.md:306`; mirrored in the findings template's Acceptance
...
318-320`; the installed `reviewer.md:30-31` prompt carries the mirrored
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:84-85`; `SKILL.md:184-185`), instructed to "Mirror
  03-decisions.md"
...
orchestrator-workflow completeness reader verifies" (`SKILL.md:164-167`).
Immediately after that quote, SKILL.md step 7 also carries a 0.13.0 addition
on the same table's placeholder/legend row (`SKILL.md:167-171`): replace it
...
reproduction requirement (`SKILL.md:171-178`); full treatment is out of scope
...
(`packages/orchestrator-workflow/CHANGELOG.md:525-539`).
...
header itself was already correct (`CHANGELOG.md:541-557`, the
...
chosen enum value when finalizing each file (`SKILL.md:202-206`). Left as
...
reviewer output contract (`CHANGELOG.md:439-460`). The Subagent misfire rule
...
the placeholder-row rule (`SKILL.md:171-178`): when acceptance rests on such
...
field (`SKILL.md:311-315`, `reviewer.md:76-80`); `matches_implementer_claim`
accepts `not_applicable` so a review that never hits the narrow trigger is
not forced to fabricate a reproduction record.

Motivating incident (`CHANGELOG.md:353-382`): agent-dx run
...
`SKILL.md:318-320` states it and adds the orchestrator's response when it is
...
(`CHANGELOG.md:262-308`, agent-tasks task 16637a96): one reviewer round in
that dogfood omitted `acceptance_recommendation` entirely.
`packages/orchestrator-workflow/test/docs-consistency.test.ts:941-959` pins
