...
(`SKILL.md:264`, reviewer output contract). Only high and critical block
...
reject` (`SKILL.md:268`; mirrored in the findings template's Acceptance
...
280-282`; the installed `reviewer.md:26-27` prompt carries the mirrored
...
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:55-56`; `SKILL.md:149-150`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`SKILL.md:150-151`). A deferred
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:57-58`; `SKILL.md:151-153`), instructed to "Mirror
  03-decisions.md"
...
orchestrator-workflow completeness reader verifies" (`SKILL.md:131-134`).
Immediately after that quote, SKILL.md step 7 also carries a 0.13.0 addition
on the same table's placeholder/legend row (`SKILL.md:135-138`): replace it
...
reproduction requirement (`SKILL.md:139-146`); full treatment is out of scope
...
chosen enum value when finalizing each file (`SKILL.md:164-168`). Left as
...
the placeholder-row rule (`SKILL.md:139-146`): when acceptance rests on such
...
same rule (`reviewer.md:41-46`), and both output contracts gained a matching
`reproduction: {method, sample_size, result, matches_implementer_claim}`
field (`SKILL.md:273-277`, `reviewer.md:66-70`); `matches_implementer_claim`
...
`SKILL.md:280-282` states it and adds the orchestrator's response when it is
