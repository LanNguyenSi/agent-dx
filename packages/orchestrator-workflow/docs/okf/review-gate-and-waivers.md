...
(`packages/orchestrator-workflow/assets/skill/references/contracts.md:221-223#"instead of inferring one from the findings list."`; the installed `reviewer.md:176#"never leave it blank or omit it."` prompt carries the mirrored
...
- Critical: "waived by the operator. The orchestrator never waives a
  ...
  (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:291#"explicitly waived: critical findings require operator"`).
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:117#"rationale."`; `packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:292#"findings require the orchestrator to record a"`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:293#"or critical finding counts as a waiver and follows the"`). A deferred
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:119#"the Accepted Waivers section of"`; `packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:295#"the Accepted Waivers section of"`), instructed to "Mirror
  03-decisions.md"
...
other ineligible finding (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:304#"high/critical or other ineligible finding."`;
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:306#"unchanged and setting Decision to"`; `05-review-findings.md:16#"| Severity | Category | Description | Suggested Fix | Decision |"`).
...
orchestrator-workflow completeness reader verifies" (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:220#"those two are what the orchestrator-workflow completeness reader verifies; for every"`).
Immediately after that quote, detailed workflow reference also carries a 0.13.0 addition
on the same table's placeholder/legend row (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:224#"rows as the template never having been filled in. When"`): replace it
...
reproduction requirement (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:231#"lint): only claims that could vary run to run trigger"`); full treatment is out of scope
...
chosen enum value when finalizing each file (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:339#"non-accepting (fail-closed)."`). Left as
...
the placeholder-row rule (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:231#"lint): only claims that could vary run to run trigger"`): when acceptance rests on such
...
same rule (`reviewer.md:226#"lint) do not trigger this."`), and both output contracts gained a matching
`reproduction: {method, sample_size, result, matches_implementer_claim}`
field (`packages/orchestrator-workflow/assets/skill/references/contracts.md:215#"matches_implementer_claim: matched | mismatched |"`, `reviewer.md:300#"residual_risks:"`); `matches_implementer_claim`
...
findings, and the installed `reviewer.md:176#"never leave it blank or omit it."` prompt carries the mirrored
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:312#"record the Fix-regression decision point"`).
...
Docs-only review default (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:209#"For a review round whose entire delta contains only explanatory documentation"`):
