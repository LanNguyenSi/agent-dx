...
(`SKILL.md:318`, reviewer output contract). Only high and critical block
...
reject` (`SKILL.md:322`; mirrored in the findings template's Acceptance
...
334-336`; the installed `reviewer.md:30-31` prompt carries the mirrored
...
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:108-109`; `SKILL.md:193-194`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`SKILL.md:194-195`). A deferred
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:110-111`; `SKILL.md:196-197`), instructed to "Mirror
  03-decisions.md"
...
orchestrator-workflow completeness reader verifies" (`SKILL.md:176-179`).
Immediately after that quote, SKILL.md step 7 also carries a 0.13.0 addition
on the same table's placeholder/legend row (`SKILL.md:179-183`): replace it
...
reproduction requirement (`SKILL.md:183-190`); full treatment is out of scope
...
(`packages/orchestrator-workflow/CHANGELOG.md:601-611`).
...
header itself was already correct (`CHANGELOG.md:617-629`, the
...
chosen enum value when finalizing each file (`SKILL.md:218-222`). Left as
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:111-141`
...
"waived by the operator" (test lines 118-124); `SKILL.md`'s "block acceptance
until fixed or explicitly waived" and "Accepted Waivers section of
`06-handoff.md`" phrasing (test lines 126-131); `06-handoff.md`'s `##
Accepted Waivers` heading and its `Finding | Severity | Rationale` header
(test lines 133-136). A negative pin (test lines 138-142) guards against a
...
reviewer output contract (`CHANGELOG.md:515-532`). The Subagent misfire rule
...
(`SKILL.md:470-471`), pinned by
`packages/orchestrator-workflow/test/docs-consistency.test.ts:400-403`. Since
...
the placeholder-row rule (`SKILL.md:183-190`): when acceptance rests on such
...
field (`SKILL.md:327-331`, `reviewer.md:76-80`); `matches_implementer_claim`
accepts `not_applicable` so a review that never hits the narrow trigger is
not forced to fabricate a reproduction record.

Motivating incident (`CHANGELOG.md:429-454`): agent-dx run
...
`SKILL.md:334-336` states it and adds the orchestrator's response when it is
...
(`CHANGELOG.md:338-362`, agent-tasks task 16637a96): one reviewer round in
that dogfood omitted `acceptance_recommendation` entirely.
`packages/orchestrator-workflow/test/docs-consistency.test.ts:975-993` pins
