...
(`SKILL.md:505#"severity: low | medium | high | critical"`, reviewer output contract). Only high and critical block
...
reject` (`SKILL.md:510#"acceptance_recommendation: accept | accept_with_notes |"`; mirrored in the findings template's Acceptance
...
- Critical: "waived by the operator. The orchestrator never waives a
  ...
  (`SKILL.md:299#"explicitly waived: critical findings require operator"`).
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:101#"rationale."`; `SKILL.md:300#"findings require the orchestrator to record a"`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`SKILL.md:301#"or critical finding counts as a waiver and follows the"`). A deferred
...
other ineligible finding (`SKILL.md:312#"high/critical or other ineligible finding."`;
...
(`SKILL.md:314#"unchanged and setting Decision to"`; `05-review-findings.md:9-11#"| Severity | Category | Description | Suggested Fix | Decision |"`).
...
pinned in `test/docs-consistency.test.ts:4173#"docs-only closing deltas stay narrowly bounded"`.
...
chosen enum value when finalizing each file (`SKILL.md:345#"non-accepting (fail-closed)."`). Left as
...
(`SKILL.md:686#"review and never satisfies the review gate, since"`), pinned by
...
`SKILL.md:703#"## Review-round escalation budget"` triggers "by the second
...
(`SKILL.md:718#"**Tier or model escalation**"`,
`SKILL.md:724#"**Advisor spawn**"`, `SKILL.md:727#"**Merge-hold**"`: raise
...
(`SKILL.md:730#"Judgment governs which of the three to pick; only that one is chosen and"`).
