...
(`SKILL.md:277`, reviewer output contract). Only high and critical block
...
reject` (`SKILL.md:281`; mirrored in the findings template's Acceptance
...
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:55-56`; `SKILL.md:160-161`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`SKILL.md:161-162`). A deferred
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:57-58`; `SKILL.md:162-163`), instructed to "Mirror
  03-decisions.md"
...
orchestrator-workflow completeness reader verifies" (`SKILL.md:143-146`).
Immediately after that quote, SKILL.md step 7 also carries a 0.13.0 addition
on the same table's placeholder/legend row (`SKILL.md:147-150`): replace it
...
reproduction requirement (`SKILL.md:150-157`); full treatment is out of scope
...
(`packages/orchestrator-workflow/CHANGELOG.md:443-457`).
...
header itself was already correct (`CHANGELOG.md:459-475`, the
...
chosen enum value when finalizing each file (`SKILL.md:177-181`). Left as
...
reviewer output contract (`CHANGELOG.md:357-378`). The Subagent misfire rule
...
the placeholder-row rule (`SKILL.md:150-157`): when acceptance rests on such
...
Motivating incident (`CHANGELOG.md:271-300`): agent-dx run
...
(`CHANGELOG.md:180-226`, agent-tasks task 16637a96): one reviewer round in
