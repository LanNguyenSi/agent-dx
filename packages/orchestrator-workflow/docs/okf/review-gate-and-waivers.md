...
- Critical: "waived by the operator. The orchestrator never waives a
  ...
  (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:294#"explicitly waived: critical findings require operator"`).
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:117#"rationale."`; `packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:295#"findings require the orchestrator to record a"`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:296#"or critical finding counts as a waiver and follows the"`). A deferred
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:119#"the Accepted Waivers section of"`; `packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:298#"the Accepted Waivers section of"`), instructed to "Mirror
  03-decisions.md"
...
other ineligible finding (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:307#"high/critical or other ineligible finding."`;
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:309#"unchanged and setting Decision to"`; `05-review-findings.md:16#"| Severity | Category | Description | Suggested Fix | Decision |"`).
...
orchestrator-workflow completeness reader verifies" (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:223#"those two are what the orchestrator-workflow completeness reader verifies; for every"`).
Immediately after that quote, detailed workflow reference also carries a 0.13.0 addition
on the same table's placeholder/legend row (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:227#"rows as the template never having been filled in. When"`): replace it
...
reproduction requirement (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:234#"lint): only claims that could vary run to run trigger"`); full treatment is out of scope
...
chosen enum value when finalizing each file (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:342#"non-accepting (fail-closed)."`). Left as
...
the placeholder-row rule (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:234#"lint): only claims that could vary run to run trigger"`): when acceptance rests on such
...
(`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:315#"record the Fix-regression decision point"`).
...
Docs-only review default (`packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:212#"For a review round whose entire delta contains only explanatory documentation"`):
