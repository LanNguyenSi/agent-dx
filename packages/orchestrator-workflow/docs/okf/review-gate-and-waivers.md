...
(`packages/orchestrator-workflow/assets/agents/reviewer.md:85#"A reviewer recommendation is not orchestrator acceptance"`). This is a
...
(`SKILL.md:509#"severity: low | medium | high | critical"`, reviewer output contract). Only high and critical block
...
reject` (`SKILL.md:514#"acceptance_recommendation: accept | accept_with_notes |"`; mirrored in the findings template's Acceptance
...
355-357`; the installed `reviewer.md:90#"never leave it blank or omit it."` prompt carries the mirrored
...
- Critical: "waived by the operator. The orchestrator never waives a
  ...
  (`SKILL.md:303#"explicitly waived: critical findings require operator"`).
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:101#"rationale."`; `SKILL.md:304#"findings require the orchestrator to record a"`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`SKILL.md:305#"or critical finding counts as a waiver and follows the"`). A deferred
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:103#"the Accepted Waivers section of"`; `SKILL.md:307#"the Accepted Waivers section of"`), instructed to "Mirror
  03-decisions.md"
...
other ineligible finding (`SKILL.md:316#"high/critical or other ineligible finding."`;
...
(`SKILL.md:318#"unchanged and setting Decision to"`; `05-review-findings.md:9-11#"| Severity | Category | Description | Suggested Fix | Decision |"`).
...
pinned in `test/docs-consistency.test.ts:4243#"docs-only closing deltas stay narrowly bounded"`.
...
orchestrator-workflow completeness reader verifies" (`SKILL.md:269#"orchestrator-workflow completeness reader verifies."`).
Immediately after that quote, SKILL.md step 7 also carries a 0.13.0 addition
on the same table's placeholder/legend row (`SKILL.md:273#"rows as the template never having been filled in. When"`): replace it
...
reproduction requirement (`SKILL.md:280#"lint): only claims that could vary run to run trigger"`); full treatment is out of scope
...
chosen enum value when finalizing each file (`SKILL.md:349#"non-accepting (fail-closed)."`). Left as
...
(`SKILL.md:690#"review and never satisfies the review gate, since"`), pinned by
...
the placeholder-row rule (`SKILL.md:280#"lint): only claims that could vary run to run trigger"`): when acceptance rests on such
...
same rule (`reviewer.md:115#"lint) do not trigger this."`), and both output contracts gained a matching
`reproduction: {method, sample_size, result, matches_implementer_claim}`
field (`SKILL.md:523#"matches_implementer_claim: matched | mismatched |"`, `reviewer.md:141#"residual_risks:"`); `matches_implementer_claim`
...
`SKILL.md:528#"instead of inferring one from the findings list."` states it and adds the orchestrator's response when it is
missing — ask the reviewer to resupply it, rather than infer one from the
findings — and the installed `reviewer.md:90#"never leave it blank or omit it."` prompt carries the mirrored
...
`SKILL.md:707#"## Review-round escalation budget"` triggers "by the second
...
(`SKILL.md:722#"**Tier or model escalation**"`,
`SKILL.md:728#"**Advisor spawn**"`, `SKILL.md:731#"**Merge-hold**"`: raise
...
(`SKILL.md:734#"Judgment governs which of the three to pick; only that one is chosen and"`).
