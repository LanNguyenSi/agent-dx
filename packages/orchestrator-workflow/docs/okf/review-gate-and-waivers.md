...
(`SKILL.md:526#"severity: low | medium | high | critical"`, reviewer output contract). Only high and critical block
...
reject` (`SKILL.md:531#"acceptance_recommendation: accept | accept_with_notes |"`; mirrored in the findings template's Acceptance
...
- Critical: "waived by the operator. The orchestrator never waives a
  ...
  (`SKILL.md:320#"explicitly waived: critical findings require operator"`).
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:108#"rationale."`; `SKILL.md:321#"findings require the orchestrator to record a"`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`SKILL.md:322#"or critical finding counts as a waiver and follows the"`). A deferred
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:110#"the Accepted Waivers section of"`; `SKILL.md:324#"the Accepted Waivers section of"`), instructed to "Mirror
  03-decisions.md"
...
other ineligible finding (`SKILL.md:333#"high/critical or other ineligible finding."`;
...
(`SKILL.md:335#"unchanged and setting Decision to"`; `05-review-findings.md:14-16#"| Severity | Category | Description | Suggested Fix | Decision |"`).
...
orchestrator-workflow completeness reader verifies" (`SKILL.md:278#"orchestrator-workflow completeness reader verifies."`).
Immediately after that quote, SKILL.md step 7 also carries a 0.13.0 addition
on the same table's placeholder/legend row (`SKILL.md:282#"rows as the template never having been filled in. When"`): replace it
...
reproduction requirement (`SKILL.md:289#"lint): only claims that could vary run to run trigger"`); full treatment is out of scope
...
chosen enum value when finalizing each file (`SKILL.md:366#"non-accepting (fail-closed)."`). Left as
...
(`SKILL.md:720#"review and never satisfies the review gate, since"`), pinned by
...
the placeholder-row rule (`SKILL.md:289#"lint): only claims that could vary run to run trigger"`): when acceptance rests on such
...
field (`SKILL.md:540#"matches_implementer_claim: matched | mismatched |"`, `reviewer.md:164#"residual_risks:"`); `matches_implementer_claim`
...
`SKILL.md:549#"instead of inferring one from the findings list."` states it and adds the orchestrator's response when it is
...
`SKILL.md:737#"## Review-round escalation budget"` triggers "by the second
...
(`SKILL.md:752#"**Tier or model escalation**"`,
`SKILL.md:758#"**Advisor spawn**"`, `SKILL.md:761#"**Merge-hold**"`: raise
...
(`SKILL.md:764#"Judgment governs which of the three to pick; only that one is chosen and"`).
