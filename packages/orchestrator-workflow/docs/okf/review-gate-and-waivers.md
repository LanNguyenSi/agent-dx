...
(`SKILL.md:347#"severity: low | medium | high | critical"`, reviewer output contract). Only high and critical block
...
reject` (`SKILL.md:352#"acceptance_recommendation: accept | accept_with_notes |"`; mirrored in the findings template's Acceptance
...
- Critical: "waived by the operator. The orchestrator never waives a
  ...
  (`SKILL.md:216#"explicitly waived: critical findings require operator"`).
- High: "waived by the orchestrator with a recorded rationale"
  (`agents-md-section.md:100-101#"rationale."`; `SKILL.md:216-217#"findings require the orchestrator to record a"`).
- Deferring counts as waiving, for both severities: "Deferring such a
  ...
  waiver and follows the same rules" (`SKILL.md:217-218#"or critical finding counts as a waiver and follows the"`). A deferred
...
- Summarized in `06-handoff.md`'s Accepted Waivers section
  (`agents-md-section.md:102-103#"the Accepted Waivers section of"`; `SKILL.md:219-220#"the Accepted Waivers section of"`), instructed to "Mirror
  03-decisions.md"
...
orchestrator-workflow completeness reader verifies" (`SKILL.md:194-197#"orchestrator-workflow completeness reader verifies."`).
Immediately after that quote, SKILL.md step 7 also carries a 0.13.0 addition
on the same table's placeholder/legend row (`SKILL.md:197-201#"rows as the template never having been filled in. When"`): replace it
...
reproduction requirement (`SKILL.md:201-208#"lint): only claims that could vary run to run trigger"`); full treatment is out of scope
...
chosen enum value when finalizing each file (`SKILL.md:247-251#"non-accepting (fail-closed)."`). Left as
...
`test/template-markers.test.ts:106-158#"expect(reviewTemplate).toMatch(/arms? the"`, independently pins the
...
(`SKILL.md:501-502#"review and never satisfies the review gate, since"`), pinned by
`packages/orchestrator-workflow/test/docs-consistency.test.ts:504-506#"never satisfies the review gate"`. Since
...
the placeholder-row rule (`SKILL.md:201-208#"lint): only claims that could vary run to run trigger"`): when acceptance rests on such
...
field (`SKILL.md:357-361#"matches_implementer_claim: matched | mismatched |"`, `reviewer.md:82-85#"residual_risks:"`); `matches_implementer_claim`
...
`SKILL.md:364-366#"instead of inferring one from the findings list."` states it and adds the orchestrator's response when it is
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:1053-1068#"the orchestrator asks the reviewer to resupply it"` pins
...
`SKILL.md:519#"## Review-round escalation budget"` triggers "by the second
...
(`SKILL.md:534#"**Tier or model escalation**"`,
`SKILL.md:540#"**Advisor spawn**"`, `SKILL.md:543#"**Merge-hold**"`: raise
...
(`SKILL.md:546#"Judgment governs which of the three to pick; only that one is chosen and"`).
