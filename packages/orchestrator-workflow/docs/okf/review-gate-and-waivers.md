...
(`SKILL.md:319`, reviewer output contract). Only high and critical block
...
reject` (`SKILL.md:323`; mirrored in the findings template's Acceptance
...
335-337`; the installed `reviewer.md:30-31` prompt carries the mirrored
...
orchestrator-workflow completeness reader verifies" (`SKILL.md:174-177`).
Immediately after that quote, SKILL.md step 7 also carries a 0.13.0 addition
on the same table's placeholder/legend row (`SKILL.md:177-181`): replace it
...
reproduction requirement (`SKILL.md:181-188`); full treatment is out of scope
...
(`packages/orchestrator-workflow/CHANGELOG.md:716-726`).
...
header itself was already correct (`CHANGELOG.md:732-744`, the
...
chosen enum value when finalizing each file (`SKILL.md:219-223`). Left as
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:110-140`
...
reviewer output contract (`CHANGELOG.md:630-647`). The Subagent misfire rule
...
`packages/orchestrator-workflow/test/docs-consistency.test.ts:399-402`. Since
...
the placeholder-row rule (`SKILL.md:181-188`): when acceptance rests on such
...
field (`SKILL.md:328-332`, `reviewer.md:76-80`); `matches_implementer_claim`
accepts `not_applicable` so a review that never hits the narrow trigger is
not forced to fabricate a reproduction record.

Motivating incident (`CHANGELOG.md:544-569`): agent-dx run
...
`SKILL.md:335-337` states it and adds the orchestrator's response when it is
...
(`CHANGELOG.md:453-477`, agent-tasks task 16637a96): one reviewer round in
