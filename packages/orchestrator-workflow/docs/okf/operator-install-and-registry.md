...
1. An explicit CLI flag always wins, the same override-vs-persist rule
   ...
   (cli-inputs.ts:258-268#"const tiers = opts.tiers ?? previous?.tiers ?? false;").
...
(init.ts:429-436#"normalizedPin === null ? undefined : (normalizedPin ?? previous?.pin);").
...
(init.ts:222-226#"? { pin: candidate.pin.trim() }"). `doctor`'s `versionLag`
