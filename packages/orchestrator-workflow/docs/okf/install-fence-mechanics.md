...
timestamp: 2026-08-30T10:32:45Z
sources:
...
- Per selected harness (`options.harnesses`), and per role `rolesForProfile(profile)` selects for that harness (0.15.0: `full` installs `{explorer,task-slicer,implementer,reviewer}`, `minimal` installs only `{implementer,reviewer}`; since 0.21.0 `full` also installs `advisor` (`{explorer,task-slicer,implementer,reviewer,advisor}`), the fifth role added purely by extending `ROLES`/`MINIMAL_PROFILE_ROLES` in `src/models.ts`, `rolesForProfile` itself unchanged, `minimal` still filters against the same two-role `MINIMAL_PROFILE_ROLES` set, so advisor is dropped from `minimal` for free, the same way explorer/task-slicer already are; see [model-preselection.md](model-preselection.md) and the manifest's `profile` field below):
  - **claude**: `.claude/skills/orchestrator-workflow/SKILL.md` and `.claude/agents/{role}.md` for each installed role (init.ts:636-641#"composeClaudeAgent(role, options.models[role]),"), plus the `CLAUDE.md` import (init.ts:653#"ensureClaudeImport(report, join(targetDir,").
  - **codex**: only `.agents/skills/orchestrator-workflow/SKILL.md` (init.ts:656-657#".agents"). No per-role agent files are written regardless of profile; README.md:129#"the skill instructs running the roles inline with the same contracts." states Codex has no standardized project-level subagent definition, the skill instructs running the roles inline instead.
  - **opencode**: `.opencode/skills/orchestrator-workflow/SKILL.md` and `.opencode/agents/{role}.md` for each installed role (init.ts:662-673#"composeOpencodeAgent(role, modelValue, defaultEffortLine),").
- `.ai/workflow/manifest.json`, written last, only when the computed desired state differs from what is recorded (init.ts:710-745#"${JSON.stringify(manifest, null, 2)}\n").
...
(init.ts:643-648#"composeClaudeAgentVariant(role, tier)," claude, :675-702 opencode); see
...
finding R2-L1): before writing an opencode variant, `init.ts:680-689#"// (e.g. a low/medium tier on a Claude-family model, or any"` checks
...
proved rather than assumed. `init.ts:695#"const effortLine = opencodeEffortLine(tier, variantModelValue);"` now computes the effort line only
...
provider prefix, R3-L4) still renders normally with just its `model:` line. The matching CLI-input-resolution half of that fix (`cli-inputs.ts:311-337#"// and need no live catalog lookup, so they are unaffected)."`)
...
itself (`harnesses: []`, sorted same as any other value, init.ts:715#"harnesses: [...options.harnesses].sort(),"). Every per-harness branch
...
(init.ts:619-626#"if (options.harnesses.length > 0) {", covered above), so
...
`previous.harnessesRecordedEmpty` (cli-inputs.ts:210-213#"previous.harnessesRecordedEmpty"),
...
F2, `cli-inputs.ts:248-249#"await promptHarnesses(detected, [], false)"`),
rather than
