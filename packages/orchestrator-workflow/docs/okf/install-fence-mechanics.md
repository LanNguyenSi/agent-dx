...
provider prefix, R3-L4) still renders normally with just its `model:` line. The matching CLI-input-resolution half of that fix (`cli-inputs.ts:313-339#"// and need no live catalog lookup, so they are unaffected)."`)
...
`previous.harnessesRecordedEmpty` (cli-inputs.ts:218-221#"previous.harnessesRecordedEmpty"),
...
F2, `cli-inputs.ts:251#"await promptHarnesses(detected, [], false)"`),
rather than
...
`Boolean(repoManifest)`, cli.ts:783-783#"previousIsRecordedManifest: Boolean(repoManifest),"),
...
(cli.ts:570-570#"harnessesRecordedEmpty: repoManifest?.harnessesRecordedEmpty,"),
