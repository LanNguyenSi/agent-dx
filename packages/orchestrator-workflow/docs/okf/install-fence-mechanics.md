...
provider prefix, R3-L4) still renders normally with just its `model:` line. The matching CLI-input-resolution half of that fix (`cli-inputs.ts:395-395#"// and need no live catalog lookup, so they are unaffected)."`)
...
`previous.harnessesRecordedEmpty` (cli-inputs.ts:263-263#"previous.harnessesRecordedEmpty"),
...
F2, `cli-inputs.ts:305-305#"stickyAnnotateDetected ?? detected,"`),
rather than
