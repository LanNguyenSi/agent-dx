...
- `CLAUDE.md` exists: if any line's whitespace-split tokens already include the literal `@AGENTS.md` (writers.ts:130-134), nothing is written, an inline mention like `"Rules: see @AGENTS.md first."` already counts (init.test.ts:240-244#"expect(importCount).toBe(1);"). Otherwise a blank line plus `@AGENTS.md` is appended once (writers.ts:139-146); a second `init` run does not duplicate it (init.test.ts:227-235#"# Claude notes\n").
