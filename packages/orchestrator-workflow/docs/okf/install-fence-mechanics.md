...
instead of that function recomputing it internally, since the caller
already needs the same value to decide whether to write the file at all;
the fixed comment now also states explicitly what is NOT skipped by this
check: a *resolved* model with no effort line (a low/medium tier on a
Claude-family model, or any Ollama model) still renders normally with just
its `model:` line. The matching CLI-side half of that fix (`cli.ts:291-322`)
