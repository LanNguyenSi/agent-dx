...
provider prefix, R3-L4) still renders normally with just its `model:` line. The matching CLI-input-resolution half of that fix (`cli-inputs.ts:365-391#"// and need no live catalog lookup, so they are unaffected)."`)
...
F2, `cli-inputs.ts:301-302#"await promptHarnesses(filesystemDetected ?? detected, [], false)"`),
rather than
...
`Boolean(repoManifest)`, cli.ts:813-813#"previousIsRecordedManifest: Boolean(repoManifest),"),
...
(cli.ts:579-579#"harnessesRecordedEmpty: repoManifest?.harnessesRecordedEmpty,"),
...
same residual F2 closed for `detected` alone -- `apply`'s CLI action passes
a fresh `detectHarnesses(targetDir)` as `resolveInitInputs`'s separate
`filesystemDetected` field instead (cli.ts:793-793#"filesystemDetected: detectHarnesses(targetDir),"),
and only the templates-only branch reads it (agent-tasks fe834823,
previously a known residual, see docs/okf/log.md's 2026-08-31 entry).
