...
provider prefix, R3-L4) still renders normally with just its `model:` line. The matching CLI-input-resolution half of that fix (`cli-inputs.ts:396-396#"// and need no live catalog lookup, so they are unaffected)."`)
...
`previous.harnessesRecordedEmpty` (cli-inputs.ts:264-264#"previous.harnessesRecordedEmpty"),
...
F2, `cli-inputs.ts:306-306#"stickyAnnotateDetected ?? detected,"`),
rather than
