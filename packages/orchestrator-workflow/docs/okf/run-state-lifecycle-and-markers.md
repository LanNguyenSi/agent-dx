...
(SKILL.md:81#"the files as the run progresses. The newest run"; packages/orchestrator-workflow/README.md:140#"one directory per unit of work, newest = active";
...
missing v1 field never identifies a run as legacy or blocks it: existing runs
continue under their recorded original contract, and uncertain provenance is
...
applies unchanged regardless of harness (SKILL.md:588#"applies unchanged.";
SKILL.md:591#"pointer rule from Run state applies unchanged."; SKILL.md:591#"pointer rule from Run state applies unchanged.").
...
(packages/orchestrator-workflow/README.md:146#"it to the repository's";
...
non-accepting (fail-closed)" (SKILL.md:339#"non-accepting (fail-closed)."). A freshly-copied run is
...
run-gate" per SKILL.md:337#"value. That marker line is the machine-readable signal"; this doc cites that in-repo statement only, it
...
(SKILL.md:263#"rows as the template never having been filled in. When"). The runtime half (grounding-mcp's reader treating a
...
`okf-kit check`) (SKILL.md:327#"validator when one is available (for example"). It is explicitly non-gating: "apply
this optional guidance" and "Repos without a bundle are unaffected"
(SKILL.md:323#"guidance: when the repo carries a curated knowledge"; SKILL.md:328#"without a bundle are unaffected"). Since 0.24.0 (placement rule) step 9 also
...
to a reusable instruction file (SKILL.md:333#"or the consuming workspace, with a pointer left behind."); the fix is to move the
...
hand-mapping terrain, SKILL.md:151#"with an index) before mapping terrain by hand, treating"); the 0.12.0 changelog entry names it
