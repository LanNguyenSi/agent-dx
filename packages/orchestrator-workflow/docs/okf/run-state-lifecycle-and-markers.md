...
applies unchanged regardless of harness (SKILL.md:690#"applies unchanged.";
SKILL.md:693#"pointer rule from Run state applies unchanged."; SKILL.md:708#"pointer rule from Run state applies unchanged.").
...
non-accepting (fail-closed)" (SKILL.md:371#"non-accepting (fail-closed)."). A freshly-copied run is
...
run-gate" per SKILL.md:369#"value. That marker line is the machine-readable signal"; this doc cites that in-repo statement only, it
...
(SKILL.md:285#"rows as the template never having been filled in. When"). The runtime half (grounding-mcp's reader treating a
...
`okf-kit check`) (SKILL.md:359#"validator when one is available (for example"). It is explicitly non-gating: "apply
this optional guidance" and "Repos without a bundle are unaffected"
(SKILL.md:355#"guidance: when the repo carries a curated knowledge"; SKILL.md:360#"without a bundle are unaffected"). Since 0.24.0 (placement rule) step 9 also
...
to a reusable instruction file (SKILL.md:365#"or the consuming workspace, with a pointer left behind."); the fix is to move the
