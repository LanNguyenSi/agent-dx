...
`TODO` falls back to the date heuristic (packages/orchestrator-workflow/CHANGELOG.md:778-783#0.9.0,
...
(CHANGELOG.md:777-790#0.9.0). Pinned by template-markers.test.ts:19#"const runBaseRe = /solution-acceptance:\s*run-base\s*=\s",33-37 (exactly
...
(CHANGELOG.md:879-886#0.7.0). Consumer is "the harness solution-acceptance
...
CHANGELOG.md:827-839#0.7.3): a live run had drifted onto
...
CHANGELOG.md:811-821#0.7.4) so the template itself never invites `fix`/`reject`
...
(CHANGELOG.md:701-719#0.12.0). Pinned by
