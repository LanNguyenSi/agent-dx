...
(packages/orchestrator-workflow/assets/skill/references/run-state-and-harness.md:81-82#"the files as the run progresses. The newest run"; packages/orchestrator-workflow/README.md:144#"one directory per unit of work, newest = active";
...
(packages/orchestrator-workflow/README.md:150#"it to the repository's";
...
non-accepting (fail-closed)" (packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:377#"non-accepting (fail-closed)."). A freshly-copied run is
...
run-gate" per packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:375#"value. That marker line is the machine-readable signal"; this doc cites that in-repo statement only, it
...
(packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:256#"rows as the template never having been filled in. When"). The runtime half (grounding-mcp's reader treating a
...
`okf-kit check`) (packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:364#"validator when one is available (for example"). It is explicitly non-gating: "apply
this optional guidance" and "Repos without a bundle are unaffected"
(packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:358#"guidance: when the repo carries a curated knowledge"; packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:365#"without a bundle are unaffected"). Step 9 is a safety net for sources
...
(packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:354-356#"hand-off check is a safety net for sources the task list missed. Before"). Since 0.24.0 (placement rule) step 9 also
...
to a reusable instruction file (packages/orchestrator-workflow/assets/skill/references/evidence-and-probes.md:371#"or the consuming workspace, with a pointer left behind."); the fix is to move the
...
(packages/orchestrator-workflow/README.md:426#"## Run modes")
...
would otherwise seed a grant. Only this grant condition is shared verbatim,
in the generated policy section, `SKILL.md`, and this marker's own
subsection of run-state-and-harness.md. The value grammar follows the
one-normative-place pattern of the run mode marker above instead: that
subsection is its one definition site
(packages/orchestrator-workflow/assets/skill/references/run-state-and-harness.md:148#"This subsection is the one definition site of the marker's value grammar."),
and the policy section and `SKILL.md` each state the orchestrator-only
scope in their own words. AGENTS.md's Outward-facing actions section
...
`template-markers.test.ts` and `run-mode.test.ts` for this marker:
`describe` blocks covering every file that carries the rule (the policy
...
on the rule sentences (an example list is covered as part of the sentence
that carries it, not example by example), a check that the grantable-class
sentence in `agents-md-section.md` names exactly `push-branch` and
`open-pr`, and a guard that no retired class token appears in the kit text.
