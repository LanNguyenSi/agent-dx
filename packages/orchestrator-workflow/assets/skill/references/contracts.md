...
emit `withdrawn: []` when nothing was withdrawn. In run mode `single`, `reproduction` also carries the result of the reviewer's duty to replay every orchestrator probe that the briefing names by definition; step 7 of the detailed workflow states the rule, and no output field is added for it.
