...
7. **Delegate review.** Send the diff to the reviewer subagent, naming in the
   ...
   them; tiers themselves are unchanged by this axis. For a review round whose entire delta is documentation, default to the `-medium` reviewer tier with `review_method: normal` where tier variants are installed; going higher is the orchestrator's call with a recorded reason, and the minimums named above are unaffected. For a change whose acceptance rests on tests that pin documentation wording, write the briefing as the Pinned-prose changes section of [review and recovery](review-and-recovery.md) requires. When the reviewer's
