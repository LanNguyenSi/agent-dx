...
8. **Decide acceptance.** Accept, request fixes, defer, or escalate to the
   ...
   `recurrence: repeated` finding whose `introduced_by_delta` is `yes` or
   `unknown` (see Round-2 halt rule below): before any further implementer
   spawn on that task, name split or redesign in `03-decisions.md`. By the
