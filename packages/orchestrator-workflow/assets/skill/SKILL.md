...
2. **Discover (optional, read-only).** When the goal, the solution, or the
   ...
   semantic code-search tool over raw grep for orientation questions, and a
   structural code-search tool over text grep for symbol lookups (callers,
   definitions). Fold its findings into a
