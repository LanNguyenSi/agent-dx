...
- A run now declares a run mode. `assets/templates/00-goal.md` carries
  ...
  The policy section also names the run's `mode` marker in its Run state list and points to the docs-only review default of the skill's Delegate review step. Because the policy section changed, an existing install receives it only with its next `init` or `apply`. In run mode `single` the reviewer replays the orchestrator's probes: step 7 of `references/evidence-and-probes.md` excludes the skip permission for probes named by definition in that mode and requires the reviewer to replay every orchestrator probe that the briefing names by definition and to report in `reproduction` whether each replayed verdict matches the recorded one, a mismatch being a finding of at least `high`; `assets/agents/reviewer.md` (every rendered variant) carries the duty and keeps it inert unless the briefing names the mode, and the reviewer output contract is unchanged.
