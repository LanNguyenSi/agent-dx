...
- A run now declares a run mode. `assets/templates/00-goal.md` carries
  ...
  The policy section also names the run's `mode` marker in its Run state list and points to the docs-only review default of the skill's Delegate review step. Because the policy section changed, an existing install receives it only with its next `init` or `apply`. In run mode `single` the reviewer replays the orchestrator's probes: step 7 of `references/evidence-and-probes.md` excludes the skip permission for probes named by definition in that mode and requires the reviewer, under any `review_method`, to replay every named orchestrator probe (named by its full definition or by a resolved immutable plan-and-result reference, not by an id alone) and to report in `reproduction` whether each replayed verdict matches the recorded one, a mismatch being a finding of at least `high` that also sets `matches_implementer_claim: mismatched`, and a briefing in that mode without any named probe being missing evidence; `assets/agents/reviewer.md` (every rendered variant) carries the duty and keeps it inert unless the briefing names the mode, and the reviewer output contract is unchanged.
