...
- A verification set can no longer look green while checking the wrong
  ...
  when a run-base marker names a linked worktree, that every literal
  repository path in the set's argv and cwd is re-resolved to that
  worktree's root before freezing, and that the frozen snapshot records the
  resolved paths. The implementer and reviewer prompts, contracts.md, and
  evidence-and-probes.md state one shared rule: repository identity
  includes the repository path (the worktree root), not only the revision;
  a set frozen against another checkout (for example the main checkout
  while the diff comes from a linked worktree) or a result checked at a
  revision other than the diff's is a misfire, not a pass, while a
  difference between the snapshot's recorded revision and the checked
  revision alone still is not. Tests pin both sentences in the reference
  files and in every rendered implementer and reviewer variant (#336).
