...
- `citations-resolve` now supports an optional **anchor** on a full
  ...
  - **Heading form** (bare/unquoted, e.g. `#0.24.0` or `#[0.24.0]`): the
    ...
    *whole* range, not merely precede its start. Deliberately capped at
    ...
    deeper subsection headings are transparent to the search instead.
    Mismatch is reported as `anchor-heading-mismatch` (wrong section) or
  ...
  - **String form** (double-quoted, e.g. `#"reproduction requirement"`):
    ...
    heading" has no meaning. Mismatch is `anchor-not-found-in-range`, a
    **warning**.
  ...
  and is checked exactly as before. Two rejected alternatives: embedding
  ...
  is easy to leave behind when a sentence is edited later.
  **Measured** (this change; see the PR for the full mutation-probe log):
  ...
  already correct; 0 false positives from the new check). A read-only
  sample against `agent-grounding/docs/okf` (not migrated to this form,
  out of scope for this change) also reports 0 anchor findings: 0 false
  positives from an anchorless corpus, as expected from the backward-
  compatible design. Two mutation probes against the `orchestrator-
  workflow` bundle: shifting one migrated citation's range into its
  neighbouring release section raised the warning count from 13 to 14
  (`anchor-heading-mismatch`), reverted to 13 clean; inserting an 8-line
  dummy entry at the top of that package's `CHANGELOG.md` (simulating a
  normal release-note insertion) raised the warning count from 13 to 29,
  flagging all 16 migrated citations as `anchor-heading-mismatch` (the
  historical failure mode this change targets), reverted to 13 clean.
