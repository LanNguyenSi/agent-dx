...
| `short-form-unbound` | A short-form citation (see below) has no full citation earlier in its own paragraph to bind to. |
| `test-range-start-not-head` / `test-range-end-not-closing` | A **paren-form** short-form citation's range into a test file does not start on a `describe(`/`it(` head line, or does not end on a matching closing `});` line. |
| `markdown-range-boundary-bracket-or-fence` | A short-form citation's range into a Markdown target starts or ends on a bare bracket or code-fence line; reported as a `notice` (never counted toward `--strict`). |
...
**Short-form citations.** Distinct from a continuation above: a *bare* (no backtick, no file name at all) colon-range `:N-M` or parenthesized range `(N-M)`, e.g. `(373-375)` or `:580-588` in running prose. A short-form citation binds to **the last full `path:N-M` citation named earlier in the same paragraph** -- not the nearest preceding citation anywhere in the doc (that is what a continuation does); a paragraph boundary is a blank (empty or whitespace-only) line. A short-form citation with no full citation earlier in its own paragraph is reported `short-form-unbound` (a warning), not silently skipped. Only a range is recognised, never a bare single number (`(1)`, `:5`): a bare number is too easily an unrelated enumeration marker (e.g. a numbered list item) to detect mechanically without a large false-positive cost. A resolved short-form citation gets every check a full citation gets, plus, for a **paren-form** short form only, the test-file/Markdown block-boundary check described above (`test-range-*` / `markdown-range-boundary-bracket-or-fence`) -- colon-form short forms are excluded from that additional check because, empirically, a colon-form short form inside a compound `path:N-M, :X-Y, :Z-W`-style list conventionally marks an approximate detail location within an already-cited range rather than an entire describe/it block, and enforcing block boundaries there produces false positives. Reserved files (`index.md`, `log.md`) are excluded from short-form matching entirely: they are append-only narrative journals that routinely narrate historical "old N-M -> new X-Y" line-number deltas as prose about the past, not live citations against current content.
