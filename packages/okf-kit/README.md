...
| `test-range-straddles-block` | (opt-in, `--require-anchors`) A full citation's own range into a `.test.ts`/`.spec.ts` target contains a `describe`/`it`/`test` block-head line on any line other than the range's own first line. **Warning**. |
