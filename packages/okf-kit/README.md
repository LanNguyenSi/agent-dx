...
| `citations-resolve` | warning / notice | For docs with a repo root, flags a `` `path:N`/`path:N-M` `` citation (and its `` `:N` ``/`` -`M` ``/`` (`N`) `` continuations, and bare paragraph-bound short forms `:N-M`/`(N-M)`) whose target file is missing, whose range is inverted or exceeds the file, or whose start line is blank or (for a non-markdown target) only a closing brace. A full citation may also carry an optional `#anchor` (e.g. `` `CHANGELOG.md:50-144#0.24.0` ``), checked against the target's own structure/content instead of just its line numbers. A backtick-delimited `` `path#heading` `` citation resolves to a whole Markdown section instead of a line range, immune to every line-number shift above it; see "Heading-section citations" below. A short-form citation's range into a test file is also checked for a describe/it block boundary. See "Citation resolution (citations-resolve)" below. |
...
**Heading-section citations.** A CHANGELOG.md that grows by inserting each new release at the TOP forces every later entry's absolute line numbers to shift on every release, so even a heading-anchored `path:N-M#anchor` citation into it needs re-pointing every time (the anchor catches a shift that lands the range in the *wrong* section; it does not stop the range itself from drifting). `` `path#heading` `` sidesteps line numbers entirely: it names a heading and resolves to that heading's own section -- from the line after the heading up to, but not including, the next heading of the same or shallower level, or end of file -- so an edit anywhere above the cited section changes nothing about the citation. Two checks:

- The heading text must match **exactly one** level 1 or 2 heading in the target, using the identical `heading.text.includes(...)` containment check the line-range anchor's heading form already uses (reused, not reinvented -- see the "Known limitations" note above, which applies here too). Zero matches is `heading-section-not-found`; more than one is `heading-section-ambiguous`, reported rather than silently resolved to the first match.
- The resolved section must be **non-empty**: at least one non-blank line before the next heading of the same or shallower level, otherwise `heading-section-empty`.

Capped at heading level 2, like the line-range anchor's heading form, and for the identical reason: a Keep-a-Changelog `CHANGELOG.md` repeats `### Added`/`### Changed`/`### Fixed` subsections under every release, so a level-3+ heading name is essentially never unique across a real CHANGELOG -- matching it would just turn every such citation into a guaranteed `heading-section-ambiguous`. This is not lifted for the new form; it stays usable for exactly the case it was built for (a release section as a whole).

An optional content anchor, `` `path#heading#"text"` `` (always the quoted form -- a section's body is prose, not a second heading to look up), must occur on **exactly one** line inside the resolved section: absent is `heading-section-content-anchor-not-found`, present on more than one line is `heading-section-content-anchor-ambiguous`. This is stricter than the line-range string anchor's "at least one line" (see above): a whole section is a much larger haystack than a caller-chosen line range, so "found somewhere" is weaker evidence the anchor still points at the intended spot.

**Backtick-delimited, unlike the line-range form.** The line-range anchor form does not require backticks (see the "Anchor syntax note" above), but a heading-section citation always does: `` `path#heading` ``, never a bare `path#heading` outside backticks. An ordinary Markdown link's target commonly has exactly this shape (`[install](docs/README.md#install)`), and unlike the line-range form -- gated by a `:N` no relative link ever contains -- there is no other character available to tell a real citation apart from a link's href. Requiring the whole citation inside one pair of backticks closes that gap without inventing a second marker: a live Markdown link's target is never itself backtick-wrapped.
