...
| `heading-section-malformed` | A backtick-delimited `` `path:#...` `` opener does not parse as a well-formed heading-section citation (an unterminated content-anchor quote, an unquoted third segment, or a non-`.md` target); reported as a `notice` (never counted toward `--strict`) so a typo does not silently vanish. |
