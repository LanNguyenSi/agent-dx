...
async function fetchReposForOwner(
  owner: string,
  octokit: any,
): Promise<string[]> {
  ...
  try {
    const result = await withRetry(async () =>
      (octokit as any).rest.repos.listForOrg({
        org: owner,
        per_page: 100,
        sort: "pushed",
      }),
    ...
    const result = await withRetry(async () =>
      (octokit as any).rest.repos.listForUser({
        username: owner,
        per_page: 100,
        sort: "pushed",
      }),
...
export function registerStandupCommands(program: Command): void {
  program
    .command("standup")
    .description(
      "Daily standup digest — show commits across repos for a time range",
    )
    .requiredOption("-o, --owner <owner>", "GitHub username or organization")
    .option(
      "-r, --repos <repos...>",
      "Specific repos to check (default: all repos for owner)",
    )
    .option("-d, --days <number>", "How many days back to look", "1")
    .option(
      "-a, --author <author>",
      "Filter commits by author (GitHub username)",
    )
