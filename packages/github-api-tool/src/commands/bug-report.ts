...
export function registerBugReportCommands(program: Command): void {
  program
    ...
    .requiredOption(
      "-r, --repo <owner/repo>",
      "Repository (e.g. LanNguyenSi/telerithm)",
    )
    ...
    .option(
      "-l, --labels <labels>",
      "Comma-separated labels (default: bug)",
      "bug",
    )
    ...
    .action(async (options) => {
      try {
        ...
        if (!title || !observed || !expected || !reproduce) {
          ...
          if (!reproduce)
            reproduce = await prompt(rl, "Steps to reproduce (or curl): ");
          if (!hypothesis)
            hypothesis = await prompt(
              rl,
              "Hypothesis (optional, Enter to skip): ",
            );
          rl.close();
        }

        const body = BUG_TEMPLATE(
          observed,
          expected,
          reproduce,
          hypothesis || "",
        ...
        outputError(
          `Failed to create bug report: ${err instanceof Error ? err.message : String(err)}`,
        );
        process.exit(1);
      }
    });
