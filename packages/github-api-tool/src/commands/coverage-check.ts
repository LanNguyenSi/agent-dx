...
export function parseCoverageSummary(
  ...
  if (pct === undefined) {
    throw new Error(
      "Cannot determine coverage percentage (missing statements and lines)",
    );
  }
...
export function registerCoverageCheckCommands(program: Command): void {
  program
    ...
    .description(
      "Check test coverage against a minimum threshold (per ENGINEERING.md)",
    )
    .option(
      "-i, --input <path>",
      ...
      try {
        ...
        if (options.json) {
          ...
          if (result.statements !== undefined)
            console.log(`   Statements : ${result.statements.toFixed(1)}%`);
          if (result.lines !== undefined)
            console.log(`   Lines      : ${result.lines.toFixed(1)}%`);
          if (result.functions !== undefined)
            console.log(`   Functions  : ${result.functions.toFixed(1)}%`);
          if (result.branches !== undefined)
            console.log(`   Branches   : ${result.branches.toFixed(1)}%`);
          console.log("");
          if (result.passed) {
            success(
              `${options.label} passes minimum threshold of ${threshold}%`,
            );
          } else {
            outputError(
              `${options.label} ${result.pct.toFixed(1)}% is below required ${threshold}%`,
            );
          }
        }
        ...
        outputError(
          `coverage-check failed: ${err instanceof Error ? err.message : String(err)}`,
        );
        process.exit(1);
      }
    });
