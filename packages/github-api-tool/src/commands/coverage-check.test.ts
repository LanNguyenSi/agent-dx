...
describe("parseCoverageSummary", () => {
  ...
  it("throws when total key is missing", () => {
    expect(() =>
      parseCoverageSummary(JSON.stringify({ "src/foo.ts": {} }), 80),
    ).toThrow('Coverage file missing "total" key');
  });

  it("throws on invalid JSON", () => {
    expect(() => parseCoverageSummary("not-json", 80)).toThrow(
  ...
  it("throws when no pct can be determined", () => {
    const json = JSON.stringify({
      total: { functions: { total: 10, covered: 9, skipped: 0, pct: 90 } },
    });
    expect(() => parseCoverageSummary(json, 80)).toThrow(
      "Cannot determine coverage percentage",
    );
  });
