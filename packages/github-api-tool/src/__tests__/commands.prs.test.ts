...
describe("pr list", () => {
  ...
  it("lists PRs and maps them to output format", async () => {
    ...
    expect(output).toHaveBeenCalledWith(
      expect.arrayContaining([
        expect.objectContaining({ number: 11, author: "alice" }),
      ]),
      expect.anything(),
    );
  });
  ...
  it("calls process.exit(1) on API failure", async () => {
    const octokit = makeOctokit();
    vi.mocked(octokit.rest.pulls.list).mockRejectedValue(
...
describe("pr review", () => {
  ...
  it("submits an APPROVE review successfully", async () => {
    ...
    expect(success).toHaveBeenCalledWith(
      "Review submitted for PR #5 (APPROVE)",
    );
  });
  ...
  it("calls process.exit(1) on API failure", async () => {
    const octokit = makeOctokit();
    vi.mocked(octokit.rest.pulls.createReview).mockRejectedValue(
...
describe("pr merge", () => {
  ...
  it("calls process.exit(1) on API failure", async () => {
    const octokit = makeOctokit();
    vi.mocked(octokit.rest.pulls.merge).mockRejectedValue(
