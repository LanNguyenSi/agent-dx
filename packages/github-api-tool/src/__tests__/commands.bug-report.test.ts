...
describe("bug-report (non-interactive, all flags supplied)", () => {
      ...
      rest: {
        issues: {
          create: vi.fn().mockResolvedValue({
            data: { number: 99, html_url: "https://gh/99" },
          }),
        },
      },
    };
  ...
  it("outputs JSON when --json flag is set", async () => {
    const mockOctokit = {
      rest: {
        issues: {
          create: vi.fn().mockResolvedValue({
            data: { number: 5, html_url: "https://gh/5" },
          }),
        },
      },
    };
  ...
  it("calls process.exit(1) when the API throws", async () => {
    const mockOctokit = {
      rest: {
        issues: { create: vi.fn().mockRejectedValue(new Error("API error")) },
      },
    };
    vi.mocked(getOctokit).mockResolvedValue(mockOctokit as never);
    ...
    const mockOctokit = {
      rest: {
        issues: {
          create: vi
            .fn()
            .mockResolvedValue({ data: { number: 1, html_url: "u" } }),
        },
      },
    };
    vi.mocked(getOctokit).mockResolvedValue(mockOctokit as never);
