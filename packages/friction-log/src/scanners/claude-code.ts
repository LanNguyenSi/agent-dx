...
export class ClaudeCodeAdapter implements Scanner {
  ...
  async scan(input: ScannerInput): Promise<ScannerOutput> {
    ...
    if (!transcriptPath) {
      throw new Error(
        "friction-log: claude-code adapter requires --transcript <path>",
      );
    }
    if (!existsSync(transcriptPath)) {
      throw new Error(
        `friction-log: transcript not found at ${transcriptPath}`,
      );
    }
    ...
    const sessionId =
      input.sessionId ?? deriveSessionIdFromPath(transcriptPath);
...
function deriveSessionIdFromPath(transcriptPath: string): string {
  const base =
    transcriptPath.replace(/\\/g, "/").split("/").pop() ?? transcriptPath;
...
function toolErrorCandidate(
  toolName: string,
  errText: string,
): RawFrictionCandidate {
...
function phraseCandidates(text: string): RawFrictionCandidate[] {
  ...
  for (const para of paragraphs) {
    ...
    if (trimmed.length === 0 || trimmed.length > MAX_PHRASE_LINE_CHARS)
      continue;
