...
export function defaultDbPath(): string {
  return (
    process.env.FRICTION_LOG_DB ??
    join(xdgDataHome(), "friction-log", "db.sqlite")
  );
}

export function defaultConfigPath(): string {
  return (
    process.env.FRICTION_LOG_CONFIG ??
    join(xdgConfigHome(), "friction-log", "config.yml")
  );
}

export function defaultMarkdownSinkDir(): string {
  return (
    process.env.FRICTION_LOG_MARKDOWN_DIR ??
    join(xdgDataHome(), "friction-log", "frictions")
  );
}
