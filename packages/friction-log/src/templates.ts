...
function templatesDir(): string {
  ...
  throw new Error(
    `friction-log: cannot locate bundled templates (looked in ${candidates.join(", ")})`,
  );
}
...
export function loadTemplate(name: string): Template {
  ...
  if (!existsSync(path)) {
    throw new Error(
      `friction-log: unknown template "${name}" (no file at ${path})`,
    );
  }
  ...
  if (!parsed || typeof parsed !== "object") {
    throw new Error(
      `friction-log: template "${name}" is not a valid YAML object`,
    );
  }
  ...
  if (!title || !body) {
    throw new Error(
      `friction-log: template "${name}" must define both title and body`,
    );
  }
