...
import {
  defaultConfigPath,
  syncExportOriginOverride,
  syncExportPathOverride,
} from "./paths.js";
...
export function loadConfig(
  ...
  try {
    ...
    throw new Error(
      `friction-log: failed to read config file ${path}: ${(err as Error).message}`,
    );
  }
  let parsed: unknown;
  try {
    ...
    throw new Error(
      `friction-log: failed to parse YAML in ${path}: ${(err as Error).message}`,
    );
  }
  if (parsed == null) return applySyncExportEnvOverrides(EMPTY_CONFIG);
  if (typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error(
      `friction-log: config at ${path} must be a YAML mapping at the top level`,
    );
  }
  ...
  if (sinksRaw != null) {
    if (typeof sinksRaw !== "object" || Array.isArray(sinksRaw)) {
      throw new Error(
        `friction-log: config at ${path} has a non-mapping "sinks" value`,
      );
    }
    for (const [name, value] of Object.entries(
      sinksRaw as Record<string, unknown>,
    )) {
      if (value == null) continue;
      if (typeof value !== "object" || Array.isArray(value)) {
        throw new Error(
          `friction-log: config sinks.${name} must be a YAML mapping`,
        );
      }
...
function parseSyncExportBlock(
  raw: unknown,
  configPath: string,
): SyncExportConfig | undefined {
  if (raw == null) return undefined;
  if (typeof raw !== "object" || Array.isArray(raw)) {
    throw new Error(
      `friction-log: config at ${configPath} has a non-mapping "sync_export" value`,
    );
  }
  ...
  if (typeof rawPath !== "string" || !rawPath.trim()) {
    throw new Error(
      `friction-log: config sync_export.path at ${configPath} must be a non-empty string`,
    );
  }
  if (typeof rawOrigin !== "string" || !rawOrigin.trim()) {
    throw new Error(
      `friction-log: config sync_export.origin at ${configPath} must be a non-empty string`,
    );
  }
  let peerPaths: string[] = [];
  if (rawPeers != null) {
    if (
      !Array.isArray(rawPeers) ||
      !rawPeers.every((p) => typeof p === "string")
    ) {
      throw new Error(
        `friction-log: config sync_export.peer_paths at ${configPath} must be a list of strings`,
      );
    }
 ...
 */
function applySyncExportEnvOverrides(
  ...
  return {
    ...config,
    syncExport: { path, origin, peerPaths: config.syncExport?.peerPaths ?? [] },
  };
}
...
export function parseSinkOpts(pairs: string[]): SinkConfig {
  ...
  for (const pair of pairs) {
    ...
    if (!m) {
      throw new Error(
        `friction-log: --sink-opt expected key=value, got "${pair}"`,
      );
    }
    const key = m[1];
    const rawValue = m[2];
