...
export class GithubIssuesSink implements Sink {
  ...
  async file(
    ...
    const labels = Array.from(
      new Set([...(parsed.labels ?? []), ...rendered.labels]),
    );
    const args = [
      "issue",
      "create",
    ...
    if (result.error) {
      throw new Error(
        `friction-log: gh CLI not runnable: ${result.error.message}`,
      );
    }
    if (result.status !== 0) {
      const stderr = (result.stderr ?? "").trim();
      throw new Error(
        `friction-log: gh issue create failed (exit ${result.status}): ${stderr || "no stderr"}`,
      );
    }
    const issueUrl = (result.stdout ?? "").trim().split("\n").pop() ?? "";
    if (!/^https:\/\/github\.com\//.test(issueUrl)) {
      throw new Error(
        `friction-log: gh stdout did not contain an issue URL: ${result.stdout}`,
      );
    }
    return {
      ok: true,
...
function normalizeStringList(
  ...
  throw new Error(
    `friction-log: github-issues "${name}" must be a string or array of strings`,
  );
}
...
function renderBody(friction: Friction, rendered: RenderedTemplate): string {
  const meta: string[] = [];
  meta.push(
    `<sub>friction-log id ${friction.id} captured ${friction.capturedAt}</sub>`,
  );
  if (friction.toolSurface)
    meta.push(`<sub>tool: \`${friction.toolSurface}\`</sub>`);
  ...
  if (friction.recurrenceOfId != null)
    meta.push(`<sub>recurrence of friction #${friction.recurrenceOfId}</sub>`);
