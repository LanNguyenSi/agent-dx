...
export class LinearSink implements Sink {
  ...
  async file(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `friction-log: linear API returned ${response.status}: ${body.slice(0, 400)}`,
      );
    }
    ...
    if (
      !json.data?.issueCreate?.success ||
      !issue?.id ||
      !issue.identifier ||
      !issue.url
    ) {
      throw new Error(
        `friction-log: linear issueCreate did not return a complete issue: ${JSON.stringify(json).slice(0, 400)}`,
      );
    }
    return {
      ok: true,
...
async function resolveStateId(
  ...
  if (!response.ok) {
    const body = await safeText(response);
    throw new Error(
      `friction-log: linear state lookup ${response.status}: ${body.slice(0, 400)}`,
    );
  }
  const json = (await response.json()) as {
    data?: {
  ...
  if (json.errors?.length) {
    throw new Error(
      `friction-log: linear state lookup errors: ${json.errors.map((e) => e.message ?? "(no message)").join("; ")}`,
    );
  }
  ...
  if (!match?.id) {
    const known = nodes
      ...
      .join(", ");
    throw new Error(
      `friction-log: linear team ${teamId} has no state matching "${stateName}". Known: ${known || "(none)"}`,
    );
  }
  return match.id;
}
