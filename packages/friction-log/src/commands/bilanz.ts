...
export async function runBilanz(
  input: BilanzCommandInput,
): Promise<BilanzCommandOutput> {
...
export function formatBilanz(b: BilanzInput): string {
  ...
  lines.push("");
  lines.push(
    `Session: \`${b.session.id}\` (${b.session.startedAt}${b.session.endedAt ? ` to ${b.session.endedAt}` : ""})`,
  );
  if (b.session.projectPaths && b.session.projectPaths.length > 0) {
    lines.push(
      `Project paths: ${b.session.projectPaths.map((p) => `\`${p}\``).join(", ")}`,
    );
  }
  lines.push("");
  ...
  const unfiled = b.frictions.filter(
    (f) => f.tasks.length === 0 && f.status === "open",
  );
  ...
  if (b.frictions.length === 0) {
    ...
    for (const f of b.frictions) {
      ...
      lines.push(
        `  - [${marker}] id=${f.id} ${f.toolSurface ? `(${f.toolSurface}) ` : ""}${f.title}`,
      );
    }
  }
  lines.push("");
  ...
  if (unfiled.length > 0) {
    lines.push(
      `**Open frictions without a filed task** (${unfiled.length}) -- consider \`friction-log file <id>\`:`,
    );
    for (const f of unfiled) {
      lines.push(
        `  - id=${f.id} ${f.toolSurface ? `(${f.toolSurface}) ` : ""}${f.title}`,
      );
    }
    lines.push("");
  }
