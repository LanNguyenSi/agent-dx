...
export function runImport(input: ImportCommandInput): ImportCommandOutput {
  if (input.format !== "markdown-frontmatter") {
    throw new Error(
      `friction-log: import format "${input.format}" is not supported in M5`,
    );
  }
  ...
  try {
    ...
    for (const existing of db.listFrictions({
      source: "import",
      limit: 100_000,
    })) {
      for (const tag of db.tagsFor(existing.id)) {
        if (tag.startsWith("import-hash:"))
          knownHashes.add(tag.slice("import-hash:".length));
      }
    }
...
export function parseMarkdownContent(raw: string): ParsedRecord | null {
  ...
  const toolSurface = pickString(frontmatter, [
    "tool_surface",
    "toolSurface",
    "tool",
    "surface",
  ...
  const sessionId = pickString(frontmatter, [
    "session_id",
    "sessionId",
    "session",
  ]);
  const capturedAt = pickIsoDate(frontmatter, [
    "captured_at",
    "capturedAt",
    "date",
  ]);
...
function pickSeverity(frontmatter: Record<string, unknown>): Severity | null {
  ...
  return (SEVERITY_VALUES as readonly string[]).includes(lower)
    ? (lower as Severity)
    : null;
}
