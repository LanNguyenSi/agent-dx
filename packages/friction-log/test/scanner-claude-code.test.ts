...
describe("ClaudeCodeAdapter", () => {
  ...
  it("extracts a tool-error candidate from is_error tool_result", async () => {
    ...
    const out = await new ClaudeCodeAdapter().scan({
      transcriptPath: path,
      sessionId: "s2",
    });
  ...
  it("extracts an MCP tool-error candidate with the tool name as surface", async () => {
          ...
          content: [
            {
              type: "tool_use",
              id: "tu1",
              name: "mcp__agent-tasks__tasks_list",
              input: {},
            },
          ],
        },
    ...
    expect(out.frictionCandidates).toHaveLength(1);
    expect(out.frictionCandidates[0].toolSurface).toBe(
  ...
  it("extracts a friction-phrase candidate from a short assistant paragraph", async () => {
    ...
    const phraseCandidate = out.frictionCandidates.find(
      (c) => c.category === "workflow-friction",
    );
  ...
  it("ignores a friction-phrase that appears in a long paragraph (likely analysis, not admission)", async () => {
    const longProse =
      "This is a very long paragraph that mentions doesn't work but only as part of a much longer analysis. ".repeat(
        5,
      );
    const path = writeTranscript("s5.jsonl", [
      {
        ...
        message: {
          role: "assistant",
          content: [{ type: "text", text: longProse }],
        },
      },
    ]);
    const out = await new ClaudeCodeAdapter().scan({
    ...
    const path = writeTranscript("s6.jsonl", [
      {
        ...
        message: {
          role: "assistant",
          content: [{ type: "text", text: "Does this not work for you?" }],
        },
      },
    ]);
    const out = await new ClaudeCodeAdapter().scan({
  ...
  it("survives malformed JSONL lines without throwing", async () => {
    const path = writeTranscript("s8.jsonl", [
  ...
  it("throws when transcript path is missing", async () => {
    await expect(new ClaudeCodeAdapter().scan({})).rejects.toThrow(
      /requires --transcript/,
    );
  });
