...
describe("payloadToScanInput", () => {
  it("honors snake_case keys", () => {
    expect(
      payloadToScanInput({ session_id: "s", transcript_path: "/p" }),
    ).toEqual({
      sessionId: "s",
      transcriptPath: "/p",
      adapter: undefined,
  ...
  it("honors camelCase keys", () => {
    expect(
      payloadToScanInput({ sessionId: "s", transcriptPath: "/p" }),
    ).toEqual({
      sessionId: "s",
      transcriptPath: "/p",
      adapter: undefined,
...
describe("runBilanz", () => {
  ...
  it("throws when the requested session is not in the db", async () => {
    await expect(runBilanz({ sessionId: "nope", dbPath })).rejects.toThrow(
      /not found/,
    );
  });
...
describe("runUpdate", () => {
  ...
  it("throws on missing friction", () => {
    expect(() =>
      runUpdate({ frictionId: 9999, status: "open", dbPath }),
    ).toThrow(/not found/);
  });
});
