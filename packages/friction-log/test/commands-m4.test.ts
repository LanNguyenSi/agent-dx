...
describe("config loader", () => {
  ...
  it("reads sinks.* mappings from YAML", () => {
    ...
    expect(cfg.sinks["github-issues"]).toEqual({
      repo: "o/r",
      labels: ["bug", "friction"],
    });
  });
...
describe("sink registry", () => {
  ...
  it("lazy-loads modules on first request and caches via factory swap", async () => {
    const fake = {
      name: "agent-tasks",
      file: vi.fn(async () => ({ ok: true, sinkTarget: "stub" })),
    };
    const restore = registerSinkFactory(
      "agent-tasks",
      async () => fake as unknown as InstanceType<typeof AgentTasksSink>,
    );
    try {
...
describe("github-issues sink", () => {
  ...
  it('throws a friendly error when "repo" is missing', async () => {
    ...
    await expect(
      sink.file(fakeFriction, fakeRendered, { sinkOpts: {} }),
...
describe("agent-tasks sink", () => {
  it("emits an MCP intent JSON when mode=mcp-emit, no network call", async () => {
    ...
    expect(lines).toHaveLength(1);
    const intent = JSON.parse(lines[0]) as {
      tool: string;
  ...
  it("POSTs to the REST endpoint with bearer auth and returns the new task id", async () => {
    ...
    expect(restCall[0]).toBe(
      "https://api.example.test/api/projects/proj-uuid/tasks",
    );
    expect((restCall[1].headers as Record<string, string>).authorization).toBe(
      "Bearer TOKEN",
    );
    const body = JSON.parse(restCall[1].body as string) as {
      title: string;
      priority: string;
    };
  ...
  it("rejects without projectId regardless of mode", async () => {
    ...
    await expect(
      sink.file(fakeFriction, fakeRendered, {
        sinkOpts: { mode: "rest", apiBase: "x", token: "y" },
      }),
  ...
  it("surfaces REST error bodies on non-2xx", async () => {
    const fetchSpy = vi.fn(
      async () => new Response("upstream said no", { status: 502 }),
    );
    const sink = new AgentTasksSink(fetchSpy, () => {});
    await expect(
      sink.file(fakeFriction, fakeRendered, {
...
describe("linear sink", () => {
  it("skips the state lookup when no state is given and posts a minimal issueCreate", async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledTimes(1);
    const linearCall = fetchSpy.mock.calls[0] as unknown as [
      string,
      RequestInit,
    ];
    const body = JSON.parse(linearCall[1].body as string) as {
      query: string;
  ...
  it("resolves a named state via a workflow-states lookup, then posts with stateId", async () => {
    ...
    const createCall = fetchSpy.mock.calls[1] as unknown as [
      string,
      RequestInit,
    ];
  ...
  it("skips the workflow-states lookup when state is already a UUID", async () => {
    ...
    await sink.file(fakeFriction, fakeRendered, {
      sinkOpts: {
        teamId: "t",
        state: "01234567-1234-1234-1234-123456789abc",
        apiKey: "k",
      },
    });
    expect(fetchSpy).toHaveBeenCalledTimes(1);
    const onlyCall = fetchSpy.mock.calls[0] as unknown as [string, RequestInit];
    const body = JSON.parse(onlyCall[1].body as string) as {
      variables: { input: Record<string, unknown> };
    };
    expect(body.variables.input.stateId).toBe(
  ...
  it("surfaces a named state that does not exist on the team", async () => {
    const fetchSpy = vi.fn(
      async () =>
        new Response(
          JSON.stringify({
            data: { team: { states: { nodes: [{ id: "s", name: "Done" }] } } },
          }),
          { status: 200, headers: { "content-type": "application/json" } },
        ),
    );
    ...
    await expect(
      sink.file(fakeFriction, fakeRendered, {
        sinkOpts: { teamId: "t", state: "Backlog", apiKey: "k" },
      }),
...
describe("runFile integration with widened sinkOpts", () => {
  ...
  it("CLI --sink-opt key=value wins over config defaults inside runFile", async () => {
    ...
    try {
      ...
      expect(out.externalRef).toBe(
        "https://github.com/cli-override/repo/issues/9",
      );
