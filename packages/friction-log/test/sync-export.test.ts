...
describe("runSyncExport", () => {
  ...
  it("writes every friction with no cap (>100 rows), sorted captured_at asc with an id tiebreaker", () => {
    ...
    try {
      ...
      const tieA = db.insertFriction({
        title: "tie-a",
        source: "manual",
        capturedAt: tieAt,
      });
      const tieB = db.insertFriction({
        title: "tie-b",
        source: "manual",
        capturedAt: tieAt,
      });
...
describe("write-through hardening", () => {
  it("(HIGH regression) a config.yml with no sync_export block but otherwise malformed does not fail any mutating command", async () => {
    ...
    try {
      ...
      expect(() =>
        runRm({ frictionId: toRemove.id, dbPath, configPath }),
      ).not.toThrow();
      ...
      const db = new FrictionDb(dbPath);
      try {
        expect(db.listFrictions({ limit: 100 }).length).toBeGreaterThanOrEqual(
          4,
        );
      } finally {
        db.close();
      ...
      expect(
        stderrSpy.mock.calls.some((c) =>
          String(c[0]).includes("sync-export write-through failed"),
        ),
      ).toBe(true);
    } finally {
