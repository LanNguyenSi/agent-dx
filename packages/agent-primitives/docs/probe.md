...
- A match is a mention of the root unless the text CONTINUES THE PATH WORD
  ...
  matching the `--log-dir` spelling for the exclusion below uses its OWN,
  narrower rule throughout -- not only at the ANSI-C sub-rule -- since
  ending the `--log-dir` match validates an EXCLUSION rather than a
  refusal: only a path separator and the shell's own word separators end
  it there, nothing wider. The exclusion bullet below has the full
  enumeration and what it costs. Everything else ends the word
...
claiming the suite executed nothing. **These detectors, the generic
fallback, `--require-baseline-evidence`, `verify`'s own `--pass-regex`
check, and `--pass-regex`'s own BASELINE-side verdict, all
...
scrolled out of the tail (on any of those paths) reads as a plain miss,
not "not present at all" -- see the truncation paragraph below for how
that is surfaced.
...
correction work. In short, the two windows differ: the baseline (and
`verify`) never see past the captured tail; only a mutant's own miss can
still be resolved from the full, untruncated log.
...
non-matching output, a non-zero exit code, under `--expect fail`, and
(when the captured tail was truncated) a full on-disk log that also
never matches the pattern -- carries no miss warning at all.

A truncated captured tail on the mutant path is not left as pure
speculation: rather than warning that the pattern "may have matched
output the run never captured," a miss whose captured tail was
truncated is checked against that run's own full, untruncated log on
disk (`test.logPath`, the same file every result already points at).
When the full log also never matches the pattern, the truncation was
never actually ambiguous, and the miss carries no warning at all (the
same as a textbook kill). When the full log DOES match outside the
captured tail, the verdict this run actually earned is `survived`, not
...
[Output shape](output-shape.md)), what type it is, and when it is present.
...
`mutation_probe` is not directly pasteable into an orchestrator
implementer's `mutation_probes` entry: the definition needed to replay a
mutant lives beside it in `mutant`. When both objects are present, map one
newly run probe as follows:

| Implementer report field | Probe-result source or value |
| --- | --- |
| `mutant` | `mutation_probe.mutant` |
| `file` | `mutant.file` |
| `anchor` | `mutant.line` |
| `before` | `mutant.before` |
| `after` | `mutant.after` |
| `verified_applied_via` | `mutation_probe.verified_applied_via` |
| `result` | `mutation_probe.result` |
| `expectation` | `mutation_probe.expectation`, or `not_applicable` when absent |
| `reason` | `mutation_probe.reason`, or an empty string when absent |
| `restored_verified` | `mutation_probe.restored_verified` |
| `replayed` | `false` for a newly introduced probe; `true` when rerunning a prior-round definition |

For a `--plan` run, apply the same mapping to each
`plan.results[].mutant` and its sibling `plan.results[].mutation_probe`.
If either sibling is absent, do not invent the missing report fields:
report the incomplete evidence and resolve it before claiming the probe.
