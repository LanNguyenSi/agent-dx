...
target, and naming why whenever it instead falls back to a co-located
guess (absent from `PATH`, resolving nothing, or doctor's aggregate
spawn deadline already spent); see the `probe` section's own "Python
