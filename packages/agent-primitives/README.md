...
a git repository (including one with a dangling `.git` entry; presence,
not target reachability, is the boundary) is skipped, named in a warning, rather than pulling in
...
3. What git tracks is never shared, asked in two places. A directory
   ...
   candidate, including an operator's own `--link`, whatever either question
   ...
   `.git` boundary and the copy's own `.git` metadata remain non-linkable.
