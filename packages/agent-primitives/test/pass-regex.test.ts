...
/**
 * `compilePassRegex` (GitHub issue #225): the one compile both `cli.ts`'s
 * `parsePassRegex` and `probe/plan.ts`'s `validatePlan` use for
 * `--pass-regex`/`passWhen.regex`. The `m` flag it always applies is what lets
 * `^OK \(` match a summary line that is not the very first line of a runner's
 * combined output (phpunit's own version banner ahead of a green suite's
 * summary, the motivating shape) -- without it, `^`/`$` anchor to the whole
 * buffer, not each line, and a header-carrying runner's baseline reads
 * `baseline_failed` despite being green.
 */
