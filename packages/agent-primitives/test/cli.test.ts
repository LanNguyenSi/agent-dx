...
describe("cli: probe --plan", () => {
  ...
  // The CLI's own pass of `global.maxChars` as the third
  // `reconcileEnvelopeDiffTruncation` argument (at both call sites) is
  // pinned in-process by `test/cli-maxchars-wiring.test.ts`, on the
  // argument itself rather than on the envelope's final byte count: the
  // few bytes the argument is worth at a given `-m` sit inside the
  // envelope's run-to-run noise (a timing digit, a temp-dir name), so a
  // spawned-CLI test calibrated to that margin passed locally and failed
  // on CI by one character. What the argument DOES with the budget is
  // pinned by the `enforceEnvelopeBudget` tests in `mutant.test.ts`.
