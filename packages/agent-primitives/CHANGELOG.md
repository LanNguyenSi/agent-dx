...
- README's `probe` section and `assets/skill/SKILL.md` document two
  probe traps observed while probing another package: a `-t` test-name
  filter is a runner-interpreted pattern (a regular expression for
  vitest/jest) that can silently match zero tests, and a mutant that
  breaks the project's own build under `--pre` reports `pre_failed`
  rather than a verdict, a common outcome for a naive literal
  replacement of a type-narrowing condition.
...
- `probe`'s phpunit zero-tests reading gains a dedicated, additive
  ...
  off after it read `"ambiguous"` where the pre-anchoring pattern read
  `"not_zero"`, a probe refusal, and a `--no-results` run lost its
  `Summary.attempted`); an open letter class matched it but also matched
  a line of one ordinary word plus the counter (`Aborted 5 / 9 ( 55%)`),
  turning a report-less output from `"ambiguous"` into `"not_zero"` and
  letting such a line override `Summary.attempted`. The closed alphabet
  fails in the safe direction (an unknown marker means a refusal, not a
  false pass) and cannot go stale unnoticed: a new directory-derived
  positive control asserts every counter-shaped line across every
  captured phpunit fixture is matched by this pattern (empty allowlist
  today, with its own mechanism test proving that allowlist cannot be
  silently neutralised), and negative cases pin that a single-word line
  ending in the counter shape is neither completion evidence nor an
  `attempted` source. Separately, the horizontal padding
...
- Envelope coverage now uses a small, deterministic 1,111-record fixture for
  ...
  test's resource profile. `--require-baseline-evidence` remains
...
- `probe`/`--plan` with `--pre` and `-i inplace` (the default) now
  ...
  skill. A library caller (`exitOnSignal: false`) that reaches this
  skipped-rebuild path with a mutant already applied and the target
...
- `verify` accepts an opt-in per-check success predicate, `--pass-regex
  ...
  `-x`-overridden is a usage error rather than a silent no-op. An empty
  pattern (e.g. a stray trailing `=` in `name=`) is a usage error too,
  since it would otherwise match any output unconditionally. A check a
  predicate decided `pass` reports the detector's own parsed summary,
  with no synthetic failure entry added -- but when that parse still
  shows a nonzero `summary.failed` or a nonempty `failures` list, a
  warning names the detector and the count, since the verdict stays
  `pass` (the predicate decides once configured) while the pattern may
  be too loose. Exit 126/127, a timeout, and an aborted check are never
  reclassified by a predicate, and each now warns that the predicate was
  never consulted, rather than dropping it silently. Edges said out loud
  rather than left silent: a predicate on a requested check that
  resolves to `skipped` (no script, no `-x`) adds a warning that the
  predicate was never consulted; a predicate-decided `fail` labels its
  synthetic failure entry from the pattern
  (`--pass-regex (...) did not match: ...`, warning
  `pass_regex_matched_nothing`) rather than from an exit code that was
  `0`; and the warnings carry `probe`'s own refinements, the 128+N
  signal band ("the suite may have been cut short") and the
  truncated-tail caveat, through `nonZeroPassWarning`/`truncationNote`
  helpers now shared from `src/pass-regex.ts` (a behaviour-preserving
  move out of `probe`'s setup, byte-identical text there). A repeated
  `--pass-regex` for one check is last-wins. No config-file
  (`.agent-primitives.json`) home for the predicate yet -- recorded as a
  follow-up on #241. The package's skill (`assets/skill/SKILL.md`) was
  updated to match; consumers should re-run `agent-primitives init`
  after this update to pick that up.
